...
u8 gbColorTable[] = {0,  0,  0,  1,  2,  3,  4,  5,  8,  0, 0,
                     0,  0,  0,  9,  10, 11, 12, 0,  0,  0, 13,
                     14, 15, 16, 17, 0,  18, 19, 20, 21, 23};

u8 gbFontColors[24][8] = {{0xC0, 0x9B, 0x9A, 0x95, 0x43, 0x00, 0x00, 0x28},
...
void Bitmap::blitKoreanChar(const char *ch,
    ...
    if (!gulim_9pt) {
        ...
        lFont.lfHeight = -MulDiv(
            8, GetDeviceCaps(screenDc, LOGPIXELSY), 72);  // Not in StarCraft
    ...
    if (!bufferBmp) {
        ...
        bufferBmp = CreateCompatibleBitmap(
            mainDc, KOR_CHAR_MAX_WIDTH, KOR_CHAR_MAX_HEIGHT);
    ...
    switch (fontSize) {
        case 0:
            ...
            break;  // SC uses 9pt, but who cares :P
        case 1:
            currentFont = gulim_9pt;
            break;
        case 2:
            currentFont = gulim_10pt;
            break;  // SC uses 11pt, but who cares :P
        case 3:
        default:
    ...
    for (int yOff = 0; yOff < chRect.bottom; ++yOff) {
        for (int xOff = 0; xOff < chRect.right; ++xOff) {
            if (bitmapBuffer[xOff + KOR_CHAR_MAX_WIDTH * yOff]) {
                this->drawDot(x + xOff + 1,
                              y + yOff + 3,
                              gbFontColors[color][0]);  // Shadow
...
bool Bitmap::blitString(const char *pszStr, int x, int y, u8 size) {
    ...
    if (x + fnt->getTextWidth(pszStr) < 0 ||
        y + fnt->getTextHeight(pszStr) < 0 || x >= this->getWidth() ||
        y >= this->getHeight())
        return false;
    ...
    for (int c = 0; pbChars[c]; ++c) {
        ...
        if (pbChars[c] <= ' ') {
            switch (pbChars[c]) {
                ...
                case 19:  // center align
                    Xoffset +=
                        (this->getWidth() - fnt->getTextWidth(pszStr)) / 2 - x;
                    continue;
                case ' ':  // space
        ...
        if (GetUserDefaultLangID() == MAKELANGID(LANG_KOREAN, SUBLANG_KOREAN) &&
            ...
            this->blitKoreanChar(
                (char *)&pbChars[c], Xoffset, Yoffset, size, color);
            if (pbChars[++c]) continue;
            break;
        }
        ...
        if (color != ~0) {
            ...
            for (int i = 0, pos = 0;
                 pos < fntChr->getHeight() * fntChr->getWidth();
                 ++i, ++pos) {
                // font position
                ...
                // x offset
                int newX =
                    Xoffset + (fntChr->getX() + pos % fntChr->getWidth());
                if (newX >= this->getWidth()) break;
                if (newX < 0) continue;

                // y offset
                int newY =
                    Yoffset + (fntChr->getY() + pos / fntChr->getWidth());
...
void Bitmap::drawLine(int x1, int y1, int x2, int y2, ColorId color) {
    // If horizontal
    if (y1 == y2) this->drawHorizontalLine(x1, x2, y1, color);
    ...
    // Trivial check
    if (this->isLineTriviallyIgnorable(x1, y1, x2, y2)) return;
...
void Bitmap::drawBox(int left, int top, int right, int bottom, ColorId color) {
    ...
    if (top >= 0) this->drawHorizontalLineUnsafe(xMin, xMax, top, color);
    ...
    if (left >= 0) this->drawVerticalLineUnsafe(left, yMin, yMax, color);
...
void Bitmap::drawCircle(int x, int y, int radius, ColorId color) {
    ...
    do {
        ...
        if (r > px || err > py)
            err += ++px * 2 + 1; /* e_xy+e_x > 0 or no 2nd y-step */
    } while (px < 0);
}
...
void Bitmap::drawFilledCircle(int x, int y, int radius, ColorId color) {
    ...
    do {
        this->drawHorizontalLine(
            x + px, x - px, y + py, color);  // Quadrants 1, 2
        this->drawHorizontalLine(
            x + px, x - px, y - py, color);  // Quadrants 3, 4
        const int r = err;
        if (r <= py) err += ++py * 2 + 1; /* e_xy+e_y < 0 */
        if (r > px || err > py)
            err += ++px * 2 + 1; /* e_xy+e_x > 0 or no 2nd y-step */
...
void Bitmap::drawDottedEllipse(int left,
    ...
    while (y0 - y1 < b) { /* too early stop of flat ellipses a=1 */
        ...
        if (nIgnoreHalf % 20 < 5) {
            this->drawDot(x0 - 1, y0, color); /* -> finish tip of ellipse */
            this->drawDot(
                x1 + 1, y0++, color);  // this->drawDot(x1+1, y0++, color);
            this->drawDot(x0 - 1, y1, color);
            this->drawDot(
                x1 + 1, y1--, color);  // this->drawDot(x1+1, y1--, color);
        } else {
            y0++;
            y1--;
        }
...
void Bitmap::drawVerticalLineUnsafe(int x, int y1, int y2, ColorId color) {
    ...
    for (int y = y1; y <= y2; ++y) this->drawDotUnsafe(x, y, color);
}
