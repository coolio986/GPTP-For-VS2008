...
FontChar *Font::getChar(char c) const {
    u8 ch = (u8)c;
    if (this != NULL && this->low <= ch && ch <= this->high &&
...
int Font::getTextWidth(const char *pszStr) const {
    ...
    // Retrieve size
    int size = 0;
    for (; *pszStr; ++pszStr) size += this->getCharWidth(*pszStr);
    return size;
}
...
int Font::maxWidth() const {
    if (this != NULL) return this->xMax;
    return 0;
}

int Font::maxHeight() const {
    if (this != NULL) return this->yMax;
    return 0;
}

int Font::getTextWidth(const char *pszStr, int size) {
    if (0 <= size && size <= 3) return fontBase[size]->getTextWidth(pszStr);
    return 0;
}

int Font::getTextHeight(const char *pszStr, int size) {
    if (0 <= size && size <= 3) return fontBase[size]->getTextHeight(pszStr);
    return 0;
}
...
int FontChar::getWidth() const {
    if (this != NULL) return this->width;
    return 0;
}

int FontChar::getHeight() const {
    if (this != NULL) return this->height;
    return 0;
}

int FontChar::getX() const {
    if (this != NULL) return this->x;
    return 0;
}

int FontChar::getY() const {
    if (this != NULL) return this->y;
    return 0;
}
