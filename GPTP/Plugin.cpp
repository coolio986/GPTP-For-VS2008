...
//  "The plugin's name string is longer than the maximum limit set by
//  MPQDraft");
...
BOOL WINAPI Plugin::GetPluginName(LPSTR lpszPluginName,
...
BOOL WINAPI Plugin::CanPatchExecutable(LPCSTR lpszEXEFileName) {
    if (lpszEXEFileName == NULL) {
        MessageBox(
            NULL, "Invalid path to patch target executable", NULL, MB_OK);
        return FALSE;
    }

    if (!checkStarCraftExeVersion(lpszEXEFileName)) return FALSE;
...
BOOL Plugin::checkStarCraftExeVersion(LPCSTR exePath) const {
    ...
    if (fileVersionInfoBufferSize > 0) {
        ...
        if (GetFileVersionInfo(exePath,
                               NULL,
                               fileVersionInfoBufferSize,
                               fileVersionInfoBuffer) &&
            VerQueryValue(fileVersionInfoBuffer,
                          "\\",
                          (LPVOID *)&fileInfo,
                          &fileInfoSize)) {
            ...
            if (requiredVersion[0] == currentVersion[0] &&
                ...
                result = TRUE;
            } else if (currentVersion[0] == 1 && currentVersion[1] == 0 &&
                       currentVersion[2] == 0 && currentVersion[3] == 2) {
                sprintf_s(errorMessage,
                          "Error: GPTP cannot patch %s"
                          "\n"
                          "\n StarEdit may be able to be loaded, but the "
                          "plugin features"
                          "\n cannot connect, so the mod is more likely to be "
                          "incompatible"
                ...
                sprintf_s(
                    ...
                    ") is incompatible with the current version of StarCraft."
                    " The game will still be loaded, but no plugin features "
                    "will be available."
                    "\n"
        ...
        sprintf_s(errorMessage,
                  "Cannot retrieve version information size from:\n  %s",
                  exePath);
    }

    if (!result) MessageBox(NULL, errorMessage, NULL, MB_OK);
    return result;
}
