...
void runWMode() {
    ...
    int msgboxID = MessageBox(
        NULL, "Do you want to play in W-Mode?", PLUGIN_NAME, MB_YESNOCANCEL);
    switch (msgboxID) {
        case IDYES:
            HINSTANCE hDll;
...
BOOL WINAPI Plugin::InitializePlugin(IMPQDraftServer *lpMPQDraftServer) {
    ...
    if (pathLen == sizeof(exePath)) {
        MessageBox(NULL,
                   "Error: Cannot check version of StarCraft.exe. The file "
                   "path is too long.",
                   NULL,
                   MB_OK);
        return FALSE;
    ...
    if (!checkStarCraftExeVersion(exePath)) return FALSE;
