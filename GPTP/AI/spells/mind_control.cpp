...
class MindControlTargetFinderProc
    : public scbw::UnitFinderCallbackMatchInterface {
   private:
    CUnit* caster;

   public:
    MindControlTargetFinderProc(CUnit* caster) : caster(caster) {}
    ...
    bool match(CUnit* target) {
        ...
        if (target->id == UnitId::shuttle || target->id == UnitId::dropship)
            if (target->hasLoadedUnit()) return true;
        ...
        switch (target->id) {
            ...
            default:
                if (units_dat::MineralCost[target->id] >=
                        units_dat::MineralCost[UnitId::dark_templar] &&
                    units_dat::GasCost[target->id] >=
                        units_dat::GasCost[UnitId::dark_templar])
                    return true;
                break;
        }
...
CUnit* findBestMindControlTarget(CUnit* caster, bool isUnderAttack) {
    ...
    return scbw::UnitFinder::getNearestTarget(
        ...
        caster->getY() + bounds,
        caster,
        MindControlTargetFinderProc(caster));
}
