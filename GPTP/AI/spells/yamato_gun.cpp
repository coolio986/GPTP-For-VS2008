...
class YamatoGunTargetFinderProc
    : public scbw::UnitFinderCallbackMatchInterface {
   private:
    CUnit* caster;

   public:
    YamatoGunTargetFinderProc(CUnit* caster) : caster(caster) {}
    ...
    bool match(CUnit* target) {
        ...
        if (target->id == UnitId::egg) return false;
...
CUnit* findBestYamatoGunTarget(CUnit* caster, bool isUnderAttack) {
    if (isUnderAttack) {
        ...
        return scbw::UnitFinder::getNearestTarget(
            ...
            caster->getY() + bounds,
            caster,
            YamatoGunTargetFinderProc(caster));
    } else {
        return scbw::UnitFinder::getNearestTarget(
