...
class DarkSwarmTargetFinderProc
   ...
   private:
    ...
    Bool32 weaponBehaviourCanHitTargetCheck(CUnit* attacker, CUnit* target) {
        ...
        // code equivalent to isUnderDarkSwarm @ 0x00476210, a function
        // different from scbw::isUnderDarkSwarm(target) that just seek a nearby
        ...
        if (target->status & UnitStatus::InAir)
            isUnderDarkSwarmStatus = 0;
                 ...
                 (*offset_0x005873F0 == 0) || !scbw::isUnderDarkSwarm(target)) {
            s32 targetX = target->getX();
            s32 targetY = target->getY();

            if (targetX < 0) targetX += 31;
        ...
        if (attacker->status & UnitStatus::InAir)
            ...
            if (scbw::getGroundHeightAtPos(attacker->getX(),
                                           attacker->getY()) >=
                scbw::getGroundHeightAtPos(target->getX(), target->getY()))
                returnValue = isUnderDarkSwarmStatus;
            else if (isUnderDarkSwarmStatus <=
                     OrderId::DarkSwarm) /*code use 0x77, the order seems to
                                            fit,but maybe*/
                returnValue = OrderId::DarkSwarm; /*that's just an accident and
                                                     it's unrelated*/
            else
                returnValue = isUnderDarkSwarmStatus;
        }
    ...
    bool match(CUnit* target) {
        ...
        if (isTargetWorthHitting(target, caster)) {
            ...
            if (targetOfTarget != NULL) {
                if (targetOfTarget->playerId < PLAYABLE_PLAYER_COUNT &&
                    ...
                    if (totGroundWeapon != WeaponId::None &&
                        weapons_dat::Behavior[totGroundWeapon] ==
                        ...
                        if (targetGroundWeapon != WeaponId::None) {
                            u8 weapon_behavior =
                                weapons_dat::Behavior[targetGroundWeapon];

                            if (weapons_dat::Behavior[targetGroundWeapon] ==
                                    WeaponBehavior::Fly_DoNotFollowTarget ||
                                weapons_dat::Behavior[targetGroundWeapon] ==
                                    WeaponBehavior::Fly_FollowTarget ||
                                weapons_dat::Behavior[targetGroundWeapon] ==
                                    WeaponBehavior::AppearOnTargetUnit ||
                                weapons_dat::Behavior[targetGroundWeapon] ==
                                    WeaponBehavior::AppearOnTargetSite ||
                                weapons_dat::Behavior[targetGroundWeapon] ==
                                    WeaponBehavior::Bounce)
                                bReturnValue = weaponBehaviourCanHitTargetCheck(
                                                   target, targetOfTarget) == 0;
                            else
                                bReturnValue = false;
        ...
        // if (weapons_dat::Behavior[totGroundWeapon] !=
        // WeaponBehavior::AppearOnAttacker)
        //  return false;
...
CUnit* findBestDarkSwarmTarget(CUnit* caster, bool isUnderAttack) {
    ...
    // Based on AI_BestUnit @ 0x004E8740 and canHitUnit @ 0x00441870
    CUnit* result =
        scbw::UnitFinder::getNearestTarget(caster->getX() - bounds,
                                           caster->getY() - bounds,
                                           caster->getX() + bounds,
                                           caster->getY() + bounds,
                                           caster,
                                           DarkSwarmTargetFinderProc(caster));
