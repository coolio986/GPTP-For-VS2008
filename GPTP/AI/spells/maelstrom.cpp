...
class MaelstromTargetFinderProc
   ...
   public:
    ...
    bool match(CUnit* target) {
        ...
        if (target->maelstromTimer) return false;
        ...
        if (isTargetAttackingAlly(target, caster) &&
            target->id != UnitId::larva && target->id != UnitId::egg &&
            target->id != UnitId::cocoon && target->id != UnitId::lurker_egg) {
            const int totalEnemyLife =
                getTotalEnemyLifeInArea(target->getX(),
                                        target->getY(),
                                        96,
                                        caster,
                                        WeaponId::Maelstrom);
            ...
            const int totalAllyLife =
                getTotalAllyLifeInArea(target->getX(),
                                       target->getY(),
                                       96,
                                       caster,
                                       WeaponId::Maelstrom);
...
CUnit* findBestMaelstromTarget(CUnit* caster, bool isUnderAttack) {
    ...
    return scbw::UnitFinder::getNearestTarget(
        ...
        caster->getY() + bounds,
        caster,
        MaelstromTargetFinderProc(caster, isUnderAttack));
}
