...
class LockdownFinderProc : public scbw::UnitFinderCallbackMatchInterface {
   ...
   public:
    LockdownFinderProc(CUnit* caster) : caster(caster) {}
    ...
    bool match(CUnit* target) {
        ...
        if (target->id == UnitId::dropship || target->id == UnitId::shuttle) {
            if (target->movementFlags & MovementFlags::Accelerating &&
        ...
        if (isTargetAttackingAlly(target, caster)) return true;
