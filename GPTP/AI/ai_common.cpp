...
bool isTargetWorthHitting(CUnit* target, CUnit* attacker) {
    ...
    if (!target->sprite->isVisibleTo(attacker->playerId)) return false;
    ...
    if (!attacker->isTargetEnemy(target)) return false;

    if (target->id == UnitId::scarab || target->id == UnitId::spider_mine ||
...
bool isTargetAttackingAlly(CUnit* target, CUnit* unit) {
    if (CUnit* targetOfTarget = target->orderTarget.unit) {
        if (targetOfTarget->playerId < 8 &&
            !unit->isTargetEnemy(targetOfTarget))
            return true;
    }
...
bool isUnitInUnsafeRegion(CUnit* unit) {
    ...
    return currentAiCaptain->captainType == 3 ||
           currentAiCaptain->unknown_0x1C || currentAiCaptain->unknown_0x20 ||
...
int getTotalEnemyLifeInArea(int x,
    ...
    for (int i = 0; i < unitStatTotalFinder.getUnitCount(); ++i) {
        ...
        if ((target != caster) && !(target->status & UnitStatus::Invincible) &&
        ...
        // if (!bStopChecking && !scbw::canWeaponTargetUnit(weaponId, target,
        // caster))
        //  bStopChecking = true;
...
int getTotalEnemyEnergyInArea(int x, int y, int searchBounds, CUnit* caster) {
    ...
    for (int i = 0; i < unitStatTotalFinder.getUnitCount(); ++i) {
        ...
        if (!(target->status & UnitStatus::Invincible) &&
            caster->isTargetEnemy(target) && target->isValidCaster())
            totalEnemyEnergy +=
                (&target->energy)[1];  // unitPointer+0xA3 in original code
    }
...
int getTotalEnemyNukeValueInArea(int x,
    ...
    for (int i = 0;
         i < unitStatTotalFinder.getUnitCount() && totalNukeTargetValue < 800;
         ++i) {
        CUnit* target = unitStatTotalFinder.getUnit(i);
