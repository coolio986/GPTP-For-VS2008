...
bool AI_spellcasterHook(CUnit* unit, bool isUnitBeingAttacked) {
    ...
    switch (unit->id) {
        case UnitId::battlecruiser:
            // Yamato Gun
            if (canCastSpellOrder(
                    unit, TechId::YamatoGun, OrderId::FireYamatoGun1)) {
                CUnit* target =
                    findBestYamatoGunTarget(unit, isUnitBeingAttacked);
        ...
        case UnitId::ghost:
            // Lockdown
            if (canCastSpellOrder(
                    unit, TechId::Lockdown, OrderId::MagnaPulse)) {
                CUnit* target =
                    findBestLockdownTarget(unit, isUnitBeingAttacked);
            ...
            if (isNukeTimerReady(unit->playerId)) {
                ...
                CUnit* target =
                    findBestNukeLaunchTarget(unit, isUnitBeingAttacked);

                if (aiCastSpellOrder(unit, target, OrderId::NukePaint)) {
                    silo->building.silo.nuke->connectedUnit = unit;
                    AIScriptController[unit->playerId].AI_LastNukeTime =
                        *elapsedTimeSeconds;
                    return true;
                }
            }
        ...
        case UnitId::science_vessel:
            // EMP Shockwave
            if (canCastSpellOrder(
                    unit, TechId::EMPShockwave, OrderId::EmpShockwave)) {
                CUnit* target =
                    findBestEmpShockwaveTarget(unit, isUnitBeingAttacked);
            ...
            // Defensive Matrix
            if (canCastSpellOrder(
                    unit, TechId::DefensiveMatrix, OrderId::DefensiveMatrix)) {
                CUnit* target =
                    findBestDefensiveMatrixTarget(unit, isUnitBeingAttacked);
            ...
            // Irradiate
            if (canCastSpellOrder(
                    unit, TechId::Irradiate, OrderId::Irradiate)) {
                CUnit* target =
                    findBestIrradiateTarget(unit, isUnitBeingAttacked);
        ...
        case UnitId::medic:
            // Restoration
            if (canCastSpellOrder(
                    unit, TechId::Restoration, OrderId::Restoration)) {
                CUnit* target =
                    findBestRestorationTarget(unit, isUnitBeingAttacked);
                ...
                if (unit == AIScriptController[unit->id].mainMedic &&
                    ...
                    CUnit* targetSituational =
                        findBestRestorationTargetSituational(
                            unit, isUnitBeingAttacked);
                    if (aiCastSpellOrder(
                            unit, targetSituational, OrderId::Restoration))
                        return true;
                }
            }
            ...
            if (unit->getMaxEnergy() == unit->energy &&
                canCastSpellOrder(
                    unit, TechId::OpticalFlare, OrderId::CastOpticalFlare)) {
                CUnit* target =
                    findBestOpticalFlareTarget(unit, isUnitBeingAttacked);
        ...
        case UnitId::queen:
            ...
            if (isUmsMode(unit->playerId)
                ...
                if (canCastSpellOrder(
                        unit, TechId::Parasite, OrderId::CastParasite)) {
                    CUnit* target =
                        findBestParasiteTarget(unit, isUnitBeingAttacked);
                    ...
                    if (aiCastSpellOrder(
                            unit, target, OrderId::CastParasite, 4))
                        return true;

                    return false;
                }

                if (isUmsMode(unit->playerId)) return false;
            }

            // Spawn Broodlings
            if (canCastSpellOrder(
                    unit, TechId::SpawnBroodlings, OrderId::SummonBroodlings)) {
                CUnit* target =
                    findBestSpawnBroodlingsTarget(unit, isUnitBeingAttacked);
            ...
            if ((isUmsMode(unit->playerId) ||
                ...
                CUnit* target =
                    findBestEnsnareTarget(unit, isUnitBeingAttacked);
        ...
        case UnitId::defiler:
            ...
            // Dark Swarm
            if (canCastSpellOrder(
                    unit, TechId::DarkSwarm, OrderId::DarkSwarm)) {
                CUnit* target =
                    findBestDarkSwarmTarget(unit, isUnitBeingAttacked);
        ...
        case UnitId::high_templar:
            // Psionic Storm
            if (canCastSpellOrder(
                    unit, TechId::PsionicStorm, OrderId::PsiStorm)) {
                CUnit* target =
                    findBestPsiStormTarget(unit, isUnitBeingAttacked);
            ...
            if (unit->getMaxEnergy() == unit->energy &&
                canCastSpellOrder(
                    unit, TechId::Hallucination, OrderId::Hallucianation1)) {
                CUnit* target =
                    findBestHallucinationTarget(unit, isUnitBeingAttacked);
        ...
        case UnitId::arbiter:
            // Stasis Field
            if (canCastSpellOrder(
                    unit, TechId::StasisField, OrderId::StasisField)) {
                CUnit* target =
                    findBestStasisFieldTarget(unit, isUnitBeingAttacked);
        ...
        case UnitId::dark_archon:
            // Feedback
            if (canCastSpellOrder(
                    unit, TechId::Feedback, OrderId::CastFeedback)) {
                CUnit* target =
                    findBestFeedbackTarget(unit, isUnitBeingAttacked);
            ...
            // Mind Control
            if (canCastSpellOrder(
                    unit, TechId::MindControl, OrderId::CastMindControl)) {
                CUnit* target =
                    findBestMindControlTarget(unit, isUnitBeingAttacked);
        ...
        case UnitId::corsair:
            // Disruption Web
            if (canCastSpellOrder(
                    unit, TechId::DisruptionWeb, OrderId::CastDisruptionWeb)) {
                CUnit* target =
                    findBestDisruptionWebTarget(unit, isUnitBeingAttacked);
...
bool canCastSpellOrder(CUnit* unit, u8 techId, u8 orderId) {
    ...
    if (orders_dat::TechUsed[orderId] < TechId::None)
        energyCost =
            techdata_dat::EnergyCost[orders_dat::TechUsed[orderId]] * 256;
...
bool aiCastSpellOrder(CUnit* unit, CUnit* target, u8 orderId, u8 aiActionFlag) {
    if (!target || target->aiActionFlags & aiActionFlag) return false;
...
bool isNukeTimerReady(u8 playerId) {
    return *elapsedTimeSeconds >=
           AIScriptController[playerId].AI_LastNukeTime +
