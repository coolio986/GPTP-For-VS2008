...
bool isMorphedBuildingHook(u16 unitId) {
    ...
    if (unitId == UnitId::lair || unitId == UnitId::hive ||
        unitId == UnitId::greater_spire || unitId == UnitId::sunken_colony ||
...
// Determines the number of units which have @p unitId and are being built by
// the
...
int getMorphBuildingTypeCountHook(const CUnit *unit,
    ...
    switch (unitId) {
        case UnitId::hatchery:
            unitCount +=
                getNumberOfUnitType(unit, UnitId::lair, ignoreIncomplete);
        case UnitId::lair:
            unitCount +=
                getNumberOfUnitType(unit, UnitId::hive, ignoreIncomplete);
            break;

        case UnitId::creep_colony:
            unitCount += getNumberOfUnitType(
                unit, UnitId::sunken_colony, ignoreIncomplete);
            unitCount += getNumberOfUnitType(
                unit, UnitId::spore_colony, ignoreIncomplete);
            break;

        case UnitId::spire:
            unitCount += getNumberOfUnitType(
                unit, UnitId::greater_spire, ignoreIncomplete);
            break;
    }
