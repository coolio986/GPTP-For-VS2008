...
bool isEggUnitHook(u16 unitId) {
    ...
    if (unitId == UnitId::egg || unitId == UnitId::cocoon ||
...
bool isRallyableEggUnitHook(u16 unitId) {
    ...
    if (unitId == UnitId::cocoon || unitId == UnitId::lurker_egg) return false;
...
u16 getUnitMorphEggTypeHook(u16 unitId) {
    ...
    if (unitId == UnitId::larva) return UnitId::egg;

    if (unitId == UnitId::mutalisk) return UnitId::cocoon;

    if (unitId == UnitId::hydralisk) return UnitId::lurker_egg;
...
u16 getCancelMorphRevertTypeHook(CUnit* eggUnit) {
    ...
    if (eggUnit->id == UnitId::cocoon) return UnitId::mutalisk;

    if (eggUnit->id == UnitId::lurker_egg) return UnitId::hydralisk;
...
s16 getUnitVerticalOffsetOnBirth(CUnit* unit) {
    ...
    if (unit->previousUnitType == UnitId::cocoon) return 0;
    ...
    if (unit->status & UnitStatus::InAir) return -42;
...
bool hasSuppliesForUnitHook(u8 playerId, u16 unitId, bool canShowErrorMessage) {
    ...
    if (supplyCost == 0 ||
        units_dat::BaseProperty[unitId] & UnitProperty::MorphFromOtherUnit)
        return true;
