...
void __fastcall cancelUnitWrapper(CUnit* unit) {
    ...
    if (unit->id == UnitId::mutalisk || unit->id == UnitId::hydralisk) return;

    // Don't bother if unit has finished morphing
    if (unit->id == UnitId::guardian || unit->id == UnitId::devourer ||
    ...
    if (unit->status & UnitStatus::GroundedBuilding) {
        ...
        resources->minerals[unit->playerId] +=
            units_dat::MineralCost[unit->id] * 3 / 4;
        ...
        resources->minerals[unit->playerId] +=
            units_dat::MineralCost[refundUnitId];
    ...
    if (cancelChangeUnitId == UnitId::None) {
        ...
        replaceSpriteImages(
            ...
            sprites_dat::ImageId
                [flingy_dat::SpriteID
                     [units_dat::Graphic[unit->previousUnitType]]],
            0);
...
s32 getRemainingBuildTimePctHook(CUnit* unit) {
    ...
    return 100 * (units_dat::TimeCost[unitId] - unit->remainingBuildTime) /
           units_dat::TimeCost[unitId];
}
...
void injectUnitMorphHooks() {
    callPatch(unitMorphWrapper_CMDRECV_UnitMorph,
              0x00486B50);  // jmp 004C1990  CMDRECV_UnitMorph
    jmpPatch(unitMorphWrapper_BTNSCOND_CanBuildUnit,
             0x00428E60);  // 00428E60  BTNSCOND_CanBuildUnit
    jmpPatch(unitMorphWrapper_Orders_Morph1_Check,
             0x0045DFB0);  // 0045DEA0  orders_Morph1 within
    jmpPatch(unitMorphWrapper_Orders_Morph1_EggType,
             0x0045E019);  // 0045DEA0  orders_Morph1 within
    jmpPatch(hasSuppliesForUnitWrapper,
             0x0042CF70);  // 0042CF70  hasSuppliesForUnit EXACT
    jmpPatch(cancelUnitWrapper,
             0x00468280);  // 00468280  AI_CancelStructure EXACT
    jmpPatch(getRemainingBuildTimePctWrapper,
             0x004669E0);  // 004669E0  sub_4669E0 EXACT
    jmpPatch(getUnitVerticalOffsetOnBirthWrapper,
             0x0045DE00);  // 0045DD60  orders_ZergBirth within
    jmpPatch(isRallyableEggUnitWrapper,
             0x0045DE57);  // 0045DD60  orders_ZergBirth within
}
