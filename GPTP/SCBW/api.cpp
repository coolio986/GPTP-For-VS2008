...
class DarkSwarmFinderProc : public scbw::UnitFinderCallbackMatchInterface {
   public:
    bool match(CUnit* unit) { return unit->id == UnitId::Spell_DarkSwarm; }
};
...
bool canWeaponTargetUnit(u8 weaponId, CUnit* target, CUnit* attacker) {
    if (weaponId >= WEAPON_TYPE_COUNT) return false;

    if (!target) return weapons_dat::TargetFlags[weaponId].terrain;
    ...
    if (tf.organic && !(targetProps & UnitProperty::Organic)) return false;

    if (tf.nonBuilding && (targetProps & UnitProperty::Building)) return false;

    if (tf.nonRobotic && (targetProps & UnitProperty::RoboticUnit))
        return false;

    if (tf.orgOrMech &&
        !(targetProps & (UnitProperty::Organic | UnitProperty::Mechanical)))
        return false;

    if (tf.playerOwned && target->playerId != attacker->playerId) return false;
...
int arctangent(int slope) {
    const unsigned int tangentTable[] = {
        7,    13,   19,   26,   32,   38,   45,   51,    58,  65,   71,
        78,   85,   92,   99,   107,  114,  122,  129,   137, 146,  154,
        163,  172,  181,  190,  200,  211,  221,  233,   244, 256,  269,
        283,  297,  312,  329,  346,  364,  384,  405,   428, 452,  479,
        509,  542,  578,  619,  664,  716,  775,  844,   926, 1023, 1141,
        1287, 1476, 1726, 2076, 2600, 3471, 5211, 10429, -1};
...
s32 getSupplyRemaining(u8 playerId, u8 raceId) {
    ...
    if (isCheatEnabled(CheatFlags::FoodForThought))
        ...
        supplyProvided = std::min(raceSupply[raceId].max[playerId],
                                  raceSupply[raceId].provided[playerId]);
...
bool hasTechResearched(u8 playerId, u16 techId) {
    ...
    if (techId < TechId::Restoration)
        ...
        return TechBw->isResearched[playerId][techId - TechId::Restoration] !=
               0;
}
...
void setTechResearchState(u8 playerId, u16 techId, bool isResearched) {
    ...
    if (techId < TechId::Restoration)
        ...
        TechBw->isResearched[playerId][techId - TechId::Restoration] =
            isResearched;
}
...
u8 getUpgradeLevel(u8 playerId, u8 upgradeId) {
    ...
    if (upgradeId < UpgradeId::UnusedUpgrade46)
        ...
        return UpgradesBw
            ->currentLevel[playerId][upgradeId - UpgradeId::UnusedUpgrade46];
}
...
void refreshScreen() { memset(refreshRegions, 1, 1200); }
...
u16 random() {
    if (*IS_IN_GAME_LOOP) {
        *lastRandomNumber = 22695477 * (*lastRandomNumber) + 1;
        return (*lastRandomNumber >> 16) %
               32768;  // Make a number between 0 and 32767
    } else
        return 0;
}
