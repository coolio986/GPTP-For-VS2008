...
/// The first nodes in StarCraft's internal linked lists (taken from BWAPI's
/// Offsets.h)
...
/// Units that are selected by the current player (or the player viewing the
/// replay).
struct UnitsSel {
    CUnit* unit[SELECTION_ARRAY_LENGTH];
};
...
/// Maybe used by Map triggers to init a progressive movement toward the
/// corresponding location, see GameImpl::setScreenPosition of BWAPI
SCBW_DATA(u32*, MoveToX, 0x0062848C);
SCBW_DATA(u32*, MoveToY, 0x006284A8);

/// Contains various information (names, player types, race types, and
/// associated forces) of each player in the current game
...
SCBW_DATA(SupplyData*,
          raceSupply,
          0x00582144);  // Array; Use CUnit::getRace() to get the index.

SCBW_DATA(const u32*,
          elapsedTimeFrames,
          0x0057F23C);  // Elapsed game time in frames
SCBW_DATA(const u32*,
          CountdownTimer,
          0x0058D6F4);  // Countdown Timer (in seconds) (From BWAPI Offsets.h)
SCBW_DATA(const u32*,
          elapsedTimeSeconds,
          0x0058D6F8);  // Elapsed game time in seconds
SCBW_DATA(
    const u8*,
    GAME_TYPE,
    0x00596820);  // Part of a larger structure; Compare with GameType::Enum.
SCBW_DATA(const s32*, ACTIVE_NATION_ID, 0x00512678);  // AKA g_ActiveNationID
SCBW_DATA(const s32*,
          ACTIVE_PLAYER_ID,
          0x0051267C);  // Used by GetActivePlayerNextSelectionFunc
SCBW_DATA(const s32*,
          LOCAL_NATION_ID,
          0x00512684);  // AKA g_LocalNationID; Actually stores the player ID.
SCBW_DATA(const s32*,
          LOCAL_HUMAN_ID,
          0x00512688);  // AKA g_LocalHumanID; Invalid in replay games.
SCBW_DATA(
    const u8*,
    CURRENT_TILESET,
    0x00596828);  // Tileset of current map. Compare with TilesetType::Enum
////possible values were 0057F1DC and [00596828]

// Added from BWAPI Offsets.h, but made const since it would not be safe to
// modify those
...
// you can get the other addresses in
// http://code.google.com/p/bwapi/source/browse/trunk/bwapi/BWAPI/Source/BW/Offsets.h
...
SCBW_DATA(u16*,
          InfestedUnitPartial,
          unitsDat[3].address);  // 00664980, (Id - UnitId::TerranCommandCenter)
                                 // for it to work, last valid id is
                                 // UnitId::Special_OvermindCocoon
SCBW_DATA(u32*, ConstructionGraphic, unitsDat[4].address);  // 006610B0
SCBW_DATA(u8*,
          StartDirection,
          unitsDat[5].address);  // 006605F0  Direction unit will face after it
                                 // is created. From 0 (top) to 31 (before top
                                 // turning clockwise).32 means random.
...
SCBW_DATA(
    u32*,
    BaseProperty,
    unitsDat[22].address);  // 00664080 AKA SpecialAbilityFlags, UnitProperty,
                            // etc. Use with UnitProperty::Enum.
SCBW_DATA(u8*,
          SeekRange,
          unitsDat[23].address);  // 00662DB8 AKA Target Acquisition Range,
                                  // Acquire Target Range, etc.
...
SCBW_DATA(u16*,
          unitsDat43,
          unitsDat[43]
              .address);  // 00660A70 //"Restriction Address" according to EUDDB
SCBW_DATA(GroupFlag*, GroupFlags, unitsDat[44].address);  // 006637A0
SCBW_DATA(u8*,
          SupplyProvided,
          unitsDat[45].address);  // 006646C8 Supply shown in-game is (true
                                  // supply) / 2; e.g. Zerglings have 1 true
                                  // supply, Marines have 2 true supply, etc.
SCBW_DATA(u8*,
          SupplyRequired,
          unitsDat[46].address);  // 00663CE8 Supply shown in-game is (true
                                  // supply) / 2; e.g. Zerglings have 1 true
                                  // supply, Marines have 2 true supply, etc.
...
SCBW_DATA(u8*,
          TurnSpeed,
          flingyDat[4].address);  // 006C9E20 Incorrectly known as "Turn Radius"
// SCBW_DATA(u8*, ???,				flingyDat[5].address);
// //006CA240
...
SCBW_DATA(u8*,
          DamageType,
          weaponsDat[7].address);  // 00657258 Normal, Explosive, etc.. Use with
                                   // DamageType::Enum.
SCBW_DATA(
    u8*,
    Behavior,
    weaponsDat[8].address);  // 00656670 Fly and follow target, appear on target
                             // unit, etc. Use with WeaponBehavior::Enum.
SCBW_DATA(u8*, RemoveAfter, weaponsDat[9].address);  // 00657040
SCBW_DATA(
    u8*,
    ExplosionType,
    weaponsDat[10].address);  // 006566F8 NormalHit, RadialSplash, EnemySplash,
                              // etc. Use with WeaponEffect::Enum.
...
// //Restriction Flags according to EUDDB SCBW_DATA(u16*, ???,
// upgradesDat[7].address);	//00655AC0	//"Icon, cmdicons.grp frame"
// according to EUDDB
...
// SCBW_DATA(u8*, ???,				upgradesDat[11].address);
// //00655B3C	//Brood War Flag according to EUDDB
}  // namespace upgrades_dat
...
// SCBW_DATA(u16*, ???,				techdataDat[4].address);
// //00656198
// SCBW_DATA(u16*, ???,				techdataDat[5].address);
// //006562F8	//Restriction Flags according to EUDDB SCBW_DATA(u16*, ???,
// techdataDat[6].address);	//00656430	//"Icon, cmdicons.grp frame"
// according to EUDDB
SCBW_DATA(u16*, Label, techdataDat[7].address);  // 006562A0
// SCBW_DATA(u8*, ???,				techdataDat[8].address);
// //00656488	//Race according to EUDDB SCBW_DATA(u8*, ???,
// techdataDat[9].address);	//00656350	//"Researched" according to
// EUDDB SCBW_DATA(u8*, ???,
// techdataDat[10].address);	//006564B4	//Brood War Flag according to
// EUDDB
}  // namespace techdata_dat
...
// ordersDat[5].address);	//006657C0 //May be "Subunit can use" used for
// debug only
SCBW_DATA(Bool8*, CanBeInterrupted, ordersDat[6].address);  // 00665040
// SCBW_DATA(u8*,	???,				ordersDat[7].address);
// //00665100
// SCBW_DATA(u8*,	???,				ordersDat[8].address);
// //00665700 //"Can Be Queued" according to EUDDB SCBW_DATA(u8*,	???,
// ordersDat[9].address);	//006651C0
SCBW_DATA(u8*,
          CanBeObstructed,
          ordersDat[10]
              .address);  // 006654C0 //"Can Be Obstructed?" according to EUDDB
// SCBW_DATA(u8*,	???,				ordersDat[11].address);
// //00664C80
// SCBW_DATA(u8*,	???,				ordersDat[12].address);
// //00664BC0 ////May be "Order requires moving"
SCBW_DATA(u8*, OrderWeaponId, ordersDat[13].address);  // 00665880
SCBW_DATA(u8*, TechUsed, ordersDat[14].address);       // 00664E00
SCBW_DATA(u8*,
          OrderIscriptAnim,
          ordersDat[15].address);  // 00664D40 //Animation (IscriptAnimation)
                                   // according to EUDDB
SCBW_DATA(u16*, ButtonIcon, ordersDat[16].address);  // 00664EC0
SCBW_DATA(u16*,
          RequirementsOffset,
          ordersDat[17].address);  // 00665580 //based on context
// SCBW_DATA(u8*,	???,				ordersDat[18].address);
// //00665400 //"Obscured Order" according to EUDDB
}  // namespace orders_dat
...
// SCBW_DATA(s8*,  ???,				spritesDat[2].address);
// //00666570
SCBW_DATA(u8*, IsVisible, spritesDat[3].address);  // 00665C48
// SCBW_DATA(s8*,  ???,				spritesDat[4].address);
// //00665AC0 //"Selection Circle" according to EUDDB, not starting at 0
// SCBW_DATA(s8*,  ???,				spritesDat[5].address);
// //00665FD8 //"Selection Vertical Offset" according to EUDDB
}  // namespace sprites_dat
...
// //"Use Full Iscript" according to EUDDB SCBW_DATA(const u8*,  ???,
// imagesDat[4].address);	//00667718 //"Draw If Cloaked" according to
// EUDDB
...
// //"Iscript ID" according to EUDDB SCBW_DATA(const u32*,  ???,
// imagesDat[8].address);	//0066C538 //"Shields Overlay" according to
// EUDDB SCBW_DATA(const u32*,  ???,		imagesDat[9].address);
// //0066B1B0 //"Attack Overlay" according to EUDDB SCBW_DATA(const u32*,  ???,
// imagesDat[10].address);	//0066A210 //"Damage Overlay" according to EUDDB
// SCBW_DATA(const u32*,  ???,		imagesDat[11].address);	//00667B00
// //"Special Overlay" according to EUDDB SCBW_DATA(const u32*,  ???,
// imagesDat[12].address);	//00666778 //"Landing Dust Overlay" according to
// EUDDB SCBW_DATA(const u32*,  ???,		imagesDat[13].address);
// //0066D8C0 //"Lift-off Dust Overlay" according to EUDDB
}  // namespace images_dat
...
SCBW_DATA(GetActivePlayerNextSelectionFunc,
          getActivePlayerNextSelection,
          0x0049A850);

/// Selections of all players
struct PlayersSel {
    CUnit* unit[PLAYABLE_PLAYER_COUNT][SELECTION_ARRAY_LENGTH];
};
...
SCBW_DATA(const ColorShiftData*,
          colorShift,
          0x005128F8);  // Use ColorRemapping::Enum as index
...
SCBW_DATA(PlayerFlags<u8>*,
          playerAlliance,
          0x0058D634);  // See scbw::isAlliedTo()
...
SCBW_DATA(const Bool32*,
          IS_GAME_PAUSED,
          0x006509C4);                              // See scbw::isGamePaused()
SCBW_DATA(const Bool8*, IS_BROOD_WAR, 0x0058F440);  // See
                                                    // scbw::isBroodWarMode()
...
SCBW_DATA(u32*,
          lastRandomNumber,
          0x0051CA14);  // See scbw::random(), scbw::randBetween()
