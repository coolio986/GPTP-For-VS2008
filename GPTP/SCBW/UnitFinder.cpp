...
int UnitFinder::getUnitCount() const { return this->unitCount; }

CUnit* UnitFinder::getUnit(int index) const {
    if (0 <= index && index <= this->unitCount) return this->units[index];
    return NULL;
}

UnitFinderData* UnitFinder::getStartX() { return unitOrderingX; }

UnitFinderData* UnitFinder::getStartY() { return unitOrderingY; }
...
void UnitFinder::search(int left, int top, int right, int bottom) {
    ...
    if (isWidthExtended) r += *MAX_UNIT_WIDTH;
    if (isHeightExtended) b += *MAX_UNIT_HEIGHT;

    // Create UnitFinderData elements for compatibility with stl functions
    UnitFinderData finderVal;

    // Search for the values using built-in binary search algorithm and
    // comparator
    ...
    for (UnitFinderData* px = pLeft; px < pRight; ++px) {
        if (unitSearchFlags[px->unitIndex] == 1) {
            ...
            if (!isWidthExtended ||
                ...
                if (unit) this->units[this->unitCount++] = unit;
            }
...
CUnit* UnitFinder::getNearest(int x,
                              int y,
                              int boundsLeft,
                              int boundsTop,
                              int boundsRight,
...
CUnit* UnitFinder::getNearestTarget(int left,
                                    int top,
                                    int right,
                                    int bottom,
                                    CUnit* sourceUnit,
    ...
    return getNearest(sourceUnit->getX(),
                      sourceUnit->getY(),
                      left,
                      top,
                      right,
...
CUnit* UnitFinder::getNearestTarget(CUnit* sourceUnit,
                                    UnitFinderCallbackMatchInterface& match) {
    return getNearestTarget(0,
                            0,
                            mapTileSize->width * 32,
                            mapTileSize->height * 32,
                            sourceUnit,
                            match);
}
...
void UnitFinder::forEach(
    scbw::UnitFinderCallbackProcInterface& callback) const {
...
CUnit* UnitFinder::getFirst(UnitFinderCallbackMatchInterface& callback) const {
    for (int i = 0; i < this->getUnitCount(); ++i)
        if (callback.match(this->getUnit(i))) return this->getUnit(i);

    return NULL;
}

CUnit* UnitFinder::getBest(
    scbw::UnitFinderCallbackScoreInterface& callback) const {
    int bestScore = -1;
    CUnit* bestUnit = NULL;
