...
struct CImage {
    ...
    /// Removes the image from the linked list of valid images. This is
    /// different from a class destructor, since CImage objects are stored in a
    ...
    /*0x24*/ Box16 grpSize;  // Bounds for GRP frame, only different from normal
                             // when part of graphic is out of bounds.
    /*0x2C*/ GrpHead* grpOffset;
    /*0x30*/ void* coloringData;  //?
    /*0x34*/ void*
        renderFunction;  // renderFunction(this->screenPosition.x,
                         // this->screenPosition.y, this->getCurrentFrame(),
                         // &rctDraw, (int)this->coloringData)
    /*0x38*/ void* updateFunction;
    /*0x3C*/ CSprite* parentSprite;
};

C_ASSERT(sizeof(CImage) == 64); /*0x40*/
// static_assert(sizeof(CImage) == 64, "The size of the CImage structure is
// invalid");
