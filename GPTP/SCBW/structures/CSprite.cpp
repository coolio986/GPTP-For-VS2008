...
void CSprite::free() {
    ...
    else if (sprite_row_position >= mapTileSize->height)
    ...
    if (this->link.prev != NULL) (this->link.prev)->link.next = this->link.next;

    if (this->link.next != NULL) (this->link.next)->link.prev = this->link.prev;
...
void CSprite::setPosition(u16 x, u16 y) {
    ...
    const int oldTileY =
        CLAMP(this->position.y / 32, 0, mapTileSize->height - 1);
    ...
    if (oldTileY != newTileY) {
        const CListExtern<CSprite, &CSprite::link> spritesOnOldTileRow(
            spritesOnTileRow->heads[oldTileY],
...
CImage *CSprite::createOverlay(u32 imageId, s8 x, s8 y, u32 direction) {
    ...
    if (overlay) {
        if (this->images.head)
            this->images.insertBefore<&CImage::link>(overlay,
                                                     this->mainGraphic);
        else {
