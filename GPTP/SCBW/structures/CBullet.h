...
// See Unit.h and compare this entire structure with the first part of the unit
// structure I'm not sure how much of it is the same so I didn't bother updating
// this.
struct CBullet {
    ...
    /*0x10*/ Target moveTarget;  // address? animation related? changes a during
                                 // the same bullet
    /*0x18*/ Point16 targetPosition;
    /*0x1C*/ Point16 targetPosition2;  // usually same as targetPosition
    /*0x20*/ u8      movementFlags;  // usually holds 0x09 for probes, 0x0b for
                                     // dragoons, 0x11 for mutas
    /*0x21*/ u8 direction1;  // not sure how this and direction2 differ from
                             // currentDirection and velocityDirection
    ...
    /*0x3C*/ s32 unknown_0x3C;  // increasing counter or timer, often the same
                                // value as unknown_0x38
    /*0x40*/ Point32 current_speed;
    /*0x48*/ u16     flingyAcceleration;
    /*0x4A*/ u8      currentDirection;
    /*0x4B*/ u8
                velocityDirection;  // seems to always be the same as currentDirection
    ...
    /*0x50*/ u16 someUnitType;     // usually holds UnitType::None (Unknown unit
                                   // type stuff?)
    ...
    /*0x62*/ u8
                hitFlags;  // 0x01 = miss target; 0x02 = no damage (hallucination)
    /*0x63*/ u8 remainingBounces;  // Bounce count for mutalisk weapon (official
                                   // name: ubChain)
    /*0x64*/ CUnit *sourceUnit;
    /*0x68*/ CUnit *nextBounceUnit;   // Next unit for bounce, temporary
    /*0x6C*/ u32 cyclicMissileIndex;  // some sort of cyclic burst counter (used
                                      // by valkyries, goliath air attack)
};

C_ASSERT(sizeof(CBullet) == 112);
// static_assert(sizeof(CBullet) == 112, "The size of the CBullet structure is
// invalid");
