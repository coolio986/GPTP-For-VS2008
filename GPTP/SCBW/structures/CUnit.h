...
struct CUnit : public CUnitLayout {
    ...
    /// Checks if the unit is a clean detector (no Lockdown, Optical Flare,
    /// etc.)
    ...
    /// Returns the amount of HP + shields displayed in game. If the unit does
    /// not have shields enabled, this returns the amount of HP displayed.
    u32 getCurrentLifeInGame() const;

    /// Returns the unit's ground weapon ID. If the unit is an unburrowed
    /// Lurker, returns WeaponId::None instead.
    ...
    /// Returns the maximum range of a weapon in pixels. The weapon is assumed
    /// to be attached to this unit for calculating upgrade effects. This is
    /// affected by the Weapon Range hook module.
    u32 getMaxWeaponRange(u8 weaponId) const;

    /// Retrieves the in-game name of this unit. In UMS maps, this returns
    /// custom names (if they exist).
    ...
    /// Returns the unit's seek range (AKA target acquisition range) in
    /// matrices. This is affected by the Weapon Range hook module.
    ...
    /// Checks if the unit is a spellcaster (has energy) and not a
    /// hallucination.
    ...
    /// Sets the unit's HP, and also updates burning / bleeding graphics and
    /// unit strength data (used by the AI). HP is guaranteed not to overflow.
    ...
    void damageWith(
        ...
        u8 damageDivisor =
            1  ///< Damage divisor (for splash damage / glave wurm calculations)
    );
    ...
    //  Issue the @p order to the unit and specify if previous orders should be
    //  stopped
    ...
    /// Returns the overlay image of this unit's sprite (or its subunit's
    /// sprite) that matches the @p imageId. If the image cannot be found,
    /// returns NULL.
    ...
    /// Checks if the @p unit can build / train / morph into @p unitId. This
    /// checks:
    /// * If the unit is actually owned by the commanding @p playerId
    /// * Whether the unit is stunned / disabled.
    ///
    /// If the unit can build / train / morph, returns 1. If the tech needs to
    /// be researched, returns -1. If the unit cannot use the tech at all,
    /// returns 0.
    ...
    /// researched, returns -1. If the unit cannot use the tech at all, returns
    /// 0.
    ...
    /// Makes the unit use the specified weapon to attack its current target
    /// unit stored in the CUnit::orderTarget.unit member. This does not affect
    /// the unit's weapon cooldown. The spawned weapon sprite obeys the weapon
    ...
    /// Retrieves the unit pointer by @p index from the unit table (first unit
    /// is indexed at 1). If invalid, returns NULL instead.
    ...
    /// Checks whether this unit can be seen by @playerId (i.e. not covered by
    /// the fog-of-war and is detectable).
...
// static_assert(sizeof(CUnit) == sizeof(CUnitLayout), "The size of the CUnit
