...
bool CUnit::hasWeapon() const {
    ...
    if (this->id == UnitId::reaver || this->id == UnitId::warbringer) {
        if (this->carrier.inHangarCount > 0) return true;
    }
...
u32 CUnit::getMovableState() {
    if (this->moveTarget.pt != this->sprite->position)
        return 0;
    else if (!(this->status & UnitStatus::Unmovable))
        return 1;
    else
        return 2;
}
...
void CUnit::reduceDefensiveMatrixHp(s32 amount) {
    ...
    if (this->defensiveMatrixHp > amount)
        ...
        while (current_image != NULL) {
            if (current_image->id < ImageId::DefensiveMatrixBack_Small ||
    ...
    if (this->defensiveMatrixTimer != 0 &&
        !(this->status & UnitStatus::Burrowed)) {
        CUnit* unit = this->subunit ? this->subunit : this;
        // getUnitOverlayAdjustment code was hardcoded into function in original
        // code)
        unit->sprite->createTopOverlay(scbw::getUnitOverlayAdjustment(unit) +
                                       ImageId::DefensiveMatrixHit_Small);
    }
}
...
void CUnit::spendUnitEnergy(u32 energy) {
    if (!(*CHEAT_STATE & CheatFlags::TheGathering)) this->energy -= energy;
}
...
void CUnit::removeOverlay(u32 imageId) {
    ...
    if (this->subunit) this->subunit->sprite->removeOverlay(imageId);
}
...
graphics::ColorId CUnit::getFactionColor(CUnit* unit) {
    ...
    if (unit != NULL) {
        if (unit->sprite != NULL)
            // note: using the sprite playerId to preserve recruitable units
            // color
        ...
        if (unit->playerId < PLAYABLE_PLAYER_COUNT) {
            // get the proper playerId in multiplayer game (don't break
            // campaigns/UMS)
        ...
        if (return_value < PLAYER_COUNT)
            ...
            return_value =
                graphics::BLACK;  // will return 0/black if abnormal playerId

    } else
        return_value = graphics::BLACK;  // will return 0/black if null pointer
                                         // is the argument
...
u32 CUnit::getDistanceToTarget(CUnit* target) const {
    ...
    if (this->isSubunit())
        unit = this->subunit;  // Current unit is a turret, so use it's base
                               // instead
...
CUnit* CUnit::getFromIndex(u16 index) {
    if (1 <= index && index <= UNIT_ARRAY_LENGTH) return &unitTable[index - 1];
    return NULL;
}

u16 CUnit::getIndex() const { return this - unitTable + 1; }
...
CUnit* CUnit::getLoadedUnit(int index) const {
    ...
    if (loadedUnit && loadedUnit->sprite &&
        !(loadedUnit->mainOrderId == OrderId::Die &&
          loadedUnit->mainOrderState == 1) &&
...
CUnit* CUnit::getFirstLoadedUnit() const {
    ...
    for (int i = 0; i < units_dat::SpaceProvided[this->id]; ++i) {
        ...
        if (firstLoadedUnit) return firstLoadedUnit;
    }
...
bool CUnit::hasLoadedUnit() const {
    ...
    for (int i = 0; i < 8; ++i)
        if (!getLoadedUnit(i)) return true;
