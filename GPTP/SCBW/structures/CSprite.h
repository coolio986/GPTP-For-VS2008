...
struct CSprite {
    ...
    void playIscriptAnim(IscriptAnimation::Enum animation,
                         bool bReplaceCurrentScript = true);

    /// AKA SpriteDestructor() @ 0x00497B40
    /// Removes the sprite from the linked list of valid images. This is
    /// different from a class destructor, since CSprite objects are stored in a
    /// static array and never truly destroyed.
    void free();
    ...
    CImage* createTopOverlay(u32 imageId,
                             s8 x = 0,
                             s8 y = 0,
                             u32 direction = 0);
    ...
    /// Checks whether this sprite is visible to @p playerId (i.e. not covered
    /// by the fog of war). This does NOT check detectability.
    ...
    /*0x0B*/ u8 selectionIndex;   // 0 <= selectionIndex <= 11. Index in the
                                  // selection area at bottom of screen.
    /*0x0C*/ u8 visibilityFlags;  // Determines whether the sprite is visible
...
// static_assert(sizeof(CSprite) == 36, "The size of the CSprite structure is
// invalid");
