...
void insertBefore(T *&head, T *&tail, T *t, T *target) {
    if (head) {
        if (head == target) head = t;
        (t->*link).prev = (target->*link).prev;
        (t->*link).next = target;
        if ((target->*link).prev) ((target->*link).prev->*link).next = t;
        (target->*link).prev = t;
    } else {
        head = t;
        tail = t;
...
void insertAfter(T *&head, T *&tail, T *t, T *target) {
    if (head) {
        if (tail == target) tail = t;
        (t->*link).prev = target;
        (t->*link).next = (target->*link).next;
        if ((target->*link).next) ((target->*link).next->*link).prev = t;
        (target->*link).next = t;
    } else {
        head = t;
        tail = t;
...
void unlink(T *&head, T *&tail, T *t) {
    if (head == t) head = (t->*link).next;
    if (tail == t) tail = (t->*link).prev;
