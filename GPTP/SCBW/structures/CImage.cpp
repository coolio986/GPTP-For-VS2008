...
void CImage::free() {
    ...
    if (!(screenLayers->game.hasBeenRefreshed))
        scbw::refreshScreen(
            ...
            this->screenPosition.y +
                this->grpSize
                    .bottom);  // should be equivalent to a call to refreshImage
                               // @ 0x004970A0 with EAX = this
...
void CImage::initializeData(CSprite* parent, u16 imageId, s8 x, s8 y) {
    ...
    this->flags = ((images_dat::IsTurnable[imageId] & 1) << 3) |
                  ((images_dat::IsClickable[imageId] & 1) << 5);
    this->frameSet = 0;
    this->direction = 0;
    this->frameIndex = 0;
