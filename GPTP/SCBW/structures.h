...
// Based on
// http://code.google.com/p/bwapi/source/browse/branches/bwapi4/bwapi/BWAPI/Source/BW/CImage.h
...
struct TILE {
    ...
    /*03*/ u8 groundHeight; /**< Ground Height(4 lower bits) - Deprecated? Some
    ...
    /*0A*/ u16 buttomEdge;
    /*0C*/ u16 _1;
    /*0E*/ u16 _2; /**<  Unknown - Edge piece has rows above it. (Recently
                   noticed; not fully understood.) o 1 = Basic edge piece. o 2 =
                   Right edge piece. o 3 = Left edge piece.*/
    /*10*/ u16 _3;
    /*12*/ u16 _4;
...
struct DOODAD {
    /*00*/ u16 index;
    /*02*/ u8 buildability; /**< 8th bit should sign not buildable. */
    /*03*/ u8 groundHeightAndOverlayFlags; /**< Ground Height(4 lower bits) -
                                            * Deprecated? Some values are
                                            * incorrect. Overlay Flags: o 0x0 -
                                            * None o 0x1 - Sprites.dat Reference
                                            * o 0x2 - Units.dat Reference (unit
                                            * sprite) o 0x4 - Overlay is Flipped
                                            */
    /*04*/ u16 overlayID;
    /*06*/ u16 _1;
...
// static_assert(sizeof(GrpFrame) == 8, "The size of the GrpFrame structure is
// invalid");
...
// static_assert(sizeof(GrpHead) == 14, "The size of the GrpHead structure is
// invalid");
...
struct LO_Header {
    ...
    Point8 getOffset(int frameNum, int overlayNum) const {
        return (
            (Point8 *)(this->frameOffsets[frameNum] + (u32)this))[overlayNum];
    }
};

C_ASSERT(sizeof(LO_Header) == 12);
// static_assert(sizeof(LO_Header) == 12, "The size of the LO_Header structure
// is invalid");
...
// static_assert(sizeof(ColorShiftData) == 20, "The size of the ColorShiftData
...
struct ActiveTile {
    ...
    /*03*/ u8 groundHeight : 3;  // ground height; 1 for low ground, 2 for
                                 // middle ground and 4 for high ground
    u8 currentlyOccupied : 1;    // unbuildable but can be made buildable
    u8 creepReceeding : 1;       // Set when the nearby structure supporting the
                                 // creep is destroyed
    u8 cliffEdge : 1;            // Set if the tile is a cliff edge
    u8 temporaryCreep : 1;       // Set when the creep occupying the area was
                            // created. Not set if creep tiles were preplaced.
                            // Used in drawing routine.
    u8 unknown4 : 1;  // Unused?
};

C_ASSERT(sizeof(ActiveTile) == 4);
// static_assert(sizeof(ActiveTile) == 4, "The size of the ActiveTile structure
// is invalid");
...
// static_assert(sizeof(GroupFlag) == 1, "The size of the GroupFlag structure is
// invalid");
...
// static_assert(sizeof(TargetFlag) == 2, "The size of the TargetFlag structure
// is invalid");
...
// static_assert(sizeof(UnitFinderData) == 8, "The size of the UnitFinderData
// structure is invalid");

// BinDlg is defined in
// https://code.google.com/p/vgce/source/browse/trunk/docs/Blizzard/Starcraft/BIN%20FORMAT.txt
// as Dialog Structure
...
struct BinDlg {
    /*0x00*/ BinDlg *next;
    /*0x04*/ Bounds
        bounds;  // 0x4 left,0x6 top,0x8 right,0xA bottom,0xC width,0xE height
    /*0x10*/ u8 *buffer;
    /*0x14*/ char *pszText;
    ...
    union {
        ...
        struct {
            ...
            /*0x3A*/ u32
                *responseAreaBitArray;  // set with a memory allocation
                                        // function, bit array is a guess
        ...
        struct {
            ...
            /*0x3E*/ u32 unk3e;  // unverified filler
        } dialogOn0x36;  // for when 0x36 is clearly BinDlg* but the rest is
                         // unknown
        struct {
            /*0x36*/ Bool32
                unkBool32_36;    // based on the value only being compared to 0
            /*0x3A*/ u32 unk3a;  // unverified filler
            /*0x3E*/ u32 unk3e;  // unverified filler
        } checkBox0x36;          // not completely sure
        struct {
            /*0x36*/ u8
                unkFlags36;         //"flag" because of possible unkFlags36 |= 1
            /*0x37*/ UNK unk37[3];  // unverified filler
            /*0x3A*/ u8 *
                *unk3aArrayCharPtr;  // seen in a different context so may be
                                     // wrong and belong to another category
...
// static_assert(sizeof(BinDlg) == 86, "The size of the BinDlg structure is
// invalid");
...
// static_assert(sizeof(SMK_Structure) == 30, "The size of the SMK_Structure
// structure is invalid");
...
// static_assert(sizeof(GuiOverlay) == 44, "The size of the GuiOverlay structure
// is invalid");
...
struct AI_Main {
    ...
    /*218*/ struct {
        ...
        u16 hasStrongestGndArmy : 1;
            /*0200*/  // Set if the AI's ground army is stronger than the total
                      // strength of the strongest enemy force
...
// static_assert(sizeof(AI_Main) == 1256, "The size of the AI_Main structure is
// invalid");
...
// static_assert(sizeof(AiCaptain) == 52, "The size of the AiCaptain structure
// is invalid");
   ...
   public:
    ...
    const char *getString(u16 index) const {
        ...
        if (index == 0)
            ...
            return (const char *)0x00501B7D;  // StringEmpty in scbwdata.h, SC
