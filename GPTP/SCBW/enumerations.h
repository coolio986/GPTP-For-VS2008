...
enum Enum : u32 {
    ...
    NeutralAccessories = 0x00000800,  // AKA "Single entity" (prevents
                                      // multi-select, set on all pickup items)
    ...
    NPCOrAccessories = 0x00800000,  // AKA "Pickup item" (data disc, crystals,
                                    // mineral chunks, gas tanks, etc.)
    MorphFromOtherUnit = 0x01000000,
    MediumOverlay =
        0x02000000,  // Used to determine overlay for various spells and effects
    LargeOverlay =
        0x04000000,  // Used to determine overlay for various spells and effects
    AutoAttackAndMove = 0x08000000,
    Attack = 0x10000000, /**< Can attack */
    Invincible = 0x20000000,
    Mechanical = 0x40000000,
    ProducesUnits = 0x80000000, /**< It can produce units directly (making
...
enum Enum : u32 {
    ...
    Cloaked = 0x00000200,
    DoodadStatesThing = 0x00000400,  ///< Unknown. ++protoss unpowered buildings
                                     ///< have this flag set
    ...
    CanTurnAroundToAttack =
        0x00010000,  // canAttack? /**< @todo Unknown */ named IsAUnit in BWAPI
    IsBuilding = 0x00020000,
    IgnoreTileCollision = 0x00040000,
    Unmovable = 0x00080000,
    IsNormal =
        0x00100000, /**< 1 for "normal" units, 0 for hallucinated units */
    NoCollide = 0x00200000,  // ++if set, other units wont collide with the unit
                             // (like burrowed units)
    UNKNOWN5 = 0x00400000,
    IsGathering = 0x00800000,  // ++if set, the unit wont collide with other
                               // units (like workers gathering)
    UNKNOWN6 = 0x01000000,
    UNKNOWN7 = 0x02000000,  // Turret related
    Invincible = 0x04000000,
    HoldingPosition =
        0x08000000,  // Set if the unit is currently holding position
    SpeedUpgrade = 0x10000000,
    CooldownUpgrade = 0x20000000,
    IsHallucination =
        0x40000000, /**< 1 for hallucinated units, 0 for "normal" units */
    IsSelfDestructing = 0x80000000  // Set for when the unit is self-destructing
...
enum Enum { Invisible = -1, Disabled = 0, Enabled = 1 };
}
