...
void disconnectAddOn_Sub464930(CUnit* unit) {
    ...
    __asm {
			PUSHAD
			MOV EAX, addon_unit
			CALL Func_PlayBuildingLiftoffSound  // that thing was too complicated
                                              // to be extracted T_T
			POPAD
    }
...
void removeUnitRefsFromTarget(CUnit* unit, CUnit* target) {
    if (unit == target->currentBuildUnit) target->currentBuildUnit = NULL;

    if (unit == target->orderTarget.unit) target->orderTarget.unit = NULL;

    if (unit == target->moveTarget.unit) target->moveTarget.unit = NULL;

    if (unit == target->autoTargetUnit) target->autoTargetUnit = NULL;

    if (unit == target->connectedUnit) target->connectedUnit = NULL;

    if (unit == target->irradiatedBy) target->irradiatedBy = NULL;
    ...
    } else if ((target->status & UnitStatus::GroundedBuilding) &&
               (target->building.addon == unit))
        target->building.addon = NULL;

    if ((units_dat::GroupFlags[target->id]
             .isFactory) &&  // equivalent condition to a call to
                             // "StarCraft.exe+7B2E0 unitIsFactoryUnit" internal
                             // function
    ...
    if (target->subunit != NULL) {
        if (units_dat::BaseProperty[target->subunit->id] &
            UnitProperty::Subunit)
            removeUnitRefsFromTarget(unit,
                                     target);  // the snake is biting his tail?
    }
    ...
    if (target->orderQueueHead != NULL) {
        ...
        while (current_order != NULL) {
            // instruction "mov [ebp-08], ecx (==current_order->target)" is lost
            // there I hope it wasn't important...at least it's not used by
            // removeOrderFromUnitQueue()
...
void readUnitsArray_Sub49EFA0(CUnit* unit,
    ...
    if (playerId_param != unit->playerId) {
        ...
        else bIsUnitReallyCompleted = true;
        ...
        __asm {
				...
				PUSH unknown_param  // there's really a case where a 32 bits
                                    // register is that value, so it's ok like
                                    // this
				MOV ECX, 0x00000001
				CALL Func_IncrementUnitScoresEx
        ...
        if (!bIsUnitReallyCompleted) unit->status &=
            ~UnitStatus::Completed;  // re-disabling the flag
        ...
        if (!units_dat::GroupFlags[unit->id].isNeutral &&
        ...
        if (unit->subunit != NULL &&
            ...
            readUnitsArray_Sub49EFA0(
                unit->subunit, playerId_param, unknown_param);
        ...
        if (units_dat::BaseProperty[unit->id] & UnitProperty::Worker &&
            ...
            if (unit->worker.powerup->playerId == playerId_param ||
...
}  // void readUnitsArray_Sub49EFA0(CUnit* unit, u8 playerId_param, u32
   // unknown_param)
...
void ordersRescuePassive(CUnit* unit) {
    ...
    if (unit->mainOrderTimer == 0) {
        ...
        unit_finder.search(
            (unit->sprite->position.x >= 0x40) ? unit->sprite->position.x - 0x40
                                               : 0,
            (unit->sprite->position.y >= 0x40) ? unit->sprite->position.y - 0x40
                                               : 0,
        ...
        if (unit_finder_items_count != 0) {
            ...
            if (playerTable[unit_finder.getUnit(unit_finder_index)->playerId]
                    .type != PlayerType::Human) {
                unit_finder_index++;

                while ((unit_finder_index != unit_finder_items_count) &&
                       (playerTable[unit_finder.getUnit(unit_finder_index)
                                        ->playerId]
                            .type != PlayerType::Human))
                    unit_finder_index++;
            ...
            if (!jump_to_A2105) {
                ...
                if ((unit->status & UnitStatus::Completed) ||
                    ...
                    jump_to_A2052 = false;
                else if (unit->id != UnitId::ZergHatchery)
                    jump_to_A2052 = true;
                else {
                    if (unit->isRemorphingBuilding())
                        jump_to_A2052 = false;
                ...
                if (!jump_to_A2052) {
                    if (!(unit->status & UnitStatus::GroundedBuilding) ||
                        !(units_dat::BaseProperty[unit->id] &
                          UnitStatus::CanNotReceiveOrders))
                        jump_to_A2052 = true;
                    else if (firstPlayerUnit->unit[unit->playerId] == NULL)
                        jump_to_A20B2 = true;
                }

                if (!jump_to_A2052 && !jump_to_A20B2) {
                    CUnit* localFirstPlayerUnit =
                        firstPlayerUnit->unit[unit->playerId];
                    ...
                    do {
                        ...
                        if (localFirstPlayerUnit->sprite != NULL &&
                            ...
                            // readUnitsArray_Sub49EFA0(firstPlayerUnit->unit[unit->playerId],
                            // unit_found->playerId);
                            __asm {
                            ...
                            if (localFirstPlayerUnit->status &
                                UnitStatus::InBuilding)
                                localFirstPlayerUnit->order(
                                    OrderId::BunkerGuard,
                                    0,
                                    0,
                                    NULL,
                                ...
                                localFirstPlayerUnit->order(
                                    units_dat::HumanIdleOrder
                                        [localFirstPlayerUnit->id],
                                    0,
                                    0,
                                    NULL,
                                    UnitId::None,
                        ...
                        localFirstPlayerUnit =
                            localFirstPlayerUnit->player_link.next;

                        if (localFirstPlayerUnit == NULL) jump_to_A20B2 = true;
                ...
                if (!jump_to_A20B2) {
                    ...
                    if (unit->status & UnitStatus::InBuilding)
                        unit->order(OrderId::BunkerGuard,
                                    0,
                                    0,
                                    NULL,
                                    UnitId::None,
                        ...
                        unit->order(units_dat::HumanIdleOrder[unit->id],
                                    0,
                                    0,
                                    NULL,
                                    UnitId::None,
                ...
                if (!scbw::isInReplay() && unit->playerId != *LOCAL_NATION_ID) {
                    ...
                    scbw::playSound(playerTable[*LOCAL_NATION_ID].race +
                                        SoundId::Misc_ZRescue_wav,
                                    unit);
                }
...
void changeUnitButtonSet(CUnit* unit, u16 buttonSetId) {
    if ((unit->status &
        ...
        else if (buttonSetId == UnitId::None)
            unit->currentButtonSet = buttonSetId;
...
void hideAndDisableUnit(CUnit* unit) {
    if (!(unit->sprite->flags & CSprite_Flags::Hidden)) {
        ...
        if (unit->sprite->flags & CSprite_Flags::Hidden) {
            ...
            __asm {
                ...
                MOV EDI, unit MOV ECX,
                    [EDI + 0x0C]  // ECX = unit->sprite
                    MOV AL,
                    [EDI + 0x00000105]  // AL = unit->pathingFlags
                    MOV[EDI + 0x00000097],
                    0x00  // unit->movementState = 0x00 (was BL and BL == 0x00 )
                    CMP BYTE PTR[ECX + 0x0D],
                    0x0C  // cmp(unit->sprite->elevationLevel,12)
                    SETB DL XOR DL,
            ...
            if (unit->subunit != NULL) {
                ...
                if (units_dat::BaseProperty[currentSubUnit->id] &
                    UnitProperty::Subunit) {
                    __asm {
    ...
    /*
       ...
       //jump to last "pop edi"
                    ...
                    XOR EBX,EBX						//EBX =
       NULL CMP ESI,EBX						//if (ESI ==
       NULL) JE STARCRAFT.EXE+E636F			//jump MOV EDI,EDI
       //unit = unit
       ...
       sub_49E590 nicknamed RemoveUnitRefsFromTarget()
                    ...
                    MOV ESI,[ESI+04]				//ESI =
       firstVisibleUnit->link.next CMP ESI,EBX
       //if(ESI != NULL) JNE STARCRAFT.EXE+E6360			//loop
       ...
       NULL MOV WORD PTR [ESI+24],FFFF		//[ESI+24] = FFFF CALL
       STARCRAFT.EXE+18700		//00418700 HideDialog
       ...
       resetOrdersUnitsDAT
                    ...
                    MOV ESI,EDI						//ESI =
       unit CALL STARCRAFT.EXE+6A300		//0046A300 sortUnit

                    TEST BYTE PTR [EDI+000000DC],02	//if(unit->status &
       UnitStatus::GroundedBuilding) JE STARCRAFT.EXE+E63CE
       //jump

                    MOV EAX,[EDI+0C]				//EAX =
       unit->sprite XOR EDX,EDX						//EDX =
       ...
       UnitStatus::InAir) JE STARCRAFT.EXE+E63DE			//jmp

                    MOV EAX,EDI						//EAX =
       unit CALL STARCRAFT.EXE+533D0		//004533D0 removeRepulseTile

                    //StarCraft.exe+E63DE
                    MOV ESI,EDI						//ESI =
       unit CALL STARCRAFT.EXE+E42A0		//004E42A0  toggleUnitPath

                    MOV ECX,[EDI+0C]				//ECX =
       unit->sprite MOV AL,[EDI+00000105]			//AL =
       ...
       *firstHiddenUnit JNE STARCRAFT.EXE+E6426			//if not equal
       ...
       //if firstHiddenUnit->link.next == NULL JE STARCRAFT.EXE+E6437
       //jump
       ...
       //always jump
                    ...
                    //StarCraft.exe+E643C
                    MOV [STARCRAFT.EXE+228428],EDI	//mov unit into some
       const MOV [STARCRAFT.EXE+2283EC],EDI  //mov unit into firstHiddenUnit
...
void showAndEnableUnit(CUnit* unit) {
    if (unit->sprite->flags & CSprite_Flags::Hidden) {
        ...
        if (unit->subunit != NULL) {
            if (units_dat::BaseProperty[unit->subunit->id] &
                UnitProperty::Subunit)
        ...
        __asm {
            PUSHAD
            MOV EDI, unit MOV ESI,
                unit CALL Func_Sub_4E5F30_GetRightClickActionOrder CALL
                    Func_UpdateUnitOrderFinder POPAD
        }
...
void setNextWaypoint_Sub4EB290(CUnit* unit) {
    if (unit->pathingFlags & 2) unit->pathingFlags |= 4;
    ...
    // Note: it's big because extracting "modified unit->moveTarget" would take
    // too much time, if it's even possible.
