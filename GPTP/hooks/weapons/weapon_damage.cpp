...
void weaponDamageHook(s32 damage,
    ...
    if (target->hitPoints != 0 && !(target->status & UnitStatus::Invincible)) {
        ...
        if (scbw::isCheatEnabled(
                CheatFlags::PowerOverwhelming) &&  // If Power Overwhelming is
                                                   // enabled
            playerTable[attackingPlayerId].type !=
                PlayerType::Human)  // and the attacker is not a human player
            damage = 0;

        if (target->status & UnitStatus::IsHallucination) damage *= 2;

        damage = damage / dmgDivisor + (target->acidSporeCount * 256);

        if (damage < 128) damage = 128;
        ...
        if (units_dat::ShieldsEnabled[target->id] && target->shields >= 256) {
            if (damageType != DamageType::IgnoreArmor) {
                s32 plasmaShieldUpg =
                    scbw::getUpgradeLevel(target->playerId,
                                          UpgradeId::ProtossPlasmaShields) *
                    256;

                if (damage > plasmaShieldUpg)  // Weird logic, Blizzard dev must
                                               // have been sleepy
        ...
        // Apply damage type/unit size factor (not checked by UndeadStar, I will
        // hope it work)
        damage =
            (damage * damageFactor[damageType]
                          .unitSizeFactor[units_dat::SizeType[target->id]]) /
            256;

        if (shieldReduceAmount == 0 && damage < 128) damage = 128;
        ...
        target->damageHp(
            ...
            attacker,
            attackingPlayerId,
            weaponId != WeaponId::Irradiate);  // Prevent Science Vessels from
                                               // being continuously revealed to
...
void createShieldOverlay(CUnit* unit, u32 attackDirection) {
    ...
    Point8 offset =
        shield_lo->getOffset(unit->sprite->mainGraphic->direction, frameAngle);

    unit->sprite->createOverlay(
        ImageId::ShieldOverlay, offset.x, offset.y, frameAngle);
}
