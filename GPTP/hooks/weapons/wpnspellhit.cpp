...
void IterateUnitsAtLocationTargetProc_Plague(CUnit* attacker,
                                             Box16* coords);  // 0x004E8280
void IterateUnitsAtLocationTargetProc_Maelstrom(CUnit* attacker,
                                                Box16* coords);  // 0x004E8280
void IterateUnitsAtLocationTargetProc_StasisField(CUnit* attacker,
                                                  Box16* coords);  // 0x004E8280
...
// no bullet because this function can be called in another context without
// bullet
void IrradiateHit(CUnit* attacker, CUnit* target, u8 attackingPlayerId) {
    ...
    if (target->irradiateTimer == 0 &&
        ...
        else if (units_dat::BaseProperty[target->id] &
                 UnitProperty::LargeOverlay)
...
void RestoreHit(CUnit* target, CBullet* bullet) {
    if (target->status & UnitStatus::IsHallucination)
        ...
        if (target->ensnareTimer != 0) {
            ...
            target->removeOverlay(ImageId::EnsnareOverlay_Small,
                                  ImageId::EnsnareOverlay_Large);
            target->ensnareTimer = 0;
        ...
        if (target->plagueTimer != 0) {
            ...
            target->removeOverlay(ImageId::PlagueOverlay_Small,
                                  ImageId::PlagueOverlay_Large);
            target->plagueTimer = 0;
        }

        if (target->irradiateTimer != 0) {
            target->irradiateTimer = 0;
            target->removeOverlay(ImageId::Irradiate_Small,
        ...
        if (target->lockdownTimer != 0) target->removeLockdown();

        if (target->maelstromTimer != 0) target->removeMaelstrom();

        if (target->acidSporeCount != 0) target->removeAcidSpores();
...
void WeaponBulletHit(CBullet* bullet, CUnit* target, u32 damageDivisor) {
    if (bullet->hitFlags &
        2)  // bullet doing no damage (shot from hallucination)
        attackOverlayAndNotify(
            bullet->sourceUnit,
            target,
            bullet->weaponType,
            bullet->direction1);  // no damage, but still ask for retaliation
    else {
        u32 base_damage = GetUnitBulletDamage(bullet, target);

        target->damageWith(base_damage,
                           bullet->weaponType,
                           bullet->sourceUnit,
                           bullet->srcPlayer,
                           bullet->direction1,
    ...
    if (target != NULL) {
        ...
        if (attacker->status & UnitStatus::IsHallucination)
            attackOverlayAndNotify(
                ...
                target,
                weaponId,
                attacker->currentDirection1);  // no damage, but still ask for
                                               // retaliation
        else {
...
void EMPShockwaveHit(CUnit* attacker, int x, int y, CBullet* bullet) {
    static u16* const maxBoxRightValue =
        (u16*)0x00628450;  // should usually be mapTileSize->width * 32
    static u16* const maxBoxBottomValue =
        (u16*)0x006284B4;  // should usually be mapTileSize->height * 32
    ...
    area_of_effect.left =
        (u16)x - weapons_dat::InnerSplashRadius[WeaponId::EMP_Shockwave];
    area_of_effect.right =
        (u16)x + weapons_dat::InnerSplashRadius[WeaponId::EMP_Shockwave];
    area_of_effect.top =
        (u16)y - weapons_dat::InnerSplashRadius[WeaponId::EMP_Shockwave];
    area_of_effect.bottom =
        (u16)y + weapons_dat::InnerSplashRadius[WeaponId::EMP_Shockwave];

    // check and fix effect if beyond width of map
    if (area_of_effect.left < 0)
        area_of_effect.left = 0;
    else if (area_of_effect.right > *maxBoxRightValue)
    ...
    else if (area_of_effect.bottom > *maxBoxBottomValue)
        ...
        if (current_unit != attacker &&  // EMP doesn't affect the attacker
            ...
            if (current_unit->status & UnitStatus::IsHallucination)
                ...
                if (current_unit->sprite->flags & CSprite_Flags::Selected &&
                    current_unit->sprite !=
                        NULL /*I'm following original code, though this order of
                                tests doesn't make sense*/
                )
                    setAllImageGroupFlagsPal11(
                        current_unit->sprite);  // redraw the unit
            }
        }
    ...
    // reload the previous temporary unit list from before the call to
    // getAllUnitsInBounds
    *tempUnitsListArraysCountsListLastIndex =
        *tempUnitsListArraysCountsListLastIndex - 1;
    *tempUnitsListCurrentArrayCount =
        tempUnitsListArraysCountsList[*tempUnitsListArraysCountsListLastIndex];
...
}  // void CorrosiveAcidHit(u32 attackingPlayerId, int x, int y, CBullet*
   // bullet)
...
void ConsumeHit(CUnit* attacker, CUnit* target) {
    if (target != NULL && !(target->status & UnitStatus::Invincible) &&
        getSpellStatString(target, TechId::Consume, attacker->playerId) ==
            0  // returning a value != 0 means there was an error
    ) {
        ...
        if (!(target->status & UnitStatus::IsHallucination)) {
            ...
            if ((energyMax - attacker->energy) <
                12800)  // 12800 is 50 energy ingame
...
void DarkSwarmHit(int x, int y, u32 attackingPlayerId, CBullet* bullet) {
    ...
    dark_swarm_unit =
        CreateUnit(UnitId::Spell_DarkSwarm, coords.x, coords.y, 11);
    ...
    if (dark_swarm_unit == NULL)
        ...
        // update various stuff (set hp, set shield...) not finished on
        // CreateUnit
...
void BroodlingHit(CUnit* attacker, CUnit* target, CBullet* bullet) {
    if (!(target->status & UnitStatus::IsHallucination)) {
        ...
        do {
            ...
            if (function_0042D810(
                ...
                jump_to_end_of_loop = !scbw::checkUnitCollisionPos(
                    attacker, &inPos, &outPos, &moveArea, false, 0);
            ...
            if (!jump_to_end_of_loop) {  // F49E2
                ...
                created_broodling = CreateUnit(
                    UnitId::ZergBroodling, pos.x, pos.y, attacker->playerId);
                ...
                if (created_broodling == NULL) {
                    ...
                    // update various stuff (set hp, set shield...) not finished
                    // on CreateUnit
                    ...
                    if (created_broodling->status & UnitStatus::IsHallucination)
                        timerBeforeDisappearance =
                            1350;  // probably old unremoved code
                    else if (created_broodling->id == UnitId::ZergBroodling)
                        timerBeforeDisappearance = 1800;  // 0x708
                    else
                        timerBeforeDisappearance = 0;

                    if (created_broodling->removeTimer == 0 ||
                        created_broodling->removeTimer >
                            timerBeforeDisappearance)
                        created_broodling->removeTimer =
                            timerBeforeDisappearance;

                    // AI stuff (may establish relationship between queen and
                    // broodling and/or be about supplies management, or
                    // something else?
    ...
    if (attacker->killCount < 255) attacker->killCount++;
    ...
    if (attacker->id == UnitId::ProtossInterceptor &&
...
void MindControlHit(CUnit* attacker, CUnit* target) {
    if (target->status & UnitStatus::IsHallucination)
        ...
        if (target->id == UnitId::ProtossDarkArchon) target->energy = 0;
    }
...
void DisruptionWebHit(u32 attackingPlayerId, int x, int y, CBullet* bullet) {
    ...
    disruption_web_unit =
        CreateUnit(UnitId::Spell_DisruptionWeb, coords.x, coords.y, 11);
    ...
    if (disruption_web_unit == NULL)
        ...
        if (disruption_web_unit->removeTimer == 0 ||
            disruption_web_unit->removeTimer > 360)
            disruption_web_unit->removeTimer = 360;  // 0x0168
    }

}  // void DisruptionWebHit(u32 attackingPlayerId, int x, int y, CBullet*
   // bullet)
...
void      fixTargetLocation(Point16* coords, u32 unitId){
    ...
    __asm {PUSHAD MOV EAX,
           unitId MOV EDX,
           coords CALL Func_fixTargetLocation POPAD}
...
void incrementUnitKillCount(CUnit* unit) {
    ...
    if (temp_unit->killCount < 255) temp_unit->killCount++;

    while (temp_unit->id == UnitId::ProtossInterceptor &&
...
void      attackOverlayAndNotify(CUnit* attacker,
    ...
    __asm {PUSHAD MOV ESI,
           ...
           target PUSH weaponType CALL Func_attackOverlayAndNotify POPAD}
...
void      incrementUnitDeathScores(CUnit* unit, u8 playerId){
    ...
    __asm {PUSHAD MOV EDI,
           unit MOVZX EDX,
           playerId CALL Func_IncrementUnitDeathScores POPAD}
