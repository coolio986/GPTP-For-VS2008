...
// This hook affects the following iscript opcodes: attackwith, attack,
// castspell This also affects CUnit::fireWeapon().
void fireWeaponHook(CUnit* unit, u8 weaponId) {
    ...
    if (weapons_dat::Behavior[weaponId] == WeaponBehavior::AppearOnTargetUnit) {
        if (unit->orderTarget.unit == NULL) return;

        getWeaponHitPos(unit, &x, &y);

    } else if (weapons_dat::Behavior[weaponId] ==
        ...
        x = unit->getX() +
            scbw::getPolarX(forwardOffset, unit->currentDirection1);
        y = unit->getY() +
            scbw::getPolarY(forwardOffset, unit->currentDirection1) -
            weapons_dat::VerticalOffset[weaponId];
    }

    if (weapons_dat::FlingyId[weaponId] != 0)
        createBullet(
            weaponId, unit, x, y, unit->playerId, unit->currentDirection1);
}
