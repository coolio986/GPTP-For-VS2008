...
void CMDACT_Stimpack() {
    ...
    for (int i = 0; i < SELECTION_ARRAY_LENGTH && !bFoundUnitAbleToStimpack;
         i++) {
    ...
    if (bFoundUnitAbleToStimpack) {
        static u8 opcode[] = {
            0x36};  // packet opcode for stimpack is 0x36, no others args
        QueueGameCommand(&opcode[0], 1);
    } else {
        char* message =
            (char*)statTxtTbl->getString(874);  // not enough life remaining
...
void useStimPacksAIHook(CUnit* unit) {
    if (unit->hitPoints > MIN_HITPOINTS_FOR_STIMPACKS) {
        scbw::playSound(RandBetween(SoundId::Terran_MARINE_TMaSti00_WAV,
                                    SoundId::Terran_MARINE_TMaSti01_WAV,
                                    31),
                        unit);
...
void CMDRECV_StimPack() {
    ...
    *selectionIndexStart         = 0;
    ...
    while (activePlayerCurrentSelection != NULL) {
        if (activePlayerCurrentSelection->canUseTech(TechId::Stimpacks,
                                                     *ACTIVE_NATION_ID) == 1 &&
            activePlayerCurrentSelection->hitPoints >
                MIN_HITPOINTS_FOR_STIMPACKS) {
            int random_value;
            ...
            scbw::playSound(SoundId::Terran_MARINE_TMaSti00_WAV + random_value,
                            activePlayerCurrentSelection);

            activePlayerCurrentSelection->damageHp(
                MIN_HITPOINTS_FOR_STIMPACKS, NULL, -1, true);
