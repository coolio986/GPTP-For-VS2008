...
void rechargeShieldsProc(CUnit* target, CUnit* battery) {
    ...
    if (target->shields > maxShields) target->shields = maxShields;
...
Bool32 unitCanRechargeShieldsHook(CUnit* target, CUnit* battery) {
    ...
    if (target->playerId != battery->playerId ||  // Is not owned by the player
        ...
        !(target->status &
          UnitStatus::IsBuilding) ||  // Is building (like archon build self)?
        target->status & UnitStatus::GroundedBuilding ||  // Is a building
        target->getRace() != RaceId::Protoss ||           // Check target race
        target->shields >= (units_dat::MaxShieldPoints[target->id] *
                            256) ||  // Already has max shields
        !(battery->status & UnitStatus::Completed) ||
        battery->energy == 0 || battery->isFrozen() || isHangarUnit(target))
        result = 0;
    else if (target->pAI == NULL)
        result = 1;
    else {  // Separate check for AI-controlled units
        if (target->mainOrderId == OrderId::RechargeShields1 ||
            target->mainOrderId == OrderId::Pickup4)
            result = 0;
        else if (target->orderQueueHead != NULL &&
                 target->orderQueueHead->orderId == OrderId::RechargeShields1)
            result = 0;
        else
            result = 1;
    }
...
void orderRechargeShieldsHook(CUnit* unit) {
    ...
    if (battery == NULL || battery->energy == 0 || battery->isFrozen()) {
        ...
        if (unit->mainOrderState == 0) {
            if (orderToMoveToTarget(unit, battery)) unit->mainOrderState = 1;
            ...
            if (movableState == 0) {  // not reached destination
                if (!unit->isTargetWithinMinRange(battery, BATTERY_RANGE))
                    bStopThere = true;
            } else if (movableState == 1) {  // unit reached destination
        ...
        if (!bStopThere && unit->mainOrderState == 2) {
            ...
            unit->sprite->createOverlay(
                ImageId::RechargeShields_Small + overlaysCounter, 0, 0, 0);
        ...
        if (!bStopThere && unit->mainOrderState == 3) {
            ...
            // do 1 recharge action? (increase unit shield, decrease battery
            // energy)
...
void insertFirstOrder(CUnit* unit, u8 orderId){
    ...
    __asm {PUSHAD MOV AL,
           orderId MOV ECX,
           unit CALL Helper_InsertFirstOrder POPAD}
