...
class irradiateProc : public scbw::UnitFinderCallbackProcInterface {
   ...
   public:
    ...
    void proc(CUnit* unit) {
        ...
        if (unit->id == UnitId::larva || unit->id == UnitId::egg ||
        ...
        if (irradiatedUnit->status & UnitStatus::InTransport ||
            ...
            const s32 damage = weapons_dat::DamageAmount[WeaponId::Irradiate] *
                               256 / weapons_dat::Cooldown[WeaponId::Irradiate];
    ...
    // If inside a transport, damage all units loaded within
        ...
        if (transport != NULL) {
            for (int i = 0; i < units_dat::SpaceProvided[transport->id]; ++i) {
                ...
                if (loadedUnit) irradiation.proc(loadedUnit);
            }
        }
...
// Note: This function is called every 8 ticks (when unit->cycleCounter reaches
// 8 == 0)
...
void updateStatusEffects(CUnit* unit) {
    if (unit->stasisTimer != 0) {
        ...
        if (unit->stasisTimer == 0) unit->removeStasisField();
    }

    if (unit->stimTimer != 0) {
        unit->stimTimer--;

        if (unit->stimTimer == 0) unit->updateSpeed();
    }
    ...
    if (unit->ensnareTimer != 0) {
        ...
        if (unit->ensnareTimer == 0) {
            unit->removeOverlay(ImageId::EnsnareOverlay_Small,
    ...
    if (unit->lockdownTimer != 0) {
        ...
        if (unit->lockdownTimer == 0) unit->removeLockdown();
    }

    if (unit->maelstromTimer != 0) {
        unit->maelstromTimer--;

        if (unit->maelstromTimer == 0) unit->removeMaelstrom();
    }
    ...
    if (unit->plagueTimer != 0) {
        ...
        if (!(unit->status & UnitStatus::Invincible)) {
            // s32 damage = (weapons_dat::DamageAmount[WeaponId::Plague] << 8) /
            // 76;
        ...
        if (unit->plagueTimer == 0)
            unit->removeOverlay(ImageId::PlagueOverlay_Small,
                                ImageId::PlagueOverlay_Large);
    }

    if (unit->isUnderStorm != 0) unit->isUnderStorm--;
    ...
    for (int i = 0; i <= 8; ++i) {
        if (unit->acidSporeTime[i] != 0) {
            ...
            if (unit->acidSporeTime[i] == 0) unit->acidSporeCount--;
        }
    }
    if (unit->acidSporeCount != 0) {
        ...
        if (!unit->getOverlay(acidOverlayId)) {
            unit->removeOverlay(ImageId::AcidSpores_1_Overlay_Small,
                                ImageId::AcidSpores_6_9_Overlay_Large);

            if (unit->subunit != NULL) unit = unit->subunit;
