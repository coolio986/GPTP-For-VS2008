...
/*
		int column = 0;
		char textHere[500];
		sprintf_s(textHere, "Vals:");
		for(int i = 0; i < *clientSelectionCount; i++) {
			CUnit *selUnit = clientSelectionGroup->unit[i];
			sprintf_s(textHere, "%s\nmainOrderId(%d): %02x", textHere, i, selUnit->mainOrderId);
			sprintf_s(textHere, "%s\nlastOrderId(%d): %02x", textHere, i, selUnit->_unknown_0x086);
			sprintf_s(textHere, "%s\nuserActionF(%d): %02d", textHere, i, selUnit->userActionFlags);
			sprintf_s(textHere, "%s\norderSignal(%d): %02d", textHere, i, selUnit->orderSignal);
			sprintf_s(textHere, "%s\nhasANextOrd(%d): %02d", textHere, i, selUnit->orderQueueHead != NULL);
		}
		graphics::drawText(10 , 10, textHere, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
*/
