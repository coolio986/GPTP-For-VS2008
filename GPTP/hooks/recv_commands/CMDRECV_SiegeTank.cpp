...
void CMDRECV_Siege(u8 bCommandType) {
    ...
    *selectionIndexStart = 0;
    ...
    while (current_selection != NULL) {
        if ((current_selection->id == UnitId::TerranSiegeTankTankMode ||
             current_selection->id == UnitId::Hero_EdmundDukeTankMode) &&
            current_selection->mainOrderId != OrderId::SiegeMode) {
            if (current_selection->canUseTech(TechId::TankSiegeMode,
                                              *ACTIVE_NATION_ID) == 1) {
                if (bCommandType != 0)
                    bCommandType = 1;
                else
                ...
                function_004754F0(current_selection,
                                  UnitId::None,
                                  *(u32*)(0x006D5C24),
                                  *(u32*)(0x006D5C20),
                                  OrderId::SiegeMode,
...
void CMDRECV_Unsiege(u8 bCommandType) {
    ...
    *selectionIndexStart = 0;
    ...
    while (current_selection != NULL) {
        if ((current_selection->id == UnitId::TerranSiegeTankSiegeMode ||
