...
bool CMDRECV_PlaceBuildingAllowed(CUnit* builder, u8 orderId, u16 builtUnitId) {
    ...
    if (orderId >= OrderId::DroneStartBuild && orderId <= OrderId::CTFCOP2) {
        if (orderId == OrderId::DroneStartBuild ||
            ...
            if (builder->canMakeUnit(builtUnitId, *ACTIVE_NATION_ID))
                bReturnValue =
                    (OrderAllowed(builder, orderId, *ACTIVE_NATION_ID) != 0);
        } else if (orderId == OrderId::Build5 ||  // Build Nydus Exit
                   orderId == OrderId::BuildingLand) {
            if (builder->id == builtUnitId)
                bReturnValue =
                    (OrderAllowed(builder, orderId, *ACTIVE_NATION_ID) != 0);
        } else if (orderId == OrderId::CTFCOP2) {  // Create flag beacon
...
void cmdRECV_PlaceBuildingNormal(s16 x, s16 y, u32 orderId, u32 builtUnitId) {
    ...
    *selectionIndexStart = 0;
    ...
    if (placeBuildingMsg(result_function_00473FB0)) {
        ...
        function_0048DE70(
            builder, orderId, DimensionX, DimensionY, builtUnitId);
    }
}
...
void cmdRECV_PlaceBuildingAddon(s16 x, s16 y, u32 orderId, u32 builtUnitId) {
    ...
    *selectionIndexStart = 0;
    ...
    if (result_function_00473FB0 == 0) {
        addonPlaceX =
            (s16)units_dat::AddonPlacement[builtUnitId -
                                           UnitId::TerranCommandCenter]
                .x;

        if (addonPlaceX < 0) addonPlaceX += 31;
        ...
        if (addonPlaceY < 0) addonPlaceY += 31;

        addonPlaceY /= 32;

        addonPlaceY = y - addonPlaceY;

        result_function_00473FB0 = function_00473FB0(builder,
                                                     builder->playerId,
                                                     addonPlaceX,
                                                     addonPlaceY,
                                                     builder->id,
    ...
    *selectionIndexStart = 0;
    ...
    if (builder != NULL) {
        if (getActivePlayerNextSelection() ==
            NULL) {  // multiple selection not allowed

            if (x < mapTileSize->width && y < mapTileSize->height &&
                CMDRECV_PlaceBuildingAllowedHelper(
                    builder, orderId, builtUnitId))
...
void cmdRECV_PlaceBuilding(s16 x, s16 y, u8 orderId, u16 builtUnitId) {
    ...
    __asm {
		PUSHAD
		MOVZX EAX, builtUnitId  // bugfix: should have been "MOV AX, builtUnitId" but for
                     // some reason it crashed
		MOV CL, orderId
		PUSH pos
