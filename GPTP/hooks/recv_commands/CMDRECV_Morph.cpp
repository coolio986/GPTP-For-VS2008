...
void CMDRECV_BuildingMorph(u16 wUnitType) {
    ...
    *selectionIndexStart = 0;
    ...
    if (building != NULL && getActivePlayerNextSelection() == NULL &&
        ...
        HasMoneyCanMake(building,
                        wUnitType) &&  // start construction if successful
...
void CMDRECV_UnitMorph(u16 wUnitType) {
    ...
    *selectionIndexStart = 0;
    ...
    while (current_unit != NULL) {
        ...
        if (current_unit->id == UnitId::ZergHydralisk &&
            wUnitType == UnitId::ZergLurker)
            bAllowMorph = current_unit->canUseTech(TechId::LurkerAspect,
                                                   *ACTIVE_NATION_ID) == 1;
        else if (current_unit->id == UnitId::ZergLarva ||
                 current_unit->id == UnitId::ZergMutalisk)
            bAllowMorph =
                current_unit->canMakeUnit(wUnitType, *ACTIVE_NATION_ID) == 1;
        else
            bAllowMorph = false;

        if (bAllowMorph && current_unit->mainOrderId != OrderId::Morph1 &&
            HasMoneyCanMake(current_unit,
                            wUnitType)  // start construction if successful
        ) {
            ...
            if (current_unit->mainOrderId != OrderId::Die) {
                ...
                current_unit->performAnotherOrder(
                    OrderId::Morph1, 0, 0, NULL, UnitId::None, 0);
            }
