...
void receive_command(int x,
    ...
    *saved_order = bActionOrder;
    ...
    function_0049A8C0((u8*)&array_of_data[0],
                      orders_dat::CanBeObstructed[bActionOrder]);
        ...
        *saved_unit = current_unit;
        ...
        while (current_unit != NULL) {
            ...
            if (orders_dat::TechUsed[current_order] != TechId::None)
                bOrderAllowed = (current_unit->canUseTech(
                                     orders_dat::TechUsed[current_order],
                                     *ACTIVE_NATION_ID) == 1);
            else
                bOrderAllowed =
                    (OrderAllowed(
                         current_unit, current_order, *ACTIVE_NATION_ID) == 1);

            if (bOrderAllowed) {
                if (current_unit ==
                    unitParam) {  // target is the caster/ordered unit
            ...
            if (!jump_to_9AD7E) {  // 9ABC4:
                ...
                if (function_0049A410(current_unit, bActionOrder) == 0)
                    jump_to_9AD7E = true;
                else if (current_unit->mainOrderId != OrderId::NukeLaunch ||
                         bActionOrder == OrderId::Die) {  // 9ABE7
                    ...
                    *saved_order_3 = bActionOrder;
                    ...
                    if (current_unit->id == UnitId::TerranMedic) {
                        if (bActionOrder == OrderId::AttackMove ||
                            bActionOrder == OrderId::Attack1) {  // 9AC09
                    ...
                    while (loop_to_9ac0d) {
                        if (!jump_to_9ac4e && !jump_to_9aca9) {
                            if (unitParam == NULL) {
                                ...
                                function_004756E0(current_unit,
                                                  unitParam,
                                                  *saved_order_3,
                                                  bCommandType);
                                ...
                                if (*saved_order_2 == OrderId::Nothing2) {
                                    ...
                                    function_004756E0(current_unit->subunit,
                                                      unitParam,
                                                      *saved_order_2,
                                                      bCommandType);
                                    jump_to_9ad74 = true;
                        ...
                        if (jump_to_9ac4e) {
                            ...
                            if (bActionOrder != OrderId::Attack1)
                                ...
                                if (unitParam != NULL)
                                    *saved_order_3 =
                                        units_dat::AttackUnitOrder[current_unit
                                                                       ->id];
                                else
                                    *saved_order_3 =
                                        units_dat::AttackMoveOrder[current_unit
                                                                       ->id];

                                if (*saved_order_3 == OrderId::Nothing2) {
                                    jump_to_9AD7E = true;
                                    loop_to_9ac0d = false;
                                } else if (current_unit->subunit->isSubunit()) {
                                    if (unitParam != NULL) {
                                        *saved_order_2 =
                                            units_dat::AttackUnitOrder
                                                [current_unit->subunit->id];
                                        jump_to_9aca9 = true;
                                    } else {
                                        *saved_order_2 =
                                            units_dat::AttackMoveOrder
                                                [current_unit->subunit->id];
                                        jump_to_9aca9 = true;
                                    }
                        ...
                        if (jump_to_9aca9) {
                            ...
                            if (*saved_order_3 == OrderId::RallyPoint1) {
                                ...
                                if (target == NULL) target = current_unit;

                                current_unit->rally.unit = target;
                                current_unit->rally.pt.x =
                                    target->sprite->position.x;
                                current_unit->rally.pt.y =
                    ...
                    if (jump_to_9ad09) {
                        ...
                        *saved_y = y;
                        ...
                        if (wUnitType == UnitId::None)
                            function_0049A500(current_unit,
                                              (u8*)&array_of_data[0]);
                        else
                            *saved_unknown_2 = *saved_unknown_1;

                        function_004756B0(current_unit,
                                          *saved_order_3,
                        ...
                        if (*saved_order_2 == OrderId::Nothing2)
                            ...
                            function_004756B0(current_unit->subunit,
                                              *saved_order_2,
                                              *saved_unknown_2 / 65536,
                                              *saved_unknown_2,
                                              bCommandType);
...
void function_004756B0(CUnit* unit,
    ...
    __asm {PUSHAD MOV DL,
           orderId MOV ESI,
           unit PUSH unk1 PUSH unk2 PUSH bCommandType CALL Func_Sub4756B0 POPAD}
...
void function_004756E0(CUnit* unit,
    ...
    __asm {PUSHAD MOV EDX,
           target MOV ESI,
           unit PUSH orderId PUSH bCommandType CALL Func_Sub4756E0 POPAD}
...
bool CanTargetSelf(CUnit* unit, u8 orderId) {
    ...
    if (unit->status & UnitStatus::GroundedBuilding &&
        ...
        return_value = true;
    else if ((unit->id == UnitId::ZergDefiler ||
              unit->id == UnitId::Hero_UncleanOne) &&
             orderId == OrderId::DarkSwarm)
        return_value = true;
    else if (unit->status & UnitStatus::IsHallucination)
        return_value = false;
    else if (unit->id == UnitId::ZergOverlord &&
             UpgradesSc->currentLevel[unit->playerId][UpgradeId::VentralSacs] ==
                 0)
        return_value = false;
    else if (units_dat::SpaceProvided[unit->id] == 0)
        return_value = false;
    else if (orderId == OrderId::MoveUnload)
        return_value = true;
    else
        return_value = false;
