...
void CMDRECV_CancelAddon() {
    ...
    *selectionIndexStart = 0;
    ...
    if (unit != NULL && getActivePlayerNextSelection() == NULL &&
        ...
        if (addon != NULL && !(addon->status & UnitStatus::Completed) &&
...
void CMDRECV_CancelTrain(u16 wUnitID) {
    ...
    *selectionIndexStart = 0;
    ...
    if (unit != NULL && getActivePlayerNextSelection() == NULL &&
        ...
        unit->id != UnitId::ZergHydralisk && unit->id != UnitId::ZergHatchery &&
        unit->id != UnitId::ZergLair && unit->id != UnitId::ZergSpire &&
