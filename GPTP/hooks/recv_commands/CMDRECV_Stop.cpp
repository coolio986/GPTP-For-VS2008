    ...
    while (current_unit != NULL) {
        *u32_0066FF60 = 0x00;
        ...
        if (current_unit->playerId != *ACTIVE_NATION_ID)
            *u32_0066FF60 = 0x01;
        else if (!(current_unit->status & UnitStatus::Completed))
            *u32_0066FF60 = 0x14;
        else if (current_unit->status & UnitStatus::DoodadStatesThing ||
                 current_unit->lockdownTimer != 0 ||
                 current_unit->stasisTimer != 0 ||
                 current_unit->maelstromTimer != 0)
            *u32_0066FF60 = 0x0A;
        else if (current_unit->status & UnitStatus::Burrowed &&
                 current_unit->id != UnitId::ZergLurker)
            *u32_0066FF60 = 0x0B;
        else if (current_unit->id == UnitId::TerranSCV &&
                 current_unit->mainOrderId == OrderId::ConstructingBuilding)
            *u32_0066FF60 = 0x08;
        else if ((current_unit->id == UnitId::TerranGhost ||
            ...
            *u32_0066FF60 = 0x08;
        else if (current_unit->id == UnitId::ProtossArchon &&
                 current_unit->mainOrderId == OrderId::CompletingArchonSummon)
            *u32_0066FF60 = 0x08;
        else if (current_unit->id == UnitId::ProtossDarkArchon &&
                 current_unit->mainOrderId == OrderId::CompletingArchonSummon)
            *u32_0066FF60 = 0x08;
        else if (orders_dat::RequirementsOffset[OrderId::ReaverStop] == 0)
            *u32_0066FF60 = 0x17;
        else if (parseRequirementOpcodes(
                     current_unit,
                     OrderId::ReaverStop,
                     *ACTIVE_NATION_ID,
                     0x00514CF8,
            ...
            if (current_unit->mainOrderId != OrderId::Die) {
                ...
                while (bContinueLoop) {
                    if (current_unit->orderQueueTail == NULL)
                        bContinueLoop = false;
                    else if (orders_dat::CanBeInterrupted
                                 [current_unit->orderQueueTail->orderId] ||
                             current_unit->orderQueueTail->orderId ==
                                 OrderId::ReaverStop)
                        removeOrderFromUnitQueue(current_unit,
                                                 current_unit->orderQueueTail);
                    else
                        bContinueLoop = false;
                }

                current_unit->performAnotherOrder(
                    OrderId::ReaverStop, 0, 0, NULL, UnitId::None, 0);
            }
        ...
        *u32_0066FF60 = 0x00;
        ...
        if (current_unit->playerId != *ACTIVE_NATION_ID)
            ...
            *u32_0066FF60 = 0x08;
                 ...
                 current_unit->mainOrderId == OrderId::CompletingArchonSummon)
            *u32_0066FF60 = 0x08;
        else if (orders_dat::RequirementsOffset[OrderId::CarrierStop] == 0)
            *u32_0066FF60 = 0x17;
        else if (parseRequirementOpcodes(
                     current_unit,
                     OrderId::CarrierStop,
            ...
            if (current_unit->mainOrderId != OrderId::Die) {
                ...
                current_unit->performAnotherOrder(
                    OrderId::CarrierStop, 0, 0, NULL, UnitId::None, 0);
            }
        ...
        *u32_0066FF60 = 0x00;
        ...
        if (current_unit->playerId != *ACTIVE_NATION_ID)
            ...
            *u32_0066FF60 = 0x08;
                 ...
                 current_unit->mainOrderId == OrderId::CompletingArchonSummon)
            *u32_0066FF60 = 0x08;
        else if (orders_dat::RequirementsOffset[OrderId::Stop] == 0)
            *u32_0066FF60 = 0x17;
        else if (parseRequirementOpcodes(
                     current_unit,
                     OrderId::Stop,
                     *ACTIVE_NATION_ID,
                     0x00514CF8,
                     orders_dat::RequirementsOffset[OrderId::Stop]) == 1)
            function_004754F0(current_unit,
                              UnitId::None,
                              *(u32*)(0x006D5C24),
                              *(u32*)(0x006D5C20),
                              OrderId::Stop,
