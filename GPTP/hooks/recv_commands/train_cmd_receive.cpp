...
void CMDRECV_TrainFighter() {
    ...
    *selectionIndexStart = 0;
    ...
    while (current_selection != NULL) {
        ...
        else if (current_selection->id == UnitId::Hero_Gantrithor)
            unitToBuild = UnitId::ProtossInterceptor;
        else if (current_selection->id == UnitId::ProtossReaver)
            unitToBuild = UnitId::ProtossScarab;
        else if (current_selection->id == UnitId::Hero_Warbringer)
            unitToBuild = UnitId::ProtossScarab;
        else
            unitToBuild =
                0;  // not translating it as marine as it's an error case

        if (current_selection->canMakeUnit(unitToBuild, *ACTIVE_NATION_ID) ==
                1 &&
            HasMoneyCanMake(
                current_selection,
                unitToBuild)  // add unit to build queue if successful
        ) {
            if (current_selection->id == UnitId::ProtossCarrier ||
    ...
    *selectionIndexStart = 0;
    ...
    if (builder != NULL && getActivePlayerNextSelection() == NULL) {
        if (builder->canMakeUnit(wUnitType, *ACTIVE_NATION_ID) == 1 &&
            ...
            if (HasMoneyCanMake(
                    builder,
                    wUnitType))  // add unit to build queue if successful
