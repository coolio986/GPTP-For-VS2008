...
void orderNewUnitToRally(CUnit* unit, CUnit* factory) {
    // Do nothing if the rally target is the factory itself or the rally target
    // position is 0
    if (factory != NULL && factory->isFactory() &&
        factory->rally.unit != factory && factory->rally.pt.x != 0) {
        // Following should be allowed only on friendly units
        if (factory->rally.unit != NULL &&
            (factory->rally.unit)->playerId == unit->playerId)
            unit->orderTo(OrderId::Follow, factory->rally.unit);
        else
            unit->orderTo(
                OrderId::Move, factory->rally.pt.x, factory->rally.pt.y);
    }
}
