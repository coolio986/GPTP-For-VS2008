...
// Returns the minimum air/ground weapon range of the @p unit, whichever is
// smaller.
...
bool checkAttackableTarget(CUnit* unit,
                           CUnit* target,
                           u32    seekRange,
                           u32    minRange = 0) {
    ...
    if (!target->sprite->isVisibleTo(unit->playerId)) return false;

    // Check if the target can be attacked.
    if (!unit->canAttackTarget(target)) return false;

    // Check if the target is an enemy
    if (!unit->isTargetEnemy(target)) return false;

    // Check if the target is within the minimum weapon range
    if (minRange != 0) {
        if (unit->getDistanceToTarget(target) <= minRange) return false;
    }

    // Check if the target is within the seek range
    if (!(unit->getDistanceToTarget(target) <= seekRange)) return false;

    CUnit* actualUnit = unit->subunit->isSubunit() ? unit->subunit : unit;

    // If the unit can't turn around, check if the target is within the attack
    // angle.
    if (!(actualUnit->status & UnitStatus::CanTurnAroundToAttack)) {
        if (!isTargetPosWithinAttackAngle(
                target->getX(),
                target->getY(),
                actualUnit,
                units_dat::GroundWeapon[actualUnit->id])) {
            return false;
        }
    }
    ...
    // don't bother
    if (cannotChaseTarget(unit, target)) return false;
...
u32 getAttackPriorityHook(CUnit* target, CUnit* attacker) {
    ...
    if (target->id == UnitId::bunker) {
        ...
        if (firstLoadedUnit) actualTarget = firstLoadedUnit;
    }

    // Units that are not important
    if (target->id == UnitId::larva || target->id == UnitId::egg ||
        target->id == UnitId::cocoon || target->id == UnitId::lurker_egg)
        return 5;
    ...
    if (!(actualTarget->status & UnitStatus::Completed) ||
        actualTarget != target)
        attackPriority += 1;
   ...
   public:
    ...
    void proc(CUnit* target) {
        if (checkAttackableTarget(unit, target, seekRange, minRange))
            attackPriorityData.addTarget(target,
                                         getAttackPriorityHook(target, unit));
    }
...
CUnit* findBestAttackTargetHook(CUnit* unit) {
    ...
    // Bunker bonus
    if (unit->status & UnitStatus::InBuilding) seekRange += 64;
    ...
    attackTargetFinder.search(unit->getX() - searchRange,
                              unit->getY() - searchRange,
                              unit->getX() + searchRange,
                              unit->getY() + searchRange);

    attackTargetFinder.forEach(
        findBestAttackTargetProc(unit, seekRange, minRange));

    return attackPriorityData.findBestTarget(unit);
}

class findRandomAttackTargetProc
    : public scbw::UnitFinderCallbackProcInterface {
   public:
...
CUnit* AttackPriorityData::findRandomTarget() const {
    ...
    while (targetCounts[priority] == 0) {
        ++priority;
        if (priority >= countof(targetCounts)) return NULL;
    }
...
CUnit* AttackPriorityData::findBestTarget(CUnit* attacker) const {
    ...
    if (targetCounts[priority] == 1) return targets[priority][0];
    ...
    if (attacker->pAI && attacker->id == UnitId::scourge) {
        ...
        u32 bestTargetDistance = scbw::getDistanceFast(attacker->getX(),
                                                       attacker->getY(),
                                                       bestTarget->getX(),
                                                       bestTarget->getY());
        ...
        for (unsigned int i = 1; i < targetCounts[priority]; ++i) {
            CUnit*    currentTarget = targets[priority][i];
            const u32 currentTargetDistance =
                scbw::getDistanceFast(attacker->getX(),
                                      attacker->getY(),
                                      currentTarget->getX(),
                                      currentTarget->getY());
...
void AttackPriorityData::reset() {
    for (int i = 0; i < countof(targetCounts); ++i) targetCounts[i] = 0;
}
...
u32 getMinimumRange(CUnit* unit) {
    ...
    if (groundWeapon == WeaponId::None) {
        ...
        if (airWeapon == WeaponId::None)
            ...
            return std::min(weapons_dat::MinRange[groundWeapon],
                            weapons_dat::MinRange[airWeapon]);
    }
}
