...
// KYSXD max larvae - start - new vals for hatchery, lair hive
u16 maxLarva(CUnit *main_unit) {
	u16 larvas = 2;
	if (main_unit) {
        switch (main_unit->id){
...
bool function_004E8C80(CUnit* unit, CUnit* main_building) {
	...
	if(unit->id == UnitId::ZergLarva) {
		...
		if(unit->connectedUnit != main_building)
			...
			(*LARVA_COUNTER)++;
			//KYSXD edit - using max larvae
			bReturnValue = (*LARVA_COUNTER > maxLarva(main_building));
		}
