...
void orders_Repair1(CUnit* unit) {
    ...
    if (target == NULL)
        jump_to_676C4 = true;
    else if (target->hitPoints >= units_dat::MaxHitPoints[target->id])
        jump_to_676C4 = true;
    else if (target->stasisTimer != 0)
        jump_to_676C4 = true;
    else if (unit->pAI != NULL && target->status & UnitStatus::IsBuilding &&
             target->orderTarget.unit != unit)
        jump_to_676C4 = true;
    else if (target->status & UnitStatus::InTransport)
        jump_to_676C4 = true;
    else if (!(units_dat::BaseProperty[target->id] & UnitProperty::Mechanical))
        jump_to_676C4 = true;
    else if (!(target->status & UnitStatus::Completed))
        jump_to_676C4 = true;

    if (!jump_to_676C4) {
        if (units_dat::GroupFlags[target->id].isZerg ||
            units_dat::GroupFlags[target->id].isProtoss ||
            !(units_dat::GroupFlags[target->id].isTerran)) {
            unit->sprite->playIscriptAnim(IscriptAnimation::WalkingToIdle,
                                          true);
            unit->orderToIdle();
            bEndThere = true;
        }
    ...
    if (!bEndThere) {
        ...
        if (!jump_to_676C4) {
            ...
            if (!jump_to_default_switch && unit->mainOrderState == 0) {
                ...
                verifyResources(
                    repairCostMinerals, repairCostGas, unit->playerId);
            ...
            if (!jump_to_default_switch && unit->mainOrderState == 1) {
                if (!(target->status & UnitStatus::Completed)) {
                    ...
                    if (orderToMoveToTarget(unit, target)) {
                        ...
                        unit->mainOrderState =
                            6;  // don't continue directly into this case
                    }
            ...
            if (!bEndThere && !jump_to_default_switch &&
                ...
                if (unit->getMovableState() == 0) {  // not reached destination
                    ...
                    if (!(unit->isTargetWithinMinRange(target, 5))) {
                        ...
                        unitOrderMoveToTargetUnit(unit, target);
                        unit->mainOrderState =
                            7;  // don't continue directly into this case
            ...
            if (!bEndThere && !jump_to_default_switch &&
                ...
                if (!(unit->isTargetWithinMinRange(target, 5))) {
                    ...
                    if (!isUnitPositions2Equal(unit)) {
                        ...
                        if (currentAngle < 0) currentAngle += 256;
                        ...
                        if (currentAngle > weapons_dat::AttackAngle[weaponId])
                            ...
                            unit->sprite->playIscriptAnim(
                                IscriptAnimation::AlmostBuilt, true);
                            unit->mainOrderState =
                                8;  // don't continue directly into this case
                            jump_to_default_switch = true;
                        }
            ...
            if (!bEndThere && !jump_to_default_switch &&
                ...
                if (unit->isTargetWithinMinRange(target, 5) &&
                    target->mainOrderId != OrderId::Liftoff)
                    jump_to_default_switch = true;
                else {
...
void unitOrderMoveToTargetUnit(CUnit* unit, CUnit* target){
    ...
    __asm {PUSHAD MOV EAX,
           unit MOV ECX,
           target CALL Func_unitOrderMoveToTargetUnit POPAD}
