...
void initializeEmptyUnitsLinkedListRef_Sub49E4E0(CUnit* unit,
                                                 u8 playerId);  // 0x0049E4E0
...
bool isInfestableUnit(CUnit* unit) {
    ...
    if (returnValue) {
        ...
        if (((100 * unitHpTest) / unitMaxHp) >= 50) returnValue = false;
    }
...
void orderMorphIntoInfested(CUnit* unitInfested) {
    ...
    bErrorReturnToIdle =
        (unitInfesting == NULL || !isInfestableUnit(unitInfested));
    ...
    if (!bErrorReturnToIdle) {
        ...
        if (!bStopFunction) {
            if (unitInfested->remainingBuildTime != 0) {
                ...
                // this is the same as calling 0047B2E0 unitIsFactoryUnit like
                // original code but faster
                if (unitInfested->isFactory())
                    unitInfested->rally.unit = unitInfested;

                // original code was *(u16*)(0x006648AC + 2*unitInfested->id);
                infestedUnitNewId =
                    units_dat::InfestedUnitPartial[unitInfested->id -
                ...
                // I don't really know, advanced internal mechanics, just have
                // to work with it
                GiveUnitHelper(unitInfested, unitInfesting->playerId, 1);
                initializeEmptyUnitsLinkedListRef_Sub49E4E0(
                ...
                // clear the orders queue of the unit (hardcoding function @
                // 0x004744D0) original code used the actual function instead of
                // hardcoding
                while (unitInfested->orderQueueHead != NULL)
                    removeOrderFromUnitQueue(unitInfested,
                                             unitInfested->orderQueueHead);
                ...
                if (unitInfested->status & UnitStatus::GroundedBuilding) {
                    ...
                    while (current_image != NULL) {
                        ...
                        current_image->playIscriptAnim(
                            IscriptAnimation::WorkingToIdle);
                ...
                infestationOverlayId =
                    units_dat::ConstructionGraphic[infestedUnitNewId];
    ...
    if (!bStopFunction) actUnitReturnToIdle(unitInfested);
}
...
void orderInfestTarget(CUnit* unitInfesting) {
    ...
    } else if (unitInfesting->mainOrderState !=
               0)  // if ==0, wait for EA33F to start infestation logic
        bStopFunction = true;
    ...
    if (!bStopFunction && bBuildTimeReachedZero) {
        ...
        // using unitInfesting->sprite->position because the queen and command
        // center are indeed at the same location, and more importantly, if the
        // infested unit got destroyed, then access to
        // unitInfested->sprite->position would cause a crash
        if (scbw::checkUnitCollisionPos(
                unitInfesting, &unitInfesting->sprite->position, &actualpos)) {
            ...
            if (unitInfesting->orderQueueHead == NULL) {
                unitInfesting->orderSimple(
                    units_dat::ReturnToIdleOrder[unitInfesting->id], false);
                bReturnToIdle = true;
            } else
                bReturnToIdle = true;
    ...
    if (bReturnToIdle)
        ...
        if (!bStopFunction) {  // EA377
            ...
            if (unitInfesting->moveTarget.pt.x !=
                    unitInfesting->nextTargetWaypoint.x ||
                unitInfesting->moveTarget.pt.y !=
                    unitInfesting->nextTargetWaypoint.y) {
                unitInfesting->nextTargetWaypoint.x =
                    unitInfesting->moveTarget.pt.x;
                unitInfesting->nextTargetWaypoint.y =
                    unitInfesting->moveTarget.pt.y;
            }
...
void incrementUnitDeathScores(CUnit* unit, u8 player){
    ...
    __asm {PUSHAD MOV EDI,
           unit MOVZX EDX,
           player CALL Func_IncrementUnitDeathScores POPAD}
...
void incrementUnitScoresEx(CUnit* unit, s32 unkScore, s32 unk2){
    ...
    __asm {PUSHAD MOV EDI,
           unit MOV ECX,
           unk2 PUSH unkScore CALL Func_incrementUnitScoresEx POPAD}
