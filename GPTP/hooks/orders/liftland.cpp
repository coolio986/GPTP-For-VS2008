...
void orders_BuildingLand(CUnit* unit) {
    ...
    if (unit->status & UnitStatus::GroundedBuilding) {
        ...
        if (unit->mainOrderId != OrderId::Die) {
            ...
            while (unit->orderQueueTail != NULL && !bLoopBreak) {
                bLoopBreak =
                    !orders_dat::CanBeInterrupted[unit->orderQueueTail
                                                      ->orderId] &&
            ...
            unit->performAnotherOrder(
                OrderId::BuildingLiftoff, coords.x, coords.y, NULL, 0);
        ...
        if (unit->mainOrderId != OrderId::Die) {
            while (unit->orderQueueTail != NULL &&
                   unit->orderQueueTail->orderId == OrderId::BuildingLand)
                removeOrderFromUnitQueue(unit, unit->orderQueueTail);

            // 6440A:
            unit->performAnotherOrder(OrderId::BuildingLand,
                                      unitTarget.pt.x,
                                      unitTarget.pt.y,
                                      unitTarget.unit,
                                      UnitId::None);
        ...
        function_004EB9C0(
            ...
            unit->orderTarget.pt.y - unit->sprite->images.tail->verticalOffset);
        ...
        if (unit->getMovableState() != 0) {
            ...
            u32 return_00473FB0 = function_00473FB0(
                unit, unit->playerId, x, y, unit->id, 0, 0, 1, 0);
            ...
            if (return_00473FB0 != 0) {
                ...
                // use a AddFogMask() function after some coordinates
                // calculations
                function_00469F60(
                ...
                if (unit->sprite->images.tail->paletteType ==
                    ...
                    unit->sprite->images.tail->flags |=
                        CImage_Flags::DontUpdate;
        ...
        if (!isUnitPositions2Equal(unit)) {
            int someValue = scbw::getAngle(unit->sprite->position.x,
    ...
    if (unit->mainOrderState == 3) {
        ...
        if ((unit->getMovableState() != 0) && (unit->orderSignal & 0x10)) {
            ...
            // maybe the landing unit would get excluded from multi-units
            // selection if that could happen in normal Starcraft gameplay?That
            // may be what this function do.
            ...
            if (unit->orderQueueHead != NULL) {
                ...
                if (unit->orderQueueHead->orderId == OrderId::Move ||
                    ...
                    if (order->orderId != OrderId::PlaceAddon) order = NULL;
                ...
                if (bGotMoveOrder)
                    // 646C7
                    // this cause the landed building to immediately take off
                    // again to execute the queued move/follow order
                    function_00474760(
                        unit, unit->orderQueueHead, OrderId::BuildingLiftoff);
            ...
            if (addon != NULL) completeAddon(unit, addon);
...
void orders_BuildingLiftoff(CUnit* unit) {
    if ((unit->status & UnitStatus::GroundedBuilding) &&
        ...
        // 649D5
        if (unit->building.addon != NULL) disconnectFromAddOn(unit);

        if (unit->isFactory()) unit->rally.unit = unit;
        ...
        if (*IS_PLACING_BUILDING) placebuildingProc();
...
void SetUnitMovementSpeed(CUnit* unit, u32 newSpeed){
    ...
    __asm {PUSHAD MOV EAX,
           unit MOV EDX,
           newSpeed CALL Func_SetUnitMovementSpeed POPAD}
