...
void orders_ReaverAttack1(CUnit* unit) {
    ...
    if (target == NULL) {
        ...
        if (!unit->canAttackTarget(target, true))
            unit->orderTo(
                OrderId::Move, unit->orderTarget.pt.x, unit->orderTarget.pt.y);
        else {
            // 656F7:
            u32 status_saved = (unit->status & UnitStatus::HoldingPosition) +
            ...
            // restore the "disabled" and "holding position" status if they were
            // lost
            unit->status &=
                ~(UnitStatus::Disabled + UnitStatus::HoldingPosition);
...
void orders_CarrierAttack1(CUnit* unit) {
    ...
    if (target == NULL) {
        ...
        if (!unit->canAttackTarget(target, true))
            ...
            // 659CF
            u32 status_saved = (unit->status & UnitStatus::Disabled) +
...
void orders_TurretAttack(CUnit* unit) {
    ...
    if (unit->status & UnitStatus::DoodadStatesThing ||
        ...
        unit->maelstromTimer != 0 || target == NULL ||
        ...
        if (unit->id == UnitId::ProtossCarrier ||
            ...
            // identical to check performed by unitIsReaver @ 0x00401490, using
            // if(!unitIsReaver())
            if (!(unit->id == UnitId::ProtossReaver ||
                  unit->id == UnitId::Hero_Warbringer))
            jump_to_77A9D = true;
        else if (target->status & UnitStatus::InAir)
            jump_to_77A45 = true;
        else if (unit->hasPathToUnit(target))
            jump_to_77AFE = true;
        else
            jump_to_77A45 = true;
    ...
    do {
        ...
        if (jump_to_77A70) {  // 77A70
            ...
            while (!bStopLoop77A70 && unit->orderQueueTail != NULL) {
                if (!orders_dat::CanBeInterrupted[unit->orderQueueTail
                                                      ->orderId] &&
        ...
        if (jump_to_77AB7) {
            if (unit->id == UnitId::ProtossArbiter && unit->pAI != NULL) {
                ...
                if (target->status & UnitStatus::InAir)
                    current_unit_weaponId =
                        units_dat::AirWeapon[current_unit->id];  // 77AD2
                else
                    current_unit_weaponId =
                        current_unit->getGroundWeapon();  // 77AE6
        ...
        if (jump_to_77AFE) {
            if (!isTargetVisible(unit, target)) {
                ...
                if (unit->id != UnitId::TerranSiegeTankTankModeTurret ||
                    ...
                    (current_subunit->canUseTech(TechId::TankSiegeMode,
                                                 unit->playerId) != TRUE) ||
                    !isUnitVisible(unit))
                    jump_to_77BD7 = true;
                else {
                    // 77B5F:
                    u32 result_getMaxWpnRange = unit->getMaxWeaponRange(
                        units_dat::GroundWeapon
                            [UnitId::TerranSiegeTankSiegeModeTurret]);

                    if (!isTargetWithinMinMovementRange(
                            current_subunit, target, result_getMaxWpnRange))
                        jump_to_77BD7 = true;
                    else {
                        // 77B77:
                        result_getMaxWpnRange = unit->getMaxWeaponRange(
                            units_dat::GroundWeapon
                                [UnitId::TerranSiegeTankTankModeTurret]);

                        if (isTargetWithinMinMovementRange(
                                current_subunit,
                                target,
                                result_getMaxWpnRange) ||
                            ...
                            unit->orderSimple(
                                units_dat::AttackUnitOrder
                                    [UnitId::TerranSiegeTankSiegeModeTurret],
                                false);

                            current_subunit->orderComputerCL(
                                OrderId::SiegeMode);

                            // this orderId is hardcoded, not calculated
                            current_subunit->issueQueuedOrderTarget(
                                units_dat::AttackUnitOrder
                                    [UnitId::TerranSiegeTankSiegeMode],
                                target,
                                false);
                        }
                ...
                if (jump_to_77BD7) {
                    ...
                    if (target != NULL) {
                        if (isTargetVisible(unit, target)) {
                            ...
                            if (target->status & UnitStatus::InAir)
                                current_unit_weaponId =
                                    current_unit->getAirWeapon();
                            else
                                current_unit_weaponId =
                                    current_unit
                                        ->getGroundWeapon();  // include the
                                                              // lurker special
                                                              // case so it's ok
                            ...
                            if (current_unit_weaponId >= WeaponId::None)
                                jump_to_77C78 = true;
                            else if (weapons_dat::MinRange
                                             [current_unit_weaponId] != 0 &&
                                     current_unit->isTargetWithinMinRange(
                                         target,
                                         weapons_dat::MinRange
                                             [current_unit_weaponId]))
                                jump_to_77C78 = true;
                            else {
                                // 77C63:
                    ...
                    if (jump_to_77C78) {
                        if ((unit->subunit)->mainOrderId ==
                                units_dat::AttackUnitOrder[(unit->subunit)
                                                               ->id] &&
                            getRightClickActionOrder(unit->subunit) !=
                                RightClickActions::NoMove_NormalAttack)
                            jump_to_77CA1 = true;
                        else
                            unit->orderComputerCL(
                                OrderId::TurretGuard);  // 77C93
                    }

                    jump_to_77C78 = false;

                    if (jump_to_77CA1) {
                        if (unit->orderTarget.pt.x !=
                        ...
                        // 77CC1
                        if (!((unit->subunit)->movementFlags &
                              MovementFlags::Accelerating))
                            getWeaponBeginIscript(unit,
                                                  IscriptAnimation::GndAttkRpt);
                    }
        ...
        if (jump_to_77CDA) {
            unit->performAnotherOrder(
                OrderId::TurretGuard, 0, 0, NULL, UnitId::None);
...
void orders_AttackFixedRange(CUnit* unit) {
    ...
    if (jump_to_77E99) {
        ...
        } else if (unit->pAI == NULL)
            unit->orderToIdle();  // 77EC8
        else
            unit->orderTo(OrderId::AttackMove,
    ...
    if (target == NULL) {  // 788EB
        ...
        if (!unit->canAttackTarget(target, true))
            ...
            if (!bStopThere && unit->mainOrderState == 1) {  // 78975
                ...
                if (!unit->isTargetWithinMinRange(target, 4) &&
                    unit->getMovableState() != 2) {  // 78998

                    if (unit->isTargetWithinMinRange(target, 256) &&
                    ...
                    if (!unit->isTargetWithinMinRange(
                            target,
                            weapons_dat::OuterSplashRadius
                                [WeaponId::Suicide_InfestedTerran])) {
                        unit->orderToIdle();
                        bStopThere = true;
                    } else {  // 789F4
                        ...
                        while (current_image != NULL) {
                            current_image->playIscriptAnim(
                                IscriptAnimation::SpecialState1);
...
void orders_SapLocation(CUnit* unit) {
    ...
    if (unit->mainOrderState == 0) {  // 78A5C
        ...
        bStopThere = !SetMoveTarget_xy(
            unit, unit->orderTarget.pt.x, unit->orderTarget.pt.y);
    ...
    if (!bStopThere && unit->mainOrderState == 1) {  // 78A89
        ...
        if (unit->sprite->position.x != unit->moveTarget.pt.x ||
            ...
            if (unit->status & UnitStatus::Unmovable) {  // 78AB8
                ...
                u32 distance =
                    scbw::getDistanceFast(unit->orderTarget.pt.x * 256,
                                          unit->orderTarget.pt.y * 256,
                                          unit->halt.x,
                                          unit->halt.y);

                if ((distance / 256) > weapons_dat::OuterSplashRadius
                                           [WeaponId::Suicide_InfestedTerran]) {
                    unit->orderToIdle();
                    bStopThere = true;
                }
...
void orders_AttackMoveEP(CUnit* unit) {
    ...
    if (function_00478370(unit, unit->mainOrderId) &&
        unit->mainOrderTimer == 0) {  // 78E0B

        unit->mainOrderTimer = 15;

        if (SAI_GetRegionIdFromPxEx(unit->sprite->position.x,
                                    unit->sprite->position.y) ==
            SAI_GetRegionIdFromPxEx(unit->orderTarget.pt.x,
                                    unit->orderTarget.pt.y)) {
            ...
            if (unit->getMovableState() != 0 &&
                (unit->orderTarget.pt.x != unit->sprite->position.x ||
                 unit->orderTarget.pt.y !=
                     unit->sprite->position.y)) {  // 78E80

                if (function_004EB9C0(  // return value is actually from
                    ...
                    if (function_00476610(unit,
                                          (s32)unit->orderTarget.pt.x,
                                          (s32)unit->orderTarget.pt.y))
                    makeToHoldPosition(unit);
            ...
            if (jump_to_78EAE)
                function_00477820(
                    unit,
                    saved_mainOrderId);  // use attack range, may cause idle,
...
void orders_AttackMove(CUnit* unit) {
    ...
    if (function_00478370(unit, unit->mainOrderId) != 0 &&
        unit->mainOrderTimer == 0) {  // 79058

        unit->mainOrderTimer = 15;

        if (unit->moveTarget.pt.x != unit->sprite->position.x ||
            unit->moveTarget.pt.y != unit->sprite->position.y
            /*|| (unit->status & UnitStatus::Unmovable) + 1 == 0)*/  // impossible
                                                                     // code yet
                                                                     // in
                                                                     // original
        )
            // Logically, this function would check one last time if a target is
            // in range, and attack it if that's the case, else AttackMove end
            // here
...
void orders_AttackUnit(CUnit* unit) {
    if (playerTable[unit->playerId].type != PlayerType::Computer ||
        (unit->orderTarget.unit != NULL &&
         units_dat::BaseProperty[(unit->orderTarget.unit)->id] &
        ...
        if (function_00478540(unit)) {  // 79C16
            ...
            if (unit->pAI != NULL) AI_AttackUnit(unit);
...
void      removeOrderFromUnitQueue(CUnit* unit, COrder* order){
    ...
    __asm {PUSHAD MOV ECX,
           unit MOV EAX,
           order CALL Func_removeOrderFromUnitQueue POPAD}
...
void      function_00474A70(CUnit* unit, CUnit* target, u8 orderId){
    ...
    __asm {PUSHAD MOV CL,
           orderId MOV EDX,
           unit MOV EAX,
           target CALL Func_Sub474A70 POPAD}
...
void      getWeaponBeginIscript(CUnit* unit, IscriptAnimation::Enum anim){
    ...
    __asm {PUSHAD MOV EAX,
           unit MOV DL,
           anim CALL Func_getWeaponBeginIscript POPAD}
...
bool isUnitCritter(CUnit* unit) {
    ...
    if (unit->id == UnitId::Critter_Rhynadon ||
...
u8 getRightClickActionOrder(CUnit* unit) {
    ...
    if (unit->id == UnitId::ZergLurker && unit->status & UnitStatus::Burrowed)
        ...
        if (temp_result == 0 && unit->status & UnitStatus::GroundedBuilding &&
            unit->isFactory())
