...
void orders_Die(CUnit* unit) {
    ...
    if (unit->userActionFlags & 4) hideAndDisableUnit(unit);

    if (unit->subunit != NULL) {
        clearUnitSprites(unit->sprite, unit->subunit->sprite);

        if (unit->subunit->subunit != NULL) {
            if (units_dat::BaseProperty[(unit->subunit->subunit)->id] &
    ...
    if (!(unit->sprite->flags & CSprite_Flags::Hidden)) {
        if (!(unit->status & UnitStatus::IsHallucination) ||
            ...
            hallucination_smoke = createThingy(
                SpriteId::Hallucination_Death1, unit->getX(), unit->getY(), 0);

            if (hallucination_smoke != NULL) {
                // allow players to see smoke if could already see unit
                // vanishing
                function_00497480(hallucination_smoke->sprite,
                                  unit->sprite->visibilityFlags);
            }
        }
    }
