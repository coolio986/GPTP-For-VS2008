...
void orders_MedicHoldPosition(CUnit* unit) {
    ...
    if (unit->mainOrderState == 0) {
        ...
        if (unit->nextTargetWaypoint.x != unit->moveTarget.pt.x ||
    ...
    if (switchValue != 1) {
        ...
        if (!(unit->orderSignal & 1)) {
            ...
            if (unit->orderQueueHead == NULL ||
                unit->orderQueueHead->orderId != OrderId::MedicHeal2)  // 6413D
                orderImmediate(
                    unit, OrderId::MedicHeal2);  // return to idle after heal
        }

        // 64146
        if ((unit->orderTarget.unit)->sprite->position.x !=
...
void orders_ReaverStop(CUnit* unit) {
    ...
    while (outHangarChild != NULL) {
        ...
        if (outHangarChild->mainOrderId != OrderId::Die) {
            ...
            while (outHangarChild->orderQueueTail != NULL &&
                   !bNotRemovableOrder) {
                if (!orders_dat::CanBeInterrupted[outHangarChild->orderQueueTail
                                                      ->orderId] &&
                    outHangarChild->orderQueueTail->orderId !=
                        OrderId::SelfDestrucing)
                    bNotRemovableOrder = true;
                else
                    removeOrderFromUnitQueue(outHangarChild,
                                             outHangarChild->orderQueueTail);
            }

            // 65539
            outHangarChild->performAnotherOrder(
                OrderId::SelfDestrucing, 0, 0, NULL, UnitId::None);
        }
    ...
    if (unit->mainOrderId != OrderId::Die) {
        ...
        // 655C3
        unit->performAnotherOrder(OrderId::Reaver,
                                  pos.x,
                                  pos.y,
...
void orders_TurretGuard(CUnit* unit) {
    if (unit->subunit->nextTargetWaypoint.x != unit->nextTargetWaypoint.x ||
    ...
    if (!function_004770E0(unit)) {
        if (unit->mainOrderTimer == 0) {
            ...
            if (units_dat::BaseProperty[unit->id] & UnitProperty::Subunit) {
                if (unit->subunit->nextTargetWaypoint.x !=
                        unit->nextTargetWaypoint.x ||
                    unit->subunit->nextTargetWaypoint.y !=
                        unit->nextTargetWaypoint.y) {
                    unit->nextTargetWaypoint.x =
                        unit->subunit->nextTargetWaypoint.x;
                    unit->nextTargetWaypoint.y =
                        unit->subunit->nextTargetWaypoint.y;
                }
            }
...
void      function_00476FC0(CUnit* unit, CUnit* target, u32 unk1, u32 unk2){
    ...
    __asm {PUSHAD PUSH unk1 PUSH unk2 MOV EAX,
           target MOV EDI,
           unit CALL Func_Sub476FC0 POPAD}
