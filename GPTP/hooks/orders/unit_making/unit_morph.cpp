...
void orders_ZergBirth(CUnit* unit) {
    if (unit->orderSignal & 4) {
        ...
        if (units_dat::BaseProperty[unit->id] & UnitProperty::TwoUnitsIn1Egg) {
            ...
            if (unit2 != NULL) AI_TrainingUnit(unit, unit2);
        }

        if (unit->id != UnitId::ZergDrone && unit->id != UnitId::ZergOverlord)
        ...
        if (bTwinEggOrCocoon) {
            if (units_dat::MovementFlags[unit->id] == MovementFlags::HoverUnit)
                pos.y -= 7;
            else if (unit->status & UnitStatus::InAir)
                pos.y -= 42;
        }

        fixTargetLocation(&pos, unit->id);

        if (pos.x != unit->sprite->position.x ||
            pos.y != unit->sprite->position.y)
        ...
        if (unit->previousUnitType != UnitId::ZergCocoon &&
            ...
            if (unit2 != NULL) orderNewUnitToRally(unit2, unit->connectedUnit);
        }
...
void orders_Morph1(CUnit* unit) {
    if (unit->mainOrderState == 0) {
        if (unit->id == UnitId::ZergLarva ||
            ...
            if (!hasSuppliesForUnit(unit->buildQueue[unit->buildQueueSlot],
                                    unit->playerId,
                                    1)) {
                ...
                else if (unit->id == UnitId::ZergMutalisk)
                ...
                if (!bStopThere) {
                    ...
                    unit->remainingBuildTime = units_dat::TimeCost
        ...
        if (advanceRemainingBuildTime_Sub466940(unit)) {
            unit->sprite->playIscriptAnim(IscriptAnimation::SpecialState1,
                                          true);
