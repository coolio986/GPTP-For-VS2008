    ...
    if (unit->status & UnitStatus::GroundedBuilding &&
        ...
        if (unit->building.upgradeResearchTime != 0xFFFF) {
            ...
            if (unit->building.upgradeType >= UpgradeId::UnusedUpgrade46)
                upgradeLevel =
                    UpgradesBw->currentLevel[unit->playerId]
                                            [unit->building.upgradeType -
        ...
        if (!bStopThere) {
            ...
            if (upgradeLevel < unit->building.upgradeLevel) {
                if (isUnitUpgradeAvailable(unit)) {
                    setUpgradeLevel(unit->playerId,
                                    unit->building.upgradeType,
                                    unit->building.upgradeLevel);
                    ApplySpeedUpgradeFromUpgradeType(
                        unit, unit->building.upgradeType);
                }
            }
    ...
    if (unit->building.techType != TechId::None) {
        ...
        if (unit->building.upgradeResearchTime != 0xFFFF) {
            ...
            if (unit->building.techType < TechId::Restoration)
                bIsResearched =
                    TechSc
                        ->isResearched[unit->playerId][unit->building.techType];
            else
                bIsResearched =
                    TechBw
                        ->isResearched[unit->playerId][unit->building.techType -
        ...
        if (!bStopThere) {
            ...
            if (unit->building.techType < TechId::Restoration)
                TechSc->isResearched[unit->playerId][unit->building.techType] =
                    1;
            else if (*IS_BROOD_WAR)
                TechBw->isResearched[unit->playerId][unit->building.techType -
                                                     TechId::Restoration] = 1;
