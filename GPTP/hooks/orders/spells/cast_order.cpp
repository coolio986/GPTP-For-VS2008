...
void orders_Spell(CUnit* unit) {
    ...
    if (unit->mainOrderState == 2) {
            ...
            if (target != NULL) {
                ...
                if (target->sprite->position.x != unit->nextTargetWaypoint.x ||
                    target->sprite->position.y !=
                        unit->nextTargetWaypoint.y) {  // 928C1
        ...
        if (!canCastSpell_0(unit)) {
            char* message = (char*)statTxtTbl->getString(876);  // Invalid
                                                                // Target
            ...
            if (!(*CHEAT_STATE & CheatFlags::TheGathering) &&
                ...
                if (unit->mainOrderState == 0) unit->mainOrderState = 1;
                ...
                if (result_00492620 == 0) {
                    ...
                    if (unit->spellCooldown == 0 &&
                        function_00476640(
                            unit, orderWeaponId) &&  // is ready to fire weapon
                                                     // (angle,range...) maybe?
                        !(unit->movementFlags & MovementFlags::Accelerating) &&
                        unit->getMovableState() != 0) {  // 92A38
                        ...
                        if (unit->pAI != NULL && target != NULL) {
                            if (orderWeaponId <=
                                WeaponId::Maelstrom) {  // Maelstrom is the last
                                                        // spell-related weapon

                                // array bound to the function
                                const u8* const array_00492B44 =
                                    (u8*)(0x00492B44);

                                // this code would not work for weapons id of
                                // NuclearMissile, YamatoGun and weapons id
                                // before those, they must be excluded somewhere
                                // else Maybe the switch and array use could be
                                // removed in a future version since enough
                                // elements use default value (switch 6)
                                u32 switch_value =
                                    array_00492B44[orderWeaponId - 0x20];

                                if (switch_value != 6) {
                                    // no idea why irradiate is so important
                                    u32 analyzed_target_status =
                                        target->irradiateTimer;

                                    if (switch_value == 2)
                                        analyzed_target_status |=
                                            target->parasiteFlags;
                        ...
                        if (!bStopHere) {  // 92AA3
                            ...
                            cooldown =
                                getUpgradedWpnCooldown(unit, orderWeaponId);

                            // a complicated way to add between -1 and 2 to the
                            // cooldown
                            ...
                            unit->movementFlags |=
                                MovementFlags::StartingAttack;

                            if (unit->sprite->images.head != NULL) {
                                CImage* current_image =
                                    unit->sprite->images.head;
                                IscriptAnimation::Enum anim =
                                    (IscriptAnimation::Enum)orders_dat::
                                        OrderIscriptAnim[unit->mainOrderId];
                            ...
                            if (unit->pAI != NULL)
                                AIScriptController[unit->playerId]
                                    .spellcasterTimer = 1;
                        }
                    }
