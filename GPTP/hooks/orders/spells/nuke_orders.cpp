    ...
    if (unit->mainOrderState != 0) {
        if (unit->mainOrderState == 6) {
            if (unit->connectedUnit == NULL ||
                unit->connectedUnit->mainOrderState == 5) {  // 64D3F
                ...
                if (unit->building.ghostNukeMissile != NULL) {
                    current_image =
                        (unit->building.ghostNukeMissile)->sprite->images.head;
        ...
        nukeDot = createThingy(SpriteId::Nuke_Target_Dot,
...
void orders_NukeGround(CUnit* unit) {
    ...
    if (unit->nextTargetWaypoint.x != unit->orderTarget.pt.x ||
    ...
    if (unit->pAI != NULL) {
        ...
        if (distance / 256 <= sightRange * 3) AI_orderUnitCloaking(unit);
    }
        ...
        if (!bEndThere) {
            ...
            while (!bLoopEnd) {
                if (current_unit == NULL)
                    bLoopEnd = true;
                else if (current_unit->id == UnitId::TerranNuclearSilo &&
            ...
            if (current_unit == NULL)
                ...
                nuke->orderTo(OrderId::NukeLaunch,
                ...
                unit->orderTo(OrderId::NukeTrack,
                              unit->sprite->position.x,
                              unit->sprite->position.y);

                if (unit->status & UnitStatus::DoodadStatesThing ||
                    unit->lockdownTimer != 0 || unit->stasisTimer != 0 ||
                    unit->maelstromTimer != 0) {  // 793C4

                    if (units_dat::BaseProperty[unit->id] &
                        UnitProperty::Building)
                        unit->currentButtonSet =
                            UnitId::Buttons_CancelNukeStrike;
                } else
