...
void spells_Recall_ActionOnValidTargetForRecall(CUnit* target, CUnit* caster) {
    ...
    function_004EBAE0(
        target, caster->sprite->position.x, caster->sprite->position.y);

    if (!scbw::checkUnitCollisionPos(
            target, &(caster->sprite->position), &new_pos, NULL, false, 0))
        ...
        // finish the teleporting (and cut the recalled unit from a possible
        // unit targeted by an order)
        ...
        recall_effect_sprite =
            createThingy(SpriteId::Recall_Field, new_pos.x, new_pos.y, 0);
        if (recall_effect_sprite != NULL) {
            recall_effect_sprite->sprite->elevationLevel =
                target->sprite->elevationLevel + 1;
            setThingyVisibilityFlags(recall_effect_sprite);
        }

        // disconnect a nuke from a ghost using it
        if (target->id == UnitId::TerranGhost ||
...
}  // void spells_Recall_ActionOnValidTargetForRecall(CUnit* target, CUnit*
   // caster)
...
void spells_Recall_ActionOnTargetInRange(CUnit* target, CUnit* caster) {
    if (!(target->sprite->flags & CSprite_Flags::Hidden) &&
        ...
        target->id != UnitId::ZergLarva && target->id != UnitId::ZergEgg &&
        target->id != UnitId::ZergLurkerEgg &&
        !(target->status & UnitStatus::IsHallucination) && target != caster)
    ...
    if (unit->mainOrderState == 1) {
        ...
        if (unit->mainOrderTimer == 0) {
            ...
            *bAtLeast1UnitWasRecalled = 0;
            ...
            // apply the recal effect on each valid target found in
            // spell_location
            IterateUnitsAtLocationTargetProc_RecallTargets(unit,
                                                           &spell_location);

            if (*bAtLeast1UnitWasRecalled != 0) {
                // original code added (RandomizeShort(0x12) & 1) to the base
                // sound id Protoss_ARBITER_PAbFol02_WAV then called the
                // playSound function using that value, but since there are only
                // 2 sounds, it's easier to see what sound is used that way. The
                // compiler may make it back like the original anyway.

                if ((RandomizeShort(0x12) & 1) == 0)
                    scbw::playSound(SoundId::Protoss_ARBITER_PAbFol02_WAV,
                                    unit);
                else
                    scbw::playSound(SoundId::Protoss_ARBITER_PAbFol03_WAV,
                                    unit);
            }
        ...
        if (!scbw::isCheatEnabled(CheatFlags::TheGathering) &&
            ...
            message = (char*)statTxtTbl->getString(
                864 + raceId);  //"Not enough energy" text

            if (message != NULL && unit->playerId == *LOCAL_NATION_ID &&
            ...
            recall_effect_sprite = createThingy(SpriteId::Recall_Field,
                                                unit->orderTarget.pt.x,
                                                unit->orderTarget.pt.y,
                                                0);
            ...
            if ((random_value & 1) == 0)
                ...
                playUnitSFX(SoundId::Protoss_ARBITER_PAbFol01_WAV,
                            unit->orderTarget.pt.x,
                            unit->orderTarget.pt.y,
                            1,
                            0);
