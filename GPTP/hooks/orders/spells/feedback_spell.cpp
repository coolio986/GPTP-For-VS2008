    ...
    if (target != NULL &&
        ...
        if (ordersSpell_Sub_4926D0(unit,
            ...
            if (target->status & UnitStatus::IsHallucination) {
                ...
                replaceFeedbackSprite(
                    target, overlaySize + SpriteId::Feedback_Hit_Small);
                ...
                if (target->energy <= 0)
                    ...
                    target->damageWith(target->energy,
                                       WeaponId::Feedback,
                                       unit,
                                       unit->playerId,
                                       1,
                    ...
                    target->energy = 0;
                    unit->spendUnitEnergy(techCost);
                    scbw::playSound(SoundId::Protoss_Darchon_feedback_wav,
                                    target);
                    ...
                    if (!target->isDead()) {
                        ...
                        if (target->subunit != NULL)
                            target->subunit->sprite->createTopOverlay(
                                overlaySize + ImageId::Feedback_Small, 0, 0, 0);
                        else
                            target->sprite->createTopOverlay(
                                overlaySize + ImageId::Feedback_Small, 0, 0, 0);
