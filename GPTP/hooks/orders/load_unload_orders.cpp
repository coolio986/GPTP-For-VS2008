...
void orders_Pickup1(CUnit* unit) {
    ...
    if (unitToLoad != NULL && scbw::canBeEnteredBy(unit, unitToLoad)) {
        ...
        if (unit->mainOrderId != OrderId::Die) {
            ...
            while (!bStopLoop) {
                ...
                if (orders_dat::CanBeInterrupted[orderId] == 0 &&
            ...
            unit->performAnotherOrder(
                OrderId::ComputerAI, 0, 0, NULL, UnitId::None);
        }
...
void orders_Pickup2(CUnit* unit) {
    ...
    if (unitToLoad == NULL) {
        ...
        if (unitToLoad->pAI == NULL ||
            unitToLoad->mainOrderId == OrderId::Pickup4) {  // E762F
                ...
                if (scbw::canBeEnteredBy(unit, unitToLoad)) {
                    if (unit->mainOrderTimer == 0) {
                        ...
                        if (!unit->isTargetWithinMinRange(unitToLoad, 1))
                            ...
                            if (unitToLoad->mainOrderId !=
                                    OrderId::EnterTransport &&
                                unitToLoad->mainOrderId != OrderId::Pickup4)
                                unitToLoad->orderTo(OrderId::EnterTransport,
                                                    unit);
                            ...
                            if (unit->pAI != NULL)
                                ...
                                function_00474A70(
                                    unit, unitToLoad, OrderId::Pickup1);
                                prepareForNextOrder(unit);
                            }
                        }
                    }
...
void orders_MoveUnload(CUnit* unit) {
    ...
    // E7739
    distance = scbw::getDistanceFast(unit->orderTarget.pt.x * 256,
    ...
    if (distance <= 0x1000) {
        if (unit->orderQueueHead != NULL) {
            ...
            unit->performAnotherOrder(
                OrderId::Unload, 0, 0, NULL, UnitId::None);
            prepareForNextOrder(unit);
        }
        ...
        if (unit->pAI != NULL) {
            u32 groundHeightAtPxPos1 = scbw::getGroundHeightAtPos(
                unit->sprite->position.x, unit->sprite->position.y);

            if (groundHeightAtPxPos1 !=
                scbw::getGroundHeightAtPos(unit->orderTarget.pt.x,
                ...
                if (distance <= 0xCA000 &&
                    ...
                    while (!bStopLoop) {
                        CUnit* unitToUnload =
                            getLoadedUnitFromIndex(unit, index);

                        if (unitToUnload != NULL) {
                            if (unitToUnload->id == UnitId::ProtossReaver &&
                                bIsDistanceGreaterThanHaltDistance)
                                bStopLoop = true;
...
void orders_Pickup4_0(CUnit* unit) {
    ...
    if (unit->playerId == 11) {
        ...
        if (unit->status & UnitStatus::InTransport)
            ...
            unit->order(units_dat::ReturnToIdleOrder[unit->id],
                        0,
                        0,
                        NULL,
                        UnitId::None,
        ...
        if (!bEndThere) {
            ...
            if (jump_to_E7C37) unit->orderTarget.unit = NULL;

            if (jump_to_E7C44) {
                if (unit->isTargetWithinMinRange(target, 1)) {  // E7C52

                    loadUnitProc(target, unit);
                    target->orderTo(OrderId::MoveUnload,
                                    unit->orderTarget.pt.x,
                                    unit->orderTarget.pt.y);
...
void orders_EnterTransport(CUnit* unit) {
    ...
    if (target != NULL && !(target->status & UnitStatus::IsHallucination) &&
        (target->id != UnitId::ZergOverlord ||
         UpgradesSc->currentLevel[unit->playerId][UpgradeId::VentralSacs] !=
             0) &&
        units_dat::SpaceProvided[target->id] != 0 &&
        scbw::canBeEnteredBy(target, unit)) {  // E7D51
        ...
        if (unit->isTargetWithinMinRange(target, 1)) loadUnitProc(target, unit);
    ...
    if (unit->status & UnitStatus::GroundedBuilding ||
        ...
        while (!bEndLoop) {
            ...
            if (*loadedUnitOffset != 0 &&
                unitTable_0059CB64[unit->loadedUnit[counter].index].link.prev !=
                    NULL) {
                CUnit* unitToUnload =
                    &(unitTable_0059CB58[unit->loadedUnit[counter].index]);

                if (unitToUnload->mainOrderId != 0 ||
                    unitToUnload->mainOrderState != 1) {
            ...
            if (!bEndLoop) {
                ...
                if (counter >= 8) bEndLoop = true;
            }

        }  // while(!bEndLoop)

        if (!bFoundUnitToUnload) jump_to_E8191 = true;
...
bool unitIsActiveTransport(CUnit* unit) {
    ...
    if (!(unit->status & UnitStatus::IsHallucination)) {
        if (unit->id != UnitId::ZergOverlord ||
            UpgradesSc->currentLevel[unit->playerId][UpgradeId::VentralSacs] !=
                0) {
            if (units_dat::SpaceProvided[unit->id] != 0) result = true;
        }
    }
