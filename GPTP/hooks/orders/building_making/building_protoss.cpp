...
void orders_ProbeBuild(CUnit* unit) {
    ...
    if (unit->mainOrderState == 0) {
        ...
        if (!(unit->movementFlags & MovementFlags::Accelerating)) {
            ...
            if (!isDistanceGreaterThanHaltDistance(unit,
                ...
                if (function_00467030(unit)) {
                    ...
                    if (isBuildingAssimilator)
                        builtUnit =
                            function_004678A0(unit, UnitId::ProtossAssimilator);
                    else
                        builtUnit =
                    ...
                    if (builtUnit == NULL) {
                        ...
                        replaceSpriteImages(
                            builtUnit->sprite, ImageId::WarpAnchor, 0);

                        if (isBuildingAssimilator)
                            function_00498D70(builtUnit->sprite,
                                              ImageId::VespeneGeyser,
                                              0,
                                              0,
                                              0);
                        ...
                        scbw::playSound(SoundId::Protoss_SHUTTLE_pshBld00_wav,
                                        unit);
...
void orders_BuildSelf2(CUnit* building) {
    ...
    if (building->mainOrderState > 3)
        jump_to_E50EE = true;
    else if (building->mainOrderState == 0) {
        if (building->remainingBuildTime == 0) {
            ...
            while (current_image != NULL) {
                if (current_image->flags & CImage_Flags::Flag04) {
                    ...
                    iscript_OpcodeCases(
                        current_image,
                        (u32)&current_image->iscriptHeaderOffset,
                        0,
                        0);
                }
    ...
    if (jump_to_E50EE) {
        ...
        if (building->shields + shieldGain >=
            (u32)units_dat::MaxShieldPoints[building->id] * 256)
...
void function_00498D70(CSprite* sprite,
    ...
    __asm {PUSHAD MOV EAX,
           sprite MOV ESI,
           imageId PUSH unk1 PUSH unk2 PUSH unk3 CALL Func_Sub498D70 POPAD}
