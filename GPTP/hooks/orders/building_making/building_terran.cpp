...
void orders_TerranBuildSelf(CUnit* building) {
    if (building->mainOrderState == 0) {
        if (building->hitPoints >
            20 * units_dat::MaxHitPoints[building->id] / 100)
            building->mainOrderState = 1;
    } else if (building->mainOrderState == 1) {
    ...
    } else if (building->mainOrderState == 2) {
        if (building->hitPoints >
            40 * units_dat::MaxHitPoints[building->id] / 100)
            building->mainOrderState = 3;
    } else if (building->mainOrderState == 3) {
        CImage* current_image = building->sprite->images.head;
    ...
    } else if (building->mainOrderState == 4) {
        if (building->hitPoints >
            60 * units_dat::MaxHitPoints[building->id] / 100)
            building->mainOrderState = 5;
    } else if (building->mainOrderState == 5) {
    ...
    } else if (building->mainOrderState == 6) {
        if (building->hitPoints >
            80 * units_dat::MaxHitPoints[building->id] / 100)
...
void orders_SCVBuild2(CUnit* unit) {
    ...
    if (builtUnit != NULL && !(builtUnit->status & UnitStatus::Completed)) {
        ...
        if (unit->mainOrderState > 8 || unit->mainOrderState == 1)
            jump_to_switch_1 = true;

        if (unit->mainOrderState == 0) {
            if (orderToMoveToTarget(unit, builtUnit)) unit->mainOrderState = 2;
        }
        ...
        if (unit->mainOrderState == 2) {
            ...
            if (movableState == 2)  // unmovable
                unit->orderToIdle();
            else if (movableState == 1) {  // reached destination

                scbw::refreshConsole();

                if (builtUnit->connectedUnit != NULL &&
                    ...
                    builtUnit->connectedUnit = unit;
                    unit->status &=
                        ~UnitStatus::IsNormal;  // remove UnitStatus::IsNormal
                                                // if present
                    unit->sprite->elevationLevel =
                        units_dat::Elevation[unit->id] + 1;

                    unit->orderSimple(OrderId::ResetCollision1, false);
                    unit->orderSimple(units_dat::ReturnToIdleOrder[unit->id],
                                      false);

                    if (function_004EB9C0(unit,
            ...
            if (unit->getMovableState() != 0) unit->mainOrderState = 4;
        } else if (unit->mainOrderState == 4) {
            s16 ebp_18, ebp_16, ebp_14, ebp_12, x_min, y_min, x_max, y_max;
            s32 ebp_08;

            ebp_18 = builtUnit->sprite->position.x -
                     builtUnit->sprite->unkflags_12 / 2;
            ebp_14 = builtUnit->sprite->position.x +
                     builtUnit->sprite->unkflags_12 / 2 - 1;
            ebp_16 = builtUnit->sprite->position.y -
                     builtUnit->sprite->unkflags_13 / 2;
            ebp_12 = builtUnit->sprite->position.y +
                     builtUnit->sprite->unkflags_13 / 2 - 1;
            ebp_08 = units_dat::BuildingDimensions[builtUnit->id].x;

            if (ebp_08 < 0)
                x_min =
                    builtUnit->sprite->position.x - (ebp_08 - 0xFFFFFFFF) / 2;
            else
            ...
            if (units_dat::BuildingDimensions[builtUnit->id].y < 0)
                y_min = builtUnit->sprite->position.y -
                        (units_dat::BuildingDimensions[builtUnit->id].y -
                         0xFFFFFFFF) /
                            2;
            else
                y_min = builtUnit->sprite->position.y -
                        units_dat::BuildingDimensions[builtUnit->id].y / 2;
            ...
            if (x_min < ebp_18) x_min = ebp_18;

            if (y_min < ebp_16) y_min = ebp_16;

            if (x_max > ebp_14) x_max = ebp_14;

            if (y_max > ebp_12) y_max = ebp_12;
            ...
            if (function_004F1870(
                    unit,
                    unit->orderTarget.pt.x - unit->sprite->position.x,
                    unit->orderTarget.pt.y - unit->sprite->position.y) != 0) {
            ...
            if (!isDistanceGreaterThanHaltDistance(
                ...
                angle = scbw::getAngle(unit->sprite->position.x,
                                       unit->sprite->position.y,
                                       unit->nextTargetWaypoint.x,
                                       unit->nextTargetWaypoint.y);

                unit->orderTarget.pt.x = (angleDistance[angle].x * 20) / 256 +
            ...
            if (!isUnitPositions2Equal(unit)) {
                s32 angle = scbw::getAngle(unit->sprite->position.x,
                                           unit->sprite->position.y,
                                           unit->nextTargetWaypoint.x,
                                           unit->nextTargetWaypoint.y);
            ...
            if (unit->mainOrderTimer != 0)
                ...
                while (current_image != NULL) {
                    current_image->playIscriptAnim(
                        IscriptAnimation::GndAttkToIdle);
                    current_image = current_image->link.next;
                }
        ...
        if (jump_to_switch_1) {
            ...
            if (*CHEAT_STATE & CheatFlags::OperationCwal) hpGain *= 16;
            ...
            if (builtUnit->status & UnitStatus::Completed) {
                ...
                while (current_image != NULL) {
                    current_image->playIscriptAnim(
                        IscriptAnimation::WalkingToIdle);
                    current_image = current_image->link.next;
                }
                ...
                if (builtUnit != NULL) {
                    ...
                    if (builtUnit->status & UnitStatus::Completed) {
                        if (builtUnit->id == UnitId::TerranRefinery ||
                    ...
                    if (!jump_to_67F3C) {  // 67F2C
                        ...
                        if (builtUnit->id >= UnitId::ResourceMineralField &&
...
void orders_SCVBuild(CUnit* unit) {
    ...
    if (unit->pAI != NULL) unit->status |= UnitStatus::IsGathering;
    ...
    if (unit->mainOrderState == 1) {
        if (unit->sprite->position.x == unit->moveTarget.pt.x &&
            ...
            if (!(unit->status & UnitStatus::Unmovable) &&
                isDistanceGreaterThanHaltDistance(unit,
                                                  unit->orderTarget.pt.x,
                ...
                if (unit->orderQueueHead == NULL) {  // 680A5
                    ...
                    function_004749A0(
                        unit, NULL, units_dat::ReturnToIdleOrder[unit->id]);
                } else
                    function_00474760(
                        unit, unit->orderQueueHead, OrderId::ResetCollision1);

                if (unit->buildQueue[unit->buildQueueSlot] ==
                    UnitId::TerranRefinery)
                    builtUnit = function_004678A0(unit, UnitId::TerranRefinery);
                else
                    builtUnit =
                        createUnit(unit->buildQueue[unit->buildQueueSlot],
                                   unit->orderTarget.pt.x,
                                   unit->orderTarget.pt.y,
                                   unit->playerId);
    ...
    if (!bEndThere && unit->mainOrderState == 0) {  // 68188
        ...
        bMoveTargetResult = SetMoveTarget_xy(
        ...
        if (bMoveTargetResult) unit->mainOrderState = 1;
    }
}
...
bool isUnitPositions2Equal(CUnit* unit) {
    return (unit->nextTargetWaypoint.x == unit->position.x &&
...
void function_00474760(CUnit* unit, COrder* order, u8 orderId){
    ...
    __asm {PUSHAD MOV ESI,
           unit MOV EDI,
           order MOV BL,
           orderId CALL Func_Sub474760 POPAD}
...
void function_004749A0(CUnit* unit, COrder* order, u8 orderId){
    ...
    __asm {PUSHAD MOV BL,
           orderId MOV ESI,
           unit MOV EDI,
           order CALL Func_Sub4749A0 POPAD}
