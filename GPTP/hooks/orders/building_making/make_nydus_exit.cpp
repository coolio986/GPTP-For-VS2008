...
void orders_Build5(CUnit* nydus_canal) {
    ...
    result_f00473FB0 = function_00473FB0(nydus_canal,
    ...
    if (result_f00473FB0 != 0) {
        ...
        } else if (nydus_canal->pAI != NULL)
            nydus_canal->orderComputerCL(OrderId::ComputerAI);
        else
            nydus_canal->orderComputerCL(
                units_dat::ReturnToIdleOrder[nydus_canal->id]);

    } else {  // 5DCDB

        CUnit* nydusExit = createUnit(UnitId::ZergNydusCanal,
        ...
        if (nydusExit == NULL) {
            ...
            nydusExit->hitPoints =
                (10 * units_dat::MaxHitPoints[nydusExit->id]) / 100;
