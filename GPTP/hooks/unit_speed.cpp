// Contains hooks that control unit movement speed, acceleration, and turn
// speed.
...
/// Calculates the unit's modified movement speed, factoring in upgrades and
/// status effects.
///
...
u32 getModifiedUnitSpeedHook(CUnit* unit, u32 baseSpeed) {
    ...
    int speedModifier = (unit->stimTimer ? 1 : 0) -
                        (unit->ensnareTimer ? 1 : 0) +
                        (unit->status & UnitStatus::SpeedUpgrade ? 1 : 0);

    if (speedModifier > 0) {
        if (unit->id == UnitId::scout || unit->id == UnitId::Hero_Mojo ||
            unit->id == UnitId::Hero_Artanis)
            speed = 1707;
        else {
            speed += (speed / 2);

            if (speed < 853) speed = 853;
        }
...
/// Calculates the unit's acceleration, factoring in upgrades and status
/// effects.
///
