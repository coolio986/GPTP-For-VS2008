...
void killAllHangarUnits(CUnit* unit) {
    ...
    while (unit->carrier.outHangarCount != 0) {
        ...
        // 66239
        unit->carrier.outHangarChild =
            childOutside->interceptor.hangar_link.prev;
...
void freeResourceContainer(CUnit* resource) {
    ...
    while (worker != NULL) {
        if (worker->worker.harvest_link.prev !=
            NULL)  // untie the previous worker from current worker (tie it to
                   // next)
            (worker->worker.harvest_link.prev)->worker.harvest_link.next =
                worker->worker.harvest_link.next;
        else
            resource->building.resource.nextGatherer =
                worker->worker.harvest_link.next;

        if (worker->worker.harvest_link.next !=
            NULL)  // untie the next worker from current worker (tie it to
                   // previous)
            (worker->worker.harvest_link.next)->worker.harvest_link.prev =
                worker->worker.harvest_link.prev;
...
void unitDestructorSpecialHook(CUnit* unit) {
    ...
    if (unit->id == UnitId::ProtossCarrier ||
        ...
        if (unit->id == UnitId::ghost || unit->id == UnitId::sarah_kerrigan ||
        ...
        if (unit->building.ghostNukeMissile != NULL) {
            (unit->building.ghostNukeMissile)
                ->sprite->playIscriptAnim(IscriptAnimation::Death, true);
        ...
        if ((unit->id >= UnitId::ResourceMineralField &&
            ...
            ((unit->id == UnitId::TerranRefinery ||
             ...
             unit->status &
                 UnitStatus::Completed) /*this test is equivalent to function
                                           unit_isGeyserUnit @ 004688B0*/
        ) {
        ...
        if (unit->id == UnitId::ProtossScarab ||
            ...
            if (unit->status & UnitStatus::Completed) {
                ...
                if (unit->interceptor.parent != NULL) {
                    if (unit->interceptor.hangar_link.next !=
                        NULL)  // untie the next interceptor/scarab from current
                               // interceptor/scarab (tie it to previous)
                        (unit->interceptor.hangar_link.next)
                            ->interceptor.hangar_link.prev =
                            unit->interceptor.hangar_link.prev;
                    else {
                        if (unit->interceptor.isOutsideHangar)
                            unit->interceptor.parent->carrier.outHangarChild =
                                unit->interceptor.hangar_link.prev;
                        else
                            unit->interceptor.parent->carrier.inHangarChild =
                                unit->interceptor.hangar_link.prev;
                    }
                    ...
                    if (unit->interceptor.hangar_link.prev !=
                        NULL)  // untie the previous interceptor/scarab from
                               // current interceptor/scarab (tie it to next)
                        (unit->interceptor.hangar_link.prev)
                            ->interceptor.hangar_link.next =
                            unit->interceptor.hangar_link.next;
            ...
            // begin hardcoding removeFromPsiProviderList @ 00493100 to avoid
            // use of a helper

            if (unit->psi_link.prev != NULL)  // untie the previous pylon from
                                              // current pylon (tie it to next)
                (unit->psi_link.prev)->psi_link.next = unit->psi_link.next;
            if (unit->psi_link.next !=
                NULL)  // untie the next pylon from current pylon (tie it to
                       // previous)
            ...
            // end hardcoding removeFromPsiProviderList @ 00493100 to avoid use
            // of a helper
