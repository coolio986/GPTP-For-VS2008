...
enum Enum : u32 {
    ...
    Two_Units_Selected__Archon_Warp =
        0x004296B0,  // BTNSCOND_CanMergeArchonTwoSelected
    Two_Units_Selected_and_Not_Researched_Dark_Archon_Meld =
...
BUTTON_SET* getCustomButtonSet() {
    ...
    // buttons
    customButtonSet.buttonsInSet =
        buttonSetTable[UnitId::TerranMarine].buttonsInSet;

    // connectedUnit should usually be init at 0xFFFF.Almost unused (maybe not
    // fully implemented).
    ...
    // For each button in the array until reaching the amount of buttonsInSet,
    // copy the data for each member of the button structure from the marine to
    // the custom set.
    for (u32 i = 0; i < customButtonSet.buttonsInSet; i++) {
        customButtonsArray[i].actFunc =
            buttonSetTable[UnitId::TerranMarine].firstButton[i].actFunc;
        customButtonsArray[i].actStringID =
            buttonSetTable[UnitId::TerranMarine].firstButton[i].actStringID;
        customButtonsArray[i].actVar =
            buttonSetTable[UnitId::TerranMarine].firstButton[i].actVar;
        customButtonsArray[i].iconID =
            buttonSetTable[UnitId::TerranMarine].firstButton[i].iconID;
        customButtonsArray[i].position =
            buttonSetTable[UnitId::TerranMarine].firstButton[i].position;
        customButtonsArray[i].reqFunc =
            buttonSetTable[UnitId::TerranMarine].firstButton[i].reqFunc;
        customButtonsArray[i].reqStringID =
            buttonSetTable[UnitId::TerranMarine].firstButton[i].reqStringID;
        customButtonsArray[i].reqVar =
            buttonSetTable[UnitId::TerranMarine].firstButton[i].reqVar;
    }
...
void updateButtonSetEx() {
    ...
    do {  // count units to know if will be using special group buttonset
          // (peons,cloakables...)
        ...
        if (unitFromSelection != NULL) {
            ...
            if (flagsHolder & UnitProperty::Worker) countWorkers++;

            if (flagsHolder & UnitProperty::Cloakable) countCloakables++;

            // count flying zerg units (to disable "burrow group" if there's
            // one)
            if (!(unitFromSelection->status & UnitStatus::InAir) &&
                units_dat::GroupFlags[unitFromSelection->id].isZerg)
                countNotInAirAndZergs++;

            if (flagsHolder & UnitProperty::Burrowable) countBurrowables++;

            // if you use a custom buttonset, and are reusing the
            // currentButtonSet property of an unit differently, change these
            // conditions to take it in account
            if (selectedButtonSetId != activeButtonSetId) {
                BUTTON_SET* selectedUnitButtonSet =
                    hooks::getButtonSet(selectedButtonSetId);
                BUTTON_SET* activeUnitButtonSet =
                    hooks::getButtonSet(activeButtonSetId);

                if (activeUnitButtonSet->firstButton !=
                    selectedUnitButtonSet->firstButton) {
                    if (selectedUnitButtonSet->connectedUnit !=
                        activeButtonSetId) {
                        // kerrigan(ghost) and raynor(vulture) have the
                        // corresponding normal unit as "connected unit", they
                        // are the only ones unless you add some custom ones

                        if (activeUnitButtonSet->connectedUnit ==
                            selectedButtonSetId) {
                            activeButtonSetId = selectedButtonSetId;
                            *BUTTONSET_CURRENT_BUTTONSETID =
                                selectedButtonSetId;
                        } else
                            *BUTTONSET_CURRENT_BUTTONSETID =
                                UnitId::Buttons_GroupMixed;
                    }
                }
    ...
    if (*BUTTONSET_CURRENT_BUTTONSETID != UnitId::Buttons_Blank) {
        ...
        else if (countCloakables == *clientSelectionCount)
            *BUTTONSET_CURRENT_BUTTONSETID = UnitId::Buttons_GroupCloaker;
        else if ((countBurrowables != 0) &&
                 (countNotInAirAndZergs == *clientSelectionCount))
            *BUTTONSET_CURRENT_BUTTONSETID =
                UnitId::Buttons_GroupBurrower;  // aerial units and non-zergs
                                                // units prevent this (but not
                                                // Ultralisk)
    }
...
void updateButtonSet_Sub458D50() {
    ...
    if (currentSubUnitMainOrderId != 0xFF)
        currentSubUnitButtonIcon =
            orders_dat::ButtonIcon[currentSubUnitMainOrderId];
    else
        currentSubUnitButtonIcon = 0xFFFF;

    if (currentUnitButtonIcon != 0xFFFF) {
        if (currentSubUnitButtonIcon == 0xFFFF ||
            currentUnitButtonIcon != IconId::Stop)
        ...
        if (*clientSelectionCount != 0) {
            if (*bDoingCancellableTargetOrder == 0) {
                if (*IS_PLACING_BUILDING)
                    *BUTTONSET_SPECIAL_BUTTONSETID =
                        UnitId::Buttons_CancelPlaceBuilding;
            } else
...
void updateCurrentButtonset() {
    ...
    if (!*bCanUpdateCurrentButtonSet) {
        ...
        if (*BUTTONSET_SPECIAL_BUTTONSETID != UnitId::Buttons_Blank) {
            if (*IS_IN_REPLAY) bNeedUpdateButtonSet = true;
        } else
            bNeedUpdateButtonSet = true;

        if (bNeedUpdateButtonSet) updateButtonSet();
    }
        ...
        if (!jumpTo59A38) {
            ...
            if (!jumpTo59A9F) {
                ...
                if ((*activePortraitUnit)->subunit == NULL)
                    ...
                    mainOrderIdHolderSubUnit_BL =
                        (*activePortraitUnit)->subunit->mainOrderId;
                    jumpTo59A86 = true;
                }
    ...
    if (!jumpTo59A83 && !jumpTo59A86 && !jumpTo59A9F) {
        ...
        if (*activePortraitUnit != NULL) {
            ...
            if (*BUTTONSET_UNIT_MAINORDERID == mainOrderIdHolderUnit_EBP) {
                ...
                if (*BUTTONSET_SUBUNIT_MAINORDERID ==
                    mainOrderIdHolderSubUnit_BL)
                    jumpTo59ABD = true;
            }
    ...
    if (jumpTo59A9F) {
        if (*IS_IN_REPLAY) replayStatBtns(*BUTTONSET_DIALOG);
...
void updateButtonSet_Sub4591D0() {
    ...
    if (current_dialog == NULL || current_dialog->index != 1) bStopThere = true;

    while (!bStopThere) {
        // 59230
        if (current_dialog->index < 1)
            bStopThere = true;
        else if (current_dialog->index > 9)
            jumpto5934E = true;
        else {  // loop to ignore the first unused/disabled buttons

            current_button_state = BUTTON_STATE::Disabled;

            while (current_button_state == BUTTON_STATE::Disabled &&
                   (buttons_count < current_buttonset->buttonsInSet)) {
                current_button_state = req_check((u32)current_button->reqFunc,
                                                 (u8)current_button->reqVar,
                                                 (u8)*LOCAL_NATION_ID,
                                                 *activePortraitUnit);

                if (current_button_state == BUTTON_STATE::Disabled) {
                    buttons_count++;
                    current_button++;  // select next button (advance by
                                       // sizeof(BUTTON))
                }
            };
        }
        ...
        if (!bStopThere && !jumpto5934E) {
            jumpto5933C = (!*IS_IN_REPLAY) && (*activePortraitUnit != NULL) &&
                ...
                if (buttons_count >= current_buttonset->buttonsInSet ||
                    ...
                    if (current_button_state == BUTTON_STATE::Invisible) {
                        ...
                        if (*unknown_0066FF60 == 0x15) {
                            current_button->actStringID =
                                0x02FA;        /*Text: Not Implemented.*/
                            current_button++;  // select next button (advance by
                                               // sizeof(BUTTON)
                            buttons_count++;
                            jumpto5934E = true;
        ...
        if (current_dialog == NULL) bStopThere = true;
...
const u32 Btn_fxnInteractFuncs[] = {0x004598D0,
                                    0x004598D0,
                                    0x004598D0,
                                    0x004598D0,
                                    0x004598D0,
...
void statbtn_BIN_CustomCtrlID(BinDlg* dialog) {
    registerUserDialogAction(
        dialog, (u32)&Btn_fxnInteractFuncs, sizeof(Btn_fxnInteractFuncs));
