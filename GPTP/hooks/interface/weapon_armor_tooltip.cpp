...
u8 getDamageFactorForTooltip(u8 weaponId, CUnit* unit) {
    ...
    if (unit->id == UnitId::valkyrie) return 1;
...
const char* getDamageTooltipString(u8 weaponId,
    ...
    const u8 upgradeLevel = scbw::getUpgradeLevel(
        unit->playerId, weapons_dat::DamageUpgrade[weaponId]);
    const u16 baseDamage = weapons_dat::DamageAmount[weaponId] * damageFactor;
    const u16 bonusDamage =
        weapons_dat::DamageBonus[weaponId] * damageFactor * upgradeLevel;
...
const char* getSpecialTooltipString(u16 iconUnitId, CUnit* unit) {
    ...
    if (iconUnitId == UnitId::interceptor)
        return getDamageTooltipString(
            WeaponId::PulseCannon, unit, 791);  //"Interceptors"
    ...
    if (iconUnitId == UnitId::spider_mine)
        return getDamageTooltipString(
            WeaponId::SpiderMines, unit, 794);  //"Spider Mines"
