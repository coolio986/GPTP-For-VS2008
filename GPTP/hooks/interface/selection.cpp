    ...
    while (!bStopBigLoop) {
        if (current_unit != unit) {
            ...
            if (current_unit->subunit != NULL) {
                ...
                if (units_dat::BaseProperty[current_unit->id] &
            ...
            if (!bDontAddToList) {
                ...
                // if the unit sprite is visible, perform additionnal visibility
                // test, else skip the unit
                if (current_unit->sprite->visibilityFlags &
                    (u8)*VISIBILITY_CHECK_0057F0B0) {
                    // if the unit is cloaked and visibility status test
                    // (detection by detectors units probably) fail, the unit is
                    // skipped
                    if (current_unit->status &
                        (UnitStatus::Cloaked + UnitStatus::RequiresDetection))
                        if (!(current_unit->visibilityStatus &
                              *VISIBILITY_CHECK_0057F0B0))
                            bDontAddToList = true;
                ...
                if (!bDontAddToList) {
                    ...
                    if (unit_isUnselectable(current_unit->id))
                        bDontAddToList = true;
                    else if (!unit_IsStandardAndMovable(current_unit))
                        bKeepForEmptyListCase =
                            true;  // skip the unit for the list, unless it's
                                   // the only available
                    else if (*IS_IN_REPLAY)
                        bKeepForEmptyListCase =
                            true;  // skip the unit for the list, unless it's
                                   // the only available
                    else if (current_unit->playerId != *LOCAL_NATION_ID)
                        bKeepForEmptyListCase =
                            true;  // skip the unit for the list, unless it's
                                   // the only available
                    else {
                        ...
                        if (unit != NULL) {
                            // all units should belong to the same player (the
                            // one owning the clicked unit)
                            if (unit->playerId != current_unit->playerId)
                                ...
                                // equivalent to " unit->status XOR
                                // current_unit->status " keep the flags
                                // appearing in only 1 of the variables
                                u32 mixed_flags =
                                ...
                                if (mixed_flags & UnitStatus::Burrowed)
                                    ...
                                    if (!isUnitBurrowed(current_unit)) {
                                        // Only select all detectables OR all
                                        // undetectables units.The test exclude
                                        // current_unit if the detectable state
                                        // differ from the one of unit.
                                        if (mixed_flags &
                                            UnitStatus::RequiresDetection)
                                            bDontAddToList = true;
                                    }
                                    ...
                                    if (!bDontAddToList) {
                                        ...
                                        // Only select hallucinations OR
                                        // non-hallucinations units.The test
                                        // exclude current_unit if the
                                        // hallucination state differ from the
                                        // one of unit.
                                        if (mixed_flags &
                                            UnitStatus::IsHallucination)
                                            bDontAddToList = true;
                                    }
                                }
                            }
                        ...
                        if (!bDontAddToList) {
                            ...
                            if (current_index_in_unit_list >=
                                SELECTION_ARRAY_LENGTH)  // action when
                                                         // unit_list is full
                                function_0046F040(current_unit,
                                                  unit_list,
                                                  unit,
                                ...
                                unit_list[current_index_in_unit_list] =
                                    current_unit;
                                current_index_in_unit_list++;
                            }
                        }
        ...
        if (bKeepForEmptyListCase) backup_current_unit = current_unit;
    ...
    if (current_index_in_unit_list == 0) {
        ...
        if (backup_current_unit == NULL) {
            ...
            if (unit == NULL)
                ...
                unit_list[0] = unit;
                current_index_in_unit_list =
                    1;  // return 1 (the clicked unit alone)
            }
            ...
            if (unit != NULL) {
                ...
                current_index_in_unit_list =
                    1;  // return 1 (an unit preserved by bKeepForEmptyListCase)
            }
        }
    }
...
void function_0046FB40(CUnit* clicked_unit) {
    ...
    CUnit*
        local_temp_array_1[SELECTION_ARRAY_LENGTH];  // used instead of an array
                                                     // starting from [EBP-3C]
    CUnit*
        local_temp_array_2[SELECTION_ARRAY_LENGTH];  // used instead of an array
                                                     // starting from [EBP-6C]

    Box16 local_temp_box16_structure;  // used instead of using from [EBP-0C] to
                                       // [EBP-06]

    bool isHoldingCtrl_OR_isDoubleClickingSelectedClickedUnit =
        *IS_HOLDING_CTRL ||
        (*IS_DOUBLE_CLICKING &&
         (clicked_unit->sprite->flags & CSprite_Flags::Selected));
    ...
    if (isHoldingCtrl_OR_isDoubleClickingSelectedClickedUnit ||
        ...
        if (!*IS_HOLDING_SHIFT) {
            ...
            if (isHoldingCtrl_OR_isDoubleClickingSelectedClickedUnit) {
                ...
                sorted_list_length = SortAllUnits(
                    clicked_unit, local_temp_array_1, units_in_bounds);

                // reload the previous temporary unit list from before the call
                // to getAllUnitsInBounds
                *tempUnitsListArraysCountsListLastIndex =
                    *tempUnitsListArraysCountsListLastIndex - 1;
                *tempUnitsListCurrentArrayCount = tempUnitsListArraysCountsList
                    [*tempUnitsListArraysCountsListLastIndex];

                if (sorted_list_length != 0) {
                    // turn the local array as the newly applied selection
                    applyNewSelect_Sub_6FA00(local_temp_array_1,
                                             sorted_list_length);

                    // this is what make the rally point briefly appear when
                    // selecting a building with one.
                    // maybe can do others things?
                    if (sorted_list_length == 1 &&
            ...
            // holding SHIFT (if not holding CTRL, then there's an active
            // selection too)

            if (isHoldingCtrl_OR_isDoubleClickingSelectedClickedUnit) {
                //{Ctrl+Shift+click on unit} OR {Shift+double-click on
                // selected unit WHILE having an active selection}
                // =>	select units same type as clicked unit, adding them to
                // the
                //		active selection
                ...
                *tempUnitsListCurrentArrayCount = tempUnitsListArraysCountsList
                ...
                if (sorted_list_length != 0) {
                    ...
                    // recombine current selection and newly selectable items in
                    // local_temp_array_2
                    u32 list_remixed_length =
                        combineLists_Sub_6F290(clicked_unit,
                                               local_temp_array_1,
                                               local_temp_array_2,
                                               sorted_list_length);

                    // turn local_temp_array_2 as the newly applied selection
                    applyNewSelect_Sub_6FA00(local_temp_array_2,
                                             list_remixed_length);
                }
                ...
                for (arrayIndex = 0;
                     ...
                     activePlayerSelection->unit[arrayIndex] != NULL;
                     arrayIndex++)
                    local_temp_array_1[arrayIndex] =
                        activePlayerSelection->unit[arrayIndex];

                if (!(clicked_unit->sprite->flags & CSprite_Flags::Selected)) {
                    // 0x0046FD13 (getting unit sprite) and 0x0046FD18 (middle
                    // of the test with the flag)

                    // unit not selected, so it's added to current selection if
                    // valid

                    if (arrayIndex < SELECTION_ARRAY_LENGTH &&
                        ...
                        // turn local_temp_array_1 as the newly applied
                        // selection
                        applyNewSelect_Sub_6FA00(local_temp_array_1,
                                                 arrayIndex);
                    }
                    ...
                    // each element is 4 bytes, multiply it by the number of
                    // elements after the one to remove
                    memcpy_size =
                        (arrayIndex - clicked_unit->sprite->selectionIndex) * 4;

                    // Copy the elements on the right of the one to delete one
                    // step left to overwrite the one to delete
                    SC_memcpy_0((u32)&local_temp_array_1[clicked_unit->sprite
                                                             ->selectionIndex],
                                (u32)&local_temp_array_1
                                    [clicked_unit->sprite->selectionIndex + 1],
                                memcpy_size);

                    // either erase the element to delete or one that was
                    // repeated after memcpy
                    ...
                    if (arrayIndex == 1) {
                        if (*IS_HOLDING_ALT) {
                            ...
                            u32 unitIndex =
                                CUnitToUnitID(local_temp_array_1[0]);
                    ...
                    if (bUpdateSelection) {
                        // Create a new unit selection (since the clicked unit
                        // is unselected) and choose it as current selection)
                        CreateNewUnitSelectionsFromList(local_temp_array_1,
                                                        arrayIndex);
...
bool isUnitBurrowed(CUnit* unit) {
    return (unit->status &
            (UnitStatus::CloakingForFree + UnitStatus::Burrowed)) != 0;
}
...
void function_0046F040(CUnit* current_unit,
    ...
    __asm {PUSHAD PUSH clicked_unit PUSH current_unit MOV ECX,
           unit_list MOV EAX,
           list_length CALL Func_Sub_6F040 POPAD}
...
void CreateNewUnitSelectionsFromList(CUnit** unit_list, u32 unit_list_length){
    ...
    __asm {PUSHAD PUSH unit_list_length MOV EAX,
           unit_list CALL Func_CreateNewUnitSelectionsFromList POPAD}
