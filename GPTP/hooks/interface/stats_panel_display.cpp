...
void stats_panel_display(BinDlg* dialog) {
    ...
    if (!(activeUnit->status & UnitStatus::IsHallucination) ||
        ...
        if (activeUnit->id != UnitId::ZergLurker ||
            ...
            if (groundWeaponMainUnit == WeaponId::None) {
                if (activeUnit->subunit == NULL)
                    ...
                    groundWeaponSubunit =
                        units_dat::GroundWeapon[activeUnit->subunit->id];
            } else
            ...
            if (!bNoGroundWeaponDisplay) {
                ...
                else if (activeUnit->id == UnitId::ZergLurker &&
                         !(activeUnit->status & UnitStatus::Burrowed))
                    groundWeapon = UnitId::None;  // 26D2A
                else if (groundWeaponMainUnit != WeaponId::None)
                    groundWeapon = groundWeaponMainUnit;  // 26D35
                else if (activeUnit->subunit == NULL)
                    groundWeapon = WeaponId::None;  // 26D41
                else
                    groundWeapon = units_dat::GroundWeapon[activeUnit->subunit
                                                               ->id];  // 26D51
        ...
        if (airWeaponMainUnit != WeaponId::None)
            ...
            airWeaponSubunit =
                units_dat::AirWeapon[activeUnit->subunit->id];  // 26D8C

        if (!bNoAirWeaponDisplay) {
            if (airWeaponSubunit == WeaponId::None)
                bNoAirWeaponDisplay = true;  // 26D94
            else if (airWeaponMainUnit != WeaponId::None)
            ...
            if (!bNoAirWeaponDisplay) {
                ...
                if (activeUnit->id == UnitId::ZergLurker &&
                    ...
                    if (groundWeapon == WeaponId::None &&
                        ...
                        groundWeapon =
                            units_dat::GroundWeapon[activeUnit->subunit
                                                        ->id];  // 26DE6
                }
                ...
                else if (airWeaponMainUnit == WeaponId::None) {  // 26DFF

                    if (activeUnit->subunit == NULL)
                        airWeapon = WeaponId::None;  // 26E06
                    else
                        airWeapon = units_dat::AirWeapon[activeUnit->subunit
                                                             ->id];  // 26E16
                }
        ...
        // 26E31
        if (activeUnit->id == UnitId::ProtossCarrier ||
        ...
        if (activeUnit->id == UnitId::TerranVulture ||
...
u16 getLastQueueSlotType(CUnit* unit) {
    ...
    if (unit->status & UnitStatus::Completed)
        ...
        if (unitId != UnitId::ZergHive && unitId != UnitId::ZergLair &&
