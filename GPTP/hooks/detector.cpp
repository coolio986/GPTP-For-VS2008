...
bool unitCanDetectHook(CUnit* unit) {
    return (units_dat::BaseProperty[unit->id] & UnitProperty::Detector &&
            ...
            unit->maelstromTimer == 0 && unit->isBlind == 0);
}

// Check related to the question of the @p detector being able to see the @p
// param->target (assuming target is cloaked). Used in a IterateUnitsAtLocation
// function (thus the special structure) Part of a bigger logic, probably should
// not be touched
void getCloakedTargetVisibility(CUnit* detector, DetectorCheckParam* param) {
    CUnit* target = param->target;

    if (!(detector->status &
          UnitStatus::IsHallucination) &&  // hallucinations cannot detect
        ...
        if (target->sprite->visibilityFlags & visibilityFlags) {
            ...
            if (detector->isTargetWithinMinRange(target, detectionRange)) {
                ...
                param->visionFlags |=
                    detector->parasiteFlags |  // detector's parasites player
                                               // owner flags (8 bits)
                    playerVision
                        ->flags[detector->playerId] |  // default visibility (32
                                                       // bits)
