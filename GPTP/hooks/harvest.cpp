    ...
    if (resource->building.resource.resourceAmount < RESOURCE_HARVESTED) {
        ...
        if (isMineral) {
            if (resource->building.resource.resourceAmount > 0)
                updateMineralPatchImage(
                    resource);  // equivalent to setResourceCount @ 68830 ?
            else
                resource->remove();
        } else if (resource->building.resource.resourceAmount <
                   RESOURCE_HARVESTED)
            scbw::showErrorMessageWithSfx(
                resource->playerId, 875, 20);  // Gas depleted message and sound
...
void transferResourceToWorkerHook(CUnit* worker, CUnit* resource) {
    if (resource->id >= UnitId::TerranRefinery &&
        ...
        if (resource->id >= UnitId::ResourceMineralField &&
            resource->id <=
        ...
        if (!bEndThere) {
            ...
            if (resourceAmount > 0) {
                if (worker->resourceType & 3) {  // Is carrying a mineral / gas
                    ...
                    // Remove powerup shadows (mineral chunk shadow, psi emitter
                    // shadow, etc.)
                    worker->sprite->removeOverlay(
                        ImageId::MineralChunkShadow,
                        ImageId::PsiEmitterShadow_Carried);

                    // Remove powerup graphics
                    worker->sprite->removeOverlay(ImageId::Flag,
                                                  ImageId::TerranGasTankType2);

                    // Remove Uraj / Khalis
                    worker->sprite->removeOverlay(ImageId::Uraj,
                                                  ImageId::Khalis);
                    worker->resourceType = 0;
                }

                setResourceAmountCarried(
                    worker, resourceAmount, chunkImageId, isMineral);
            }
        }
    }
