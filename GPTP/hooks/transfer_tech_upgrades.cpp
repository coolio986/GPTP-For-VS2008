...
// passed from one player to another, either by triggers or casting Mind
// Control.
...
struct TechTransferData {
    u16 unitId;
    u16 relatedTech[5];  // Maximum number of tech (use TechId::None to mark a
...
const UpgradeTransferData upgradeTransferData[] = {
    ...
    {UnitId::ghost,
     UpgradeId::OcularImplants,
     UpgradeId::MoebiusReactor,
     UpgradeId::None},
    ...
    // Zerg
    {UnitId::overlord,
     UpgradeId::VentralSacs,
     UpgradeId::Antennae,
     UpgradeId::None},
    {UnitId::zergling,
     UpgradeId::MetabolicBoost,
     UpgradeId::AdrenalGlands,
     UpgradeId::None},
    {UnitId::hydralisk,
     UpgradeId::MuscularAugments,
     UpgradeId::GroovedSpines,
     UpgradeId::None},
    ...
    {UnitId::reaver,
     UpgradeId::ScarabDamage,
     UpgradeId::ReaverCapacity,
     UpgradeId::None},
    {UnitId::shuttle, UpgradeId::GraviticDrive, UpgradeId::None},
    {UnitId::observer,
     UpgradeId::SensorArray,
     UpgradeId::GraviticBoosters,
     UpgradeId::None},
    {UnitId::scout,
     UpgradeId::ApialSensors,
     UpgradeId::GraviticThrusters,
     UpgradeId::None},
...
}  // bool researchHelper(u32 researchTechFunc, CUnit* sourceUnit, u32 techId,
   // u32 relativeVariable, u32 destPlayerId)
...
void transferUnitTechToPlayerHook(CUnit* sourceUnit,
    ...
    if (researchTechFunc == NULL || sourceUnit == NULL)
        ...
        for (TechTransferData* data = (TechTransferData*)techDataArrayPosBegin;
             !bBreakMainLoop && (data != techDataArrayPosEnd);
             data++) {
            if (sourceUnit->id == data->unitId) {
                ...
                for (int techIndex = 0;
                     !bBreakSubLoop && techIndex < relatedTechArraySize;
                     techIndex++) {
                    ...
                    if (techId == TechId::None)
                        ...
                        if (techId >= TechId::Restoration)
                            bIsResearched =
                                TechBw->isResearched[sourceUnit->playerId]
                                                    [techId -
                                                     TechId::Restoration];
                        else
                            bIsResearched =
                                TechSc->isResearched[sourceUnit->playerId]
                                                    [techId];

                        // probably research the tech of unit if not already
                        // researched by player if return false, stop checking
                        // techs and TechTransferData
                        if (!researchHelper(researchTechFunc,
                                            sourceUnit,
                                            techId,
                                            bIsResearched,
                                            targetPlayerId)) {
        ...
        }  // for (TechTransferData* data =
           // (TechTransferData*)techDataArrayPosBegin; !bBreakMainLoop && (data
           // != techDataArrayPosEnd); data++)

    }  // if (researchTechFunc != NULL && sourceUnit != NULL)

}  // void transferUnitTechToPlayerHook(CUnit* sourceUnit, u8 targetPlayerId,
   // u32 researchTechFunc)
...
void transferUnitUpgradesToPlayerHook(CUnit* sourceUnit,
    ...
    const UpgradeTransferData* upgradeDataArrayPosBegin =
        &upgradeTransferData[0];  // original was (UpgradeTransferData*)
                                  // 0x005012E2;
    const UpgradeTransferData* upgradeDataArrayPosEnd =
        &upgradeTransferData[ARRAY_SIZE(
            upgradeTransferData)];  // original was (UpgradeTransferData*)
                                    // 0x005013DC;
    ...
    if (researchUpgradesFunc == NULL || sourceUnit == NULL)
        ...
        for (UpgradeTransferData* data =
                 (UpgradeTransferData*)upgradeDataArrayPosBegin;
             !bBreakMainLoop && (data != upgradeDataArrayPosEnd);
             data++) {
            if (sourceUnit->id == data->unitId) {
                bBreakSubLoop = false;
                ...
                for (int upgradeIndex = 0;
                     !bBreakSubLoop && upgradeIndex < relatedUpgradeArraySize;
                     upgradeIndex++) {
                    ...
                    if (upgradeId == UpgradeId::None)
                        ...
                        if (upgradeId >= UpgradeId::UnusedUpgrade46)
                            sourceUpgradeLevel =
                                UpgradesBw
                                    ->currentLevel[sourceUnit->playerId]
                                                  [upgradeId -
                                                   UpgradeId::UnusedUpgrade46];
                        else
                            sourceUpgradeLevel =
                                UpgradesSc->currentLevel[sourceUnit->playerId]
                                                        [upgradeId];

                        // probably increase the upgrade level of player until
                        // reaching same point as sourceUnit if return false,
                        // stop checking upgrades and UpgradeTransferData
                        if (!researchHelper(researchUpgradesFunc,
                                            sourceUnit,
                                            upgradeId,
                                            sourceUpgradeLevel,
                                            targetPlayerId)) {
           ...
           // (upgradeTransferData*)upgradeDataArrayPosBegin; !bBreakMainLoop &&
           // (data != upgradeDataArrayPosEnd); data++)
