...
bool isValidPsiProviderType(u16 unitId) {
    ...
    if (units_dat::BaseProperty[unitId] &
        (UnitProperty::Worker | UnitProperty::ResourceContainer |
         UnitProperty::NPCOrAccessories))
        return false;
    ...
    if (unitId == UnitId::ghost || unitId == UnitId::sarah_kerrigan ||
        ...
        unitId == UnitId::Hero_InfestedDuran || unitId == UnitId::nydus_canal ||
        unitId == UnitId::nuclear_silo || unitId == UnitId::hatchery ||
        unitId == UnitId::lair || unitId == UnitId::hive)
        return false;
...
void addPsiField(CUnit* unit) {
    ...
    if (*firstPsiFieldProvider == unit || unit->psi_link.next ||
        unit->psi_link.prev)
        return;

    unit->psi_link.prev = NULL;
    unit->psi_link.next = *firstPsiFieldProvider;
    if (*firstPsiFieldProvider) (*firstPsiFieldProvider)->psi_link.prev = unit;
