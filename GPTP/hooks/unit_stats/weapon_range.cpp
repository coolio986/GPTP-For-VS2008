// Contains hooks that control attack range and seek range (AKA target
// acquisition range).
...
u8 getSeekRangeHook(CUnit* unit) {
    ...
    if ((unitId == UnitId::ghost || unitId == UnitId::sarah_kerrigan ||
        ...
        seekRange = 0;
    else if (unitId > UnitId::Hero_FenixDragoon)
        ...
        switch (unitId) {
            case UnitId::TerranMarine:
                if (scbw::getUpgradeLevel(unit->playerId,
                                          UpgradeId::U_238Shells))
                    bonusAmount = 1;
                break;
...
u32 getMaxWeaponRangeHook(CUnit* unit, u8 weaponId) {
    ...
    if (unit->status & UnitStatus::InBuilding) bonusAmount = 64;
    ...
    switch (unit->id) {
        ...
        case UnitId::fenix_dragoon:
            if (scbw::isBroodWarMode()) bonusAmount += 64;
            break;
        case UnitId::goliath:
