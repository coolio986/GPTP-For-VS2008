...
void RestoreAllUnitStats(
    CUnit* unit);  // 0x00492F70 (hooked by hooks\update_status_effects.cpp)
...
void updateUnitEnergy(CUnit* unit) {
    ...
    if (unit->isValidCaster() &&  // If the unit is hallucination or not a
                                  // spellcaster, don't regenerate energy
        (unit->status &
         UnitStatus::Completed)  // If the unit is not fully constructed, don't
                                 // regenerate energy
    ) {
        ...
        if (unit->status &
            ...
            if (!(unit->hasEnergy(cloakingEnergyCost))) {
                if (unit->secondaryOrderId == OrderId::Cloak)
                    unit->setSecondaryOrder(
                        OrderId::Nothing2);  // Supposedly, immediately decloaks
                                             // the unit.

                bStopHere = true;
            }

            if (!bStopHere)  // the function prevent energy consumption if THE
                             // GATHERING enabled
            ...
            if (unit->id == UnitId::ProtossDarkArchon &&
                ...
                maxEnergy =
                    12800;  // 50 * 256; Identical to energy amount on spawn
            else
            ...
            if (unit->energy == maxEnergy)
                ...
                if (unit->energy > maxEnergy) unit->energy = maxEnergy;
            }
        }
...
/// Updates unit timers, regenerates hp and shields, and burns down Terran
/// buildings. Identical to function @ 0x004EC290
void updateUnitStateHook(CUnit* unit) {
    // Timers
    if (unit->mainOrderTimer) unit->mainOrderTimer--;
    if (unit->groundWeaponCooldown) unit->groundWeaponCooldown--;
    if (unit->airWeaponCooldown) unit->airWeaponCooldown--;
    if (unit->spellCooldown) unit->spellCooldown--;
    ...
    if (units_dat::ShieldsEnabled[unit->id] != 0) {
        ...
        if (unit->shields != maxShields) {
            ...
            if (unit->shields > maxShields) unit->shields = maxShields;

            if (unit->sprite->flags &
                CSprite_Flags::Selected)  // If the unit is currently selected,
                                          // redraw its graphics
                setAllImageGroupFlagsPal11(unit->sprite);
        }
    }
    ...
    if (unit->id == UnitId::zergling || unit->id == UnitId::Hero_DevouringOne)
        if (unit->groundWeaponCooldown == 0) unit->orderQueueTimer = 0;
    ...
    if (unit->status & UnitStatus::Completed ||
        ...
        if (unit->cycleCounter >= 8) {
            unit->cycleCounter = 0;
            RestoreAllUnitStats(
                unit);  // use update_status_effects hook if available
        }
    }
    ...
    if (unit->status & UnitStatus::Completed) {
        ...
        // Recent order timer
        if (unit->recentOrderTimer != 0) unit->recentOrderTimer--;
        ...
        if (!bStopHere) {
            ...
            if (raceId != RaceId::Zerg && raceId != RaceId::Protoss &&
