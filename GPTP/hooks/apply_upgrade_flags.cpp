...
void applyUpgradeFlagsToNewUnitHook(CUnit* unit) {
    ...
    if (unit->id >= UnitId::TerranVulture && unit->id <= UnitId::ZergLurker) {
        // slightly different from original code, because completely separating
        // Bool8 calculation and update of speed to do everything at once after
        // the checks Note that updateSpeed() is called once even without
        // upgrades

        if (unit->id == UnitId::TerranVulture ||
            unit->id == UnitId::Hero_JimRaynorVulture)
            bSpeedUpgrade =
                UpgradesSc
                    ->currentLevel[unit->playerId][ScUpgrade::IonThrusters];
        else if (unit->id == UnitId::ZergOverlord)
            bSpeedUpgrade =
                UpgradesSc->currentLevel[unit->playerId]
                                        [ScUpgrade::PneumatizedCarapace];
        else if (unit->id == UnitId::ZergZergling) {
            bSpeedUpgrade =
                UpgradesSc
                    ->currentLevel[unit->playerId][ScUpgrade::MetabolicBoost];
            bCooldownUpgrade =
                UpgradesSc
            ...
            bSpeedUpgrade =
                UpgradesSc
                    ->currentLevel[unit->playerId][ScUpgrade::MuscularAugments];
        else if (unit->id == UnitId::ProtossZealot)
            bSpeedUpgrade =
                UpgradesSc
                    ->currentLevel[unit->playerId][ScUpgrade::LegEnhancements];
        else if (unit->id == UnitId::ProtossScout)
            bSpeedUpgrade =
                UpgradesSc->currentLevel[unit->playerId]
                                        [ScUpgrade::GraviticThrusters];
        else if (unit->id == UnitId::ProtossShuttle)
            bSpeedUpgrade =
                UpgradesSc
                    ->currentLevel[unit->playerId][ScUpgrade::GraviticDrive];
        else if (unit->id == UnitId::ProtossObserver)
            bSpeedUpgrade =
                UpgradesSc
                    ->currentLevel[unit->playerId][ScUpgrade::GraviticBoosters];
        else if (unit->id == UnitId::ZergUltralisk)
            bSpeedUpgrade =
                UpgradesSc->currentLevel[unit->playerId]
                                        [BwUpgrade::AnabolicSynthesis];
        else if (unit->id == UnitId::Hero_Torrasque)
            bSpeedUpgrade = *IS_BROOD_WAR;
        else if (unit->id == UnitId::Hero_HunterKiller ||
    ...
    }  // if(unit->id >= UnitId::TerranVulture && unit->id <=
       // UnitId::ZergLurker)
}
...
void applyUpgradeFlagsToExistingUnitsHook(CUnit* unit, u8 upgradeId) {
    ...
    if (upgradeId >= UpgradeId::IonThrusters &&
        ...
        if (Func_UpgradeRestrictionProc != NULL) {
            ...
            while (current_unit != NULL) {
                ...
                if (upgradeRestrictionProc(current_unit,
                                           unitTypeUpgradedId,
                                           Func_UpgradeRestrictionProc)) {
                    if (bCooldownUpgrade)
    ...
    }  // if(upgradeId >= UpgradeId::IonThrusters && upgradeId <=
