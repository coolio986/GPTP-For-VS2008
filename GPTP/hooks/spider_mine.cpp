...
class SpiderMineTargetFinder : public scbw::UnitFinderCallbackMatchInterface {
   ...
   public:
    ...
    bool match(CUnit* target) {
        ...
        if (!spiderMine->isTargetEnemy(target)) return false;
...
CUnit* findBestSpiderMineTargetHook(CUnit* spiderMine) {
    ...
    if (spiderMine->status & UnitStatus::CanNotAttack) return NULL;

    range = 32 * spiderMine->getSeekRange();

    return scbw::UnitFinder::getNearestTarget(
        spiderMine->getX() - range,
        spiderMine->getY() - range,
        spiderMine->getX() + range,
        spiderMine->getY() + range,
        spiderMine,
        SpiderMineTargetFinder(spiderMine));
}
...
void orders_VultureMine(CUnit* unit) {
    ...
    if (unit->mainOrderState == 4) {
    ...
    } else  // unlike the previous states, going directly from state 4 to 5 in
        ...
        if (unit->orderTarget.unit != NULL) {
            if (unit->isTargetWithinMinRange(unit->orderTarget.unit,
                                             SPIDERMINE_SEEKING_RANGE)) {
                // probably trigger movement/acceleration of unit
                // toward the target
                function_00495400(unit, unit->orderTarget.unit);

                if (unit->isTargetWithinMinRange(unit->orderTarget.unit,
                                                 SPIDERMINE_DETONATE_RANGE))
                    bIsCompletelyInRange = true;        // jump to 63F22
                else if (unit->getMovableState() != 2)  // unit not unmovable
                    bEndThere = true;
            }
        }
