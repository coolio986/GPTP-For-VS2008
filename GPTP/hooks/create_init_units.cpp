...
void CreateInitialMeleeBuildings(u8 raceId, u32 playerId) {
    ...
    else if (raceId == RaceId::Terran)
        buildingId = UnitId::TerranCommandCenter;
    else if (raceId == RaceId::Protoss)
    ...
    if (buildingSizeX < 0) buildingSizeX++;
    if (buildingSizeY < 0) buildingSizeY++;

    searchBox.left =
        (startPositions[playerId].x - (s16)(buildingSizeX / 2)) & 0x0000FFE0;
    searchBox.top =
        (startPositions[playerId].y - (s16)(buildingSizeY / 2)) & 0x0000FFE0;
    searchBox.bottom =
        buildingSizeY / 2 +
        ((startPositions[playerId].y - (s16)(buildingSizeY / 2)) & 0x0000FFE0) +
        buildingSizeY / 2 - 1;
    ...
    created_unit = createUnit(
        buildingId,
        ((startPositions[playerId].x - (buildingSizeX / 2)) & 0x0000FFE0) +
            buildingSizeX / 2,
        ((startPositions[playerId].y - (buildingSizeY / 2)) & 0x0000FFE0) +
            buildingSizeY / 2,
        playerId);
    ...
    if (created_unit != NULL) {
        ...
        if (spreadsCreep(created_unit->id, 1) ||
            units_dat::BaseProperty[created_unit->id] &
                UnitProperty::CreepBuilding) {
            function_004148F0(created_unit->sprite->position.x,
        ...
        if (*(raceIdOffset - 1) == 2 || *(raceIdOffset - 1) == 1) {
            ...
            if (*victory_conditions == 0 &&  // check triggers for conditions
                ...
                *tournament_mode == 0 &&     // tournament mode off
                playerId < 8 &&
                (user_select_slots[playerId]) !=
                    0  // race selected instead of using map settings?
            )
                startingUnits = 2;
            ...
            if (startingUnits == 1 || startingUnits == 2) {
                ...
                else if (*raceIdOffset == RaceId::Terran)
                    unitId = UnitId::TerranSCV;
                else if (*raceIdOffset == RaceId::Protoss)
                    unitId = UnitId::ProtossProbe;
                else
                    unitId = UnitId::None;

                created_unit = createUnit(unitId,
                                          startPositions[playerId].x,
...
void CreateInitialOverlord(u8 playerId) {
    ...
    if (startPositions[playerId].x < *maxBoxRightValue / 2)
        overlordStartX = startPositions[playerId].x +
                         units_dat::UnitBounds[UnitId::ZergHatchery].right +
                         units_dat::UnitBounds[UnitId::ZergHatchery].left + 1;
    else
        overlordStartX = startPositions[playerId].x -
                         (units_dat::UnitBounds[UnitId::ZergHatchery].right +
                          units_dat::UnitBounds[UnitId::ZergHatchery].left + 1);
    ...
    if (startPositions[playerId].y < *maxBoxBottomValue / 2)
        overlordStartY = startPositions[playerId].y +
                         units_dat::UnitBounds[UnitId::ZergHatchery].bottom +
                         units_dat::UnitBounds[UnitId::ZergHatchery].top + 1;
    else
        overlordStartY = startPositions[playerId].y -
                         (units_dat::UnitBounds[UnitId::ZergHatchery].bottom +
                          units_dat::UnitBounds[UnitId::ZergHatchery].top + 1);
    ...
    created_unit = createUnit(
        UnitId::ZergOverlord, overlordStartX, overlordStartY, playerId);
