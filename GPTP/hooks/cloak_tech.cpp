    ...
    for (int i = 0; i < SELECTION_ARRAY_LENGTH && (bReturnValue == TRUE); i++) {
        ...
        if (unit_from_selection != NULL) {
            ...
            if (unit_from_selection->canUseTech(techId, playerId) == TRUE) {
                if (unit_from_selection->status & UnitStatus::CloakingForFree &&
                    !(unit_from_selection->status & UnitStatus::Burrowed))
                    bReturnValue = FALSE;
                else if (!(unit_from_selection->status &
...
Bool32 BTNSCOND_CanCloak(CUnit* unit, u32 playerId, u16 reqVar) {
    ...
    for (int i = 0; i < SELECTION_ARRAY_LENGTH && (bReturnValue == FALSE);
         i++) {
        unit_from_selection = clientSelectionGroup->unit[i];
        ...
        if (unit_from_selection != NULL) {
            ...
            if (unit_from_selection->canUseTech(techId, playerId) ==
                    TRUE &&  // must be able to use cloaking tech for more
                             // checks to be necessary
                (!(unit_from_selection->status &
                   UnitStatus::CloakingForFree) ||  // to cloak, unit should not
                                                    // already be cloaked for
                                                    // free
                 unit_from_selection->status &
                     UnitStatus::Burrowed) &&  // if burrowed, cloaking for
                                               // free, but still allowed to
                                               // cloak
                !(unit_from_selection->status &
                  UnitStatus::RequiresDetection)  // if unit doesn't already
                                                  // require detection
            )
                bReturnValue = TRUE;
        }
