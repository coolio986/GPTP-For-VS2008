...
* 0x00429210: BTNSCOND_IsCloaked  (Used by Infested Kerrigan, Infested Duran,
and Mixed Cloakable Group)
* 0x004292C0: BTNSCOND_CanCloak   (Used by Infested Kerrigan, Infested Duran,
and Mixed Cloakable Group)
...
* 0x00491A50: getCloakingTech     (Used by "Create Unit With Properties" in
StarEdit + create_unit opcode in aiscript.bin)
* 0x00491A90: getCloakingEnergyConsumption (Used by energy regen; This is
editable from the energy hook stuff)
* 0x00423540: currentUnitSelectionCanCloak(Called by CMDACT_Cloak for all units;
also checks energy amount)
...
void applyCloakingOrderHook(CUnit* unit) {
    if (unit->status & UnitStatus::RequiresDetection) return;

    // original code is hardcoded instead of using a function like
    // getCloakingTech
    const u16 energyCost =
        techdata_dat::EnergyCost[hooks::getCloakingTech(unit)] * 256;
...
Bool32 __cdecl currentUnitSelectionCanCloakWrapper() {
    ...
    for (int i = 0; i < SELECTION_ARRAY_LENGTH; ++i) {
        ...
        if (unit != NULL) {
            if (scbw::isCheatEnabled(CheatFlags::TheGathering) ||
                unit->energy >=
                    techdata_dat::EnergyCost[hooks::getCloakingTech(unit)] *
                        256)
                return true;
        }
    }
    ...
    scbw::showErrorMessageWithSfx(
        (*activePortraitUnit)->playerId, 864 + race, 156 + race);
