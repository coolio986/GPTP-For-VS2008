...
void InitializeDialog(BinDlg* dialog, void* dlgFunc) {
    ...
    dialog->flags |=
        (BinDlgFlags::NoClickSound + BinDlgFlags::VerticalAlignmentBottom);
    dialog->onInitDialog0x46.unk4a = 0;

    if (dlgFunc != NULL) dialog->fxnInteract = dlgFunc;

    dialog->onInitDialog0x36.responseAreaBitArray =
        (u32*)SMemAlloc(dialog->onInitDialog0x36.responseAreaWidth *
                            dialog->onInitDialog0x36.responseAreaHeight,
                        0x00505F98,
                        481,  // 0x000001E1
                        0);
    ...
    // equivalent to "if(dialog->flags & BinDlgFlags::VerticalAlignmentBottom)
...
bool unit_IsStandardAndMovable(CUnit* unit) {
    ...
    if (units_dat::BaseProperty[unit->id] & UnitProperty::Building ||
        ...
        unit->id == UnitId::ZergEgg || unit->id == UnitId::Critter_Rhynadon ||
...
void      SetCallbackTimer(BinDlg* dialog, u32 unk1, u32 unk2, u32 unk3){
    ...
    __asm {PUSHAD PUSH unk3 PUSH unk2 MOV EAX,
           unk1 MOV ECX,
           dialog CALL Func_SetCallbackTimer POPAD}
